        ...
        for obj_gt, obj_gt_info in zip(img_gt, img_gt_info):
            if obj_gt_info["visib_fract"] >= args.min_visib_gt:
                ...
                if args.stdo > 0.0:
                    ...
                    rot_vec_pert = rng.normal(
                        loc=0.0, scale=args.stdo * np.ones(3)
                    )
                    cam_R_m2c = (
                        cam_R_m2c @ R.from_rotvec(rot_vec_pert).as_matrix()
                    )
                if args.stdt > 0.0:
                    cam_t_m2c += rng.normal(
                        loc=0.0, scale=args.stdt * np.ones(3)
                    ).reshape((3, 1))
                result = {
