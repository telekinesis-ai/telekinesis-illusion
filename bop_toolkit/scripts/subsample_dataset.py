...
def subsample_images(num_images, scene_gt, scene_gt_info):
    ...
    for im_id in im_ids:
        for gt_id in range(len(scene_gt[im_id])):
            ...
            R = np.eye(4)
            R[:3, :3] = np.array(scene_gt[im_id][gt_id]["cam_R_m2c"]).reshape(
                3, 3
            )
            ...
            gt_desc = (
                quat_dists + t + [scene_gt_info[im_id][gt_id]["visib_fract"]]
            )
    ...
    for im_id in im_ids:
        ...
        descs_inds = [
            i
            for i, src_im_id in enumerate(gt_src_ims_list)
            if src_im_id == im_id
        ]
...
for scene_id in scene_ids:
    print(
        "Processing dataset at {}, scene: {}".format(args.dataset_dir, scene_id)
    )
    ...
    im_ids_selected = subsample_images(
        args.num_scene_images, scene_gt, scene_gt_info
    )
    im_ids_selected = sorted(im_ids_selected)
