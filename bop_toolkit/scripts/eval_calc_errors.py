...
# ------------------------------------------------------------------------------
vsd_deltas_str = ",".join(
    ["{}:{}".format(k, v) for k, v in p["vsd_deltas"].items()]
)
...
for result_filename in p["result_filenames"]:
    ...
    if p["error_type"] not in dp_split["supported_error_types"]:
        raise ValueError(
            """{} error is not among {} """
            """supported error types: {}""".format(
                p["error_type"], dataset, dp_split["supported_error_types"]
            )
        )
    ...
    if p["error_type"] in ["vsd", "cus"]:
        ...
        for obj_id in dp_model["obj_ids"]:
            ren.add_object(
                obj_id, dp_model["model_tpath"].format(obj_id=obj_id)
            )
    ...
    for target in targets:
        if p["eval_mode"] == "localization":
            assert "inst_count" in target, (
                "inst_count is required for localization eval_mode"
            )
            targets_org.setdefault(target["scene_id"], {}).setdefault(
                target["im_id"], {}
            )[target["obj_id"]] = target
        else:
            targets_org.setdefault(target["scene_id"], {})[target["im_id"]] = (
                target
            )

    # Load pose estimates.
    logger.info("Loading pose estimates...")
    max_num_estimates_per_image = (
        p["max_num_estimates_per_image"]
        if p["eval_mode"] == "detection"
        else None
    )
    ests = inout.load_bop_results(
        os.path.join(p["results_path"], result_filename),
        max_num_estimates_per_image=max_num_estimates_per_image,
    )
    ...
    for scene_id, scene_targets in targets_org.items():
        ...
        scene_gt_info = inout.load_json(
            dp_split[tpath_keys["scene_gt_info_tpath"]].format(
                scene_id=scene_id
            ),
            keys_to_int=True,
        )
        ...
        def calculate_errors_per_image(im_metaData):
            ...
            if "cam_K" in scene_camera[im_id]:
                ...
                "cam_model" in scene_camera[im_id]
                and scene_camera[im_id]["cam_model"]
            ...
            if p["error_type"] in ["vsd", "cus", "proj"]:
                assert K is not None, (
                    "Error type {} is not supported for non pinhole cameras".format(
                        p["error_type"]
                    )
                )
            ...
            for obj_id, target in im_targets.items():
                ...
                for est_id, est in obj_ests_sorted:
                    ...
                    for gt_id, gt in enumerate(scene_gt[im_id]):
                        ...
                        R_g = gt["cam_R_m2c"]
                        t_g = gt["cam_t_m2c"]
                        gt_visib_fract = scene_gt_info[im_id][gt_id][
                            "visib_fract"
                        ]
                        ...
                        if p["error_type"] == "vsd":
                            ...
                            if cam is not None:
                                ...
                                raise ValueError(
                                    "Either 'K' or 'cam_model' should be defined at this point"
                                )
                ...
                assert len(im_errs) == per_image_ests_counter, (
