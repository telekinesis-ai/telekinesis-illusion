...
parser.add_argument(
    "--use_all_gt", action="store_true", default=p["use_all_gt"]
)
...
split_type = (
    None if args.dataset_split_type is None else str(args.dataset_split_type)
)
...
for scene_id in tqdm(dp_split["scene_ids"], colour="green"):
    ...
    # Output coco path
    coco_gt_path = dp_split[tpath_keys["scene_gt_coco_tpath"]].format(
        scene_id=scene_id
    )
    if args.bbox_type == "modal":
        coco_gt_path = coco_gt_path.replace(
            "scene_gt_coco", "scene_gt_coco_modal"
        )
    misc.log(
        f"Calculating Coco Annotations - dataset: {args.dataset} ({args.dataset_split}, {args.dataset_split_type}), scene: {scene_id}"
    )
    ...
    for im_id, inst_list in tqdm(scene_gt.items(), colour="blue"):
        ...
        # For classic BOP datasets whose only visual modality is "gray"
        # (e.g. itodd), use gray_tpath instead of the default rgb_tpath.
        img_tpath_key = tpath_keys["rgb_tpath"]
        if (
            img_tpath_key == "rgb_tpath"
            and isinstance(dp_split.get("im_modalities"), list)
            and "rgb" not in dp_split["im_modalities"]
            and "gray" in dp_split["im_modalities"]
        ):
            img_tpath_key = "gray_tpath"
        img_path = dp_split[img_tpath_key].format(
            scene_id=scene_id, im_id=im_id
        )
        relative_img_path = os.path.relpath(
            img_path, os.path.dirname(coco_gt_path)
        )
        im_size = dataset_params.get_im_size(
        ...
        for idx, inst in enumerate(inst_list):
            ...
            if args.bbox_type == "amodal":
                binary_inst_mask_full = inout.load_depth(mask_full_p).astype(
                    bool
                )
