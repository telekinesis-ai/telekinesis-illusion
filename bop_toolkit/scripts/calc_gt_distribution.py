...
for scene_id in scene_ids:
    ...
    misc.log(
        f"Processing - dataset: {p['dataset']} ({p['dataset_split']}, {p['dataset_split_type']}), scene: {scene_id}"
    )
    ...
    scene_gt_info_path = dp_split[tpath_keys["scene_gt_info_tpath"]].format(
        scene_id=scene_id
    )
...
# Print stats.
misc.log(
    "Stats of the GT poses in dataset {} {}:".format(
...
prefix = (
    f"{p['modality']}_{p['sensor']}_" if isinstance(p["modality"], str) else ""
)
