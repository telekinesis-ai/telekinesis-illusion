...
for result_filename in p["result_filenames"]:
    ...
    check_passed, check_msg = inout.check_coco_results(
        results_path, ann_type=p["ann_type"]
    )
    if not check_passed:
    ...
    if not results_org:
        misc.log(
            "No valid coco results for annotation type: {}".format(
                p["ann_type"]
            )
        )
    ...
    for i, scene_id in enumerate(targets_org):
        ...
        scene_coco_ann_path = dp_split[
            tpath_keys["scene_gt_coco_tpath"]
        ...
        scene_coco_results = (
            results_org[scene_id] if scene_id in results_org else []
        )
        ...
        scene_coco_ann["images"] = [
            img
            for img in scene_coco_ann["images"]
        ...
        scene_coco_results = [
            res
            for res in scene_coco_results
            if res["image_id"] in target_img_ids
        ]
        ...
        if i == 0:
            ...
            dataset_coco_ann, image_id_offset = (
                pycoco_utils.merge_coco_annotations(
                    dataset_coco_ann, scene_coco_ann
                )
            )
    ...
    _, _, times, times_available = inout.check_consistent_timings(
        coco_results, "image_id"
    )
    ...
    try:
        ...
        cocoEval.summarize()
        coco_scores = {
            res_types[i]: stat for i, stat in enumerate(cocoEval.stats)
        }

        # Calculate the average estimation time per image.
        coco_scores["average_time_per_image"] = (
            np.mean(list(times.values())) if times_available else -1.0
        )
    ...
    final_scores_path = os.path.join(
        p["eval_path"],
        result_name,
        "scores_bop22_coco_{}.json".format(p["ann_type"]),
    )
    if p["ann_type"] == "bbox" and p["bbox_type"] == "modal":
