...
parser.add_argument(
    "--dont_use_all_gt", action="store_false", dest="use_all_gt"
)
...
for scene_id in scene_ids:
    tpath_keys = dataset_params.scene_tpaths_keys(
        p["modality"], p["sensor"], scene_id
    )

    # Load scene GT.
    scene_camera_path = dp_split[tpath_keys["scene_camera_tpath"]].format(
        scene_id=scene_id
    )
    scene_camera = inout.load_scene_camera(scene_camera_path)
    scene_gt_path = dp_split[tpath_keys["scene_gt_tpath"]].format(
        scene_id=scene_id
    )
    ...
    mask_dir_path = os.path.dirname(
        dp_split[tpath_keys["mask_tpath"]].format(
    ...
    mask_visib_dir_path = os.path.dirname(
        dp_split[tpath_keys["mask_visib_tpath"]].format(
            scene_id=scene_id, im_id=0, gt_id=0
        )
    )
    ...
    for im_id in scene_gt:
        ...
        # Load depth image.
        depth_fpath = dp_split[tpath_keys["depth_tpath"]].format(
