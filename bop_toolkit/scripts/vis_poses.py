...
def main(args):
    ...
    dp_model = dataset_params.get_model_params(
        args.datasets_path, dataset, model_type
    )
    ...
    if args.mode == "gt":
        ...
        if args.targets_filename is not None:
            ...
            for target in targets:
                scene_im_ids.setdefault(target["scene_id"], set()).add(
                    target["im_id"]
                )
        else:
            scene_im_ids = None
        ...
        ests = inout.load_bop_results(
            os.path.join(args.results_path, result_filename)
        )
    ...
    if (
        vis_depth_diff
        or (vis_rgb and args.vis_rgb_resolve_visib)
        or do_extra_vis
    ):
    ...
    for scene_id in tqdm(scene_ids, desc="Scenes"):
        ...
        vis_save_dir = None
        for im_id, poses_img in tqdm(
            poses_scene_vis.items(), desc=f"Scene {scene_id}"
        ):
            ...
            if vis_rgb:
                inout.save_im(
                    vis_rgb_path, vis_res["vis_im_rgb"], jpg_quality=95
                )
            if vis_depth_diff:
            ...
            if do_extra_vis:
                ...
                bres_est = get_depth_map_and_obj_masks_from_renderings(
                    res_per_obj_est
                )
                ...
                if "contour" in args.vis_types:
                    ...
                    for idx in range(len(res_per_obj_est)):
                        ...
                        percent = calc_mask_visib_percent(
                            mask_obj, depth_obj_mask
                        )
                ...
                if "bbox3d" in args.vis_types:
                    ...
                    for idx in range(len(res_per_obj_est)):
                        ...
                        mesh = trimesh.Trimesh(
                            vertices=pts, faces=model["faces"]
                        )
                        ...
                        for sym in syms_per_obj[obj_id]:
                            ...
                            pts_gt_sym = misc.transform_pts_Rt(
                                pts, R_gt_sym, t_gt_sym
                            )
...
if __name__ == "__main__":
    ...
    if args.vis_path is None:
        ...
        args.vis_path = os.path.join(
            config.output_path, f"vis_{vis_type}_poses"
        )
