...
for scene_id in tqdm(dp_split["scene_ids"]):
    ...
    scene_gt_path = os.path.join(
        ...
        dp_split["split"],
        f"{scene_id:06d}",
        f"scene_gt{prefix}.json",
    )
    scene_gt_info_path = os.path.join(
        dp_split["base_path"],
        dp_split["split"],
        f"{scene_id:06d}",
        f"scene_gt_info{prefix}.json",
    )
    if not os.path.exists(scene_gt_path) or not os.path.exists(
        scene_gt_info_path
    ):
    ...
    if p["dataset_name"] == "hot3d":
        ...
        gt_available, num_instances = check_scene_gt(
            scene_gt, scene_gt_info, selected_im_ids
        )
        assert gt_available, (
            "Missing image ids in the scene_gt or scene_gt_info."
        )
    elif p["dataset_name"] == "handal":
        avail_im_ids = sorted(list(scene_gt.keys()))
        # select with frame_rate = 0.125, i.e. every 8 frames
        selected_im_ids = [
            im_id for idx, im_id in enumerate(avail_im_ids) if idx % 8 == 0
        ]
        _, num_instances = check_scene_gt(
            scene_gt, scene_gt_info, selected_im_ids
        )
    else:
...
print(
    f"Total number of test images: {len(test_list)}, total number of instances: {total_num_instances}"
)
