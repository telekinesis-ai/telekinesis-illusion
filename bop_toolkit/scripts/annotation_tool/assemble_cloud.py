...
def cloud_gpu(cloud):  # TODO use the function to get dynamic target
    ...
    cloud_t.point["colors"] = (
        cloud_t.point["colors"].to(o3d.core.Dtype.Float32) / 255.0
    )
...
def main():
    dataset_split_path = os.path.join(
        p["dataset_path"],
        p["dataset_split"] + "_" + p["dataset_split_type"]
        if p["dataset_split_type"]
        else p["dataset_split"],
    )
    ...
    if p["scene_ids"] is None:
        ...
        p["scene_ids"] = [
            int(os.path.basename(scene_path)) for scene_path in scenes_paths
        ]
        p["scene_ids"].sort()

    scenes_paths = [
        os.path.join(dataset_split_path, f"{scene_id:06d}")
    ...
    for scene_path in scenes_paths:  # samples are not ordered
        ...
        assembled_cloud_file = os.path.join(
        ...
        for index in tqdm(scene_camera_data.keys()):
            ...
            world2cam = np.eye(4)
            world2cam[:3, :3] = np.array(
                scene_camera_data[str(index)]["cam_R_w2c"]
            ).reshape(3, 3)
            world2cam[:3, 3] = np.array(
                scene_camera_data[str(index)]["cam_t_w2c"]
            )
            ...
            depth_scale = (
                1 / scene_camera_data[str(index)]["depth_scale"]
            )  # depth scale in mm - open3d need inverse of BOP depth scale
            ...
            assembled_cloud += transformed_cloud.voxel_down_sample(
                voxel_size=p["voxel_size"]
            )

        assembled_cloud = assembled_cloud.voxel_down_sample(
            voxel_size=p["voxel_size"]
        )
