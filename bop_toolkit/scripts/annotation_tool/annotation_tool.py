...
class AnnotationScene:
    ...
    def add_obj(
        self, obj_geometry, obj_name, obj_instance, transform=np.identity(4)
    ):
...
class AppWindow:
    ...
    def __init__(self, width, height, scenes):
        ...
        view_ctrls = gui.CollapsableVert(
            "View control", 0, gui.Margins(em, 0, 0, 0)
        )
        ...
        self._camera_follow_object.set_on_checked(
            self._set_change_cam_to_object
        )
        ...
        generate_save_annotation = gui.Button(
            "generate annotation - save/update"
        )
    ...
    def _update_scene_numbers(self):
        self._scene_number.text = (
            "Scene: " + f"{self._annotation_scene.scene_num:06}"
        )
        if p["tool_model"] == "individual":
            self._image_number.text = (
                "Image: " + f"{self._annotation_scene.image_num:06}"
            )
    ...
    def _transform(self, event):
        ...
        def move(x, y, z, rx, ry, rz):
            ...
            if x != 0 or y != 0 or z != 0:
                ...
                rot_mat_obj_center = (
                    active_obj.obj_geometry.get_rotation_matrix_from_xyz(
                        (rx, ry, rz)
                    )
                )
                T_neg = np.vstack(
                    (
                        np.hstack((np.identity(3), -center.reshape(3, 1))),
                        [0, 0, 0, 1],
                    )
                )
                R = np.vstack(
                    (
                        np.hstack((rot_mat_obj_center, [[0], [0], [0]])),
                        [0, 0, 0, 1],
                    )
                )
                T_pos = np.vstack(
    ...
    def _on_refine(self):
        ...
        if len(target.points) == 0:
            self._on_error(
                "Target point cloud is empty after cropping around the refinement area. No points to register."
            )
            return gui.Widget.EventCallbackResult.HANDLED
        ...
        if cuda_available:
            ...
            def cloud_gpu(cloud, color_available):
                ...
                if color_available:
                    cloud_t.point["colors"] = (
                        cloud_t.point["colors"].to(o3d.core.Dtype.Float32)
                        / 255.0
                    )
            ...
            icp_transform = (
                reg_transform.transformation.cpu().numpy()
            ...
            if color_available:
                print(
                    "Colored ICP is not implemented without CUDA, using PointToPoint ICP"
                )
            reg = o3d.pipelines.registration.registration_icp(
                source,
                target,
                p["max_correspondence_distance"],
                trans_init,
                ICP_method,
    ...
    def _on_generate(self):
        ...
        with open(json_6d_path, "w+") as gt_scene:
            ...
            for obj in self._annotation_scene.get_objects():
                ...
                obj_data = {
                    "cam_R_m2c": transform_cam_to_object[0:3, 0:3]
                    .flatten()
    ...
    def _on_highlight_obj(self, light):
        ...
        if light:
            self.settings.annotation_obj_material.base_color = [
                0.9,
                0.3,
                0.3,
                1.0,
    ...
    def _add_mesh(self):
        ...
        self._obj_meshes_colored = all(
            [self._obj_meshes_colored, obj_mesh_colored]
        )
        cam_trans = self._scene.scene.camera.get_model_matrix()
        infront_of_cam_trans = np.eye(4)
        infront_of_cam_trans[
            2, 3
        ] += -500  # drop object 50 mm in front of the camera
        ...
        new_mesh_name = (
            str(self._meshes_available.selected_value)
            + "_"
            + str(new_mesh_instance)
        )
        self._scene.scene.add_geometry(
            new_mesh_name,
            object_geometry,
    ...
    def _make_point_cloud(self, rgb_img, depth_img, cam_K):
        ...
        pcd.points = o3d.utility.Vector3dVector(
            np.asarray(pcd.points) * 1000
        )  # convert point cloud to mm
    ...
    def _load_object(self, obj_mesh_path):
        ...
        samples, face_index, colors = trimesh.sample.sample_surface(
            trimesh_mesh, 500000, sample_color=True
        )
        samples, face_index, colors = (
            samples.view(np.ndarray),
            face_index.view(np.ndarray),
            colors.view(np.ndarray),
        )
        ...
        if obj_mesh_colored:
            obj_geometry.colors = o3d.utility.Vector3dVector(
                colors[:, :3] / 255
            )
    ...
    def scene_load(self, scenes_path, scene_num, image_num):
        ...
        try:
            if p["tool_model"] == "individual":
                ...
                if os.path.exists(
                    os.path.join(scene_path, "rgb", f"{image_num:06}" + ".png")
                ):
                    rgb_path = os.path.join(
                        scene_path, "rgb", f"{image_num:06}" + ".png"
                    )
                elif os.path.exists(
                    os.path.join(scene_path, "rgb", f"{image_num:06}" + ".jpg")
                ):
                    rgb_path = os.path.join(
                        scene_path, "rgb", f"{image_num:06}" + ".jpg"
                    )
                rgb_img = cv2.imread(rgb_path)
                depth_path = os.path.join(
                    scene_path, "depth", f"{image_num:06}" + ".png"
                )
                depth_img = cv2.imread(depth_path, -1)
                depth_img = np.float32(
                    depth_img * depth_scale / 1000
                )  # convet depth to meter - convert it later to mm after making point cloud in open3d

                geometry = self._make_point_cloud(
                    rgb_img, depth_img, cam_K
                )  # point cloud in mm
            elif p["tool_model"] == "sequence":
                point_cloud_path = os.path.join(
                    scene_path, "assembled_cloud_world.pcd"
                )
        ...
        if not os.path.exists(scene_gt_path):
            ...
            with open(scene_gt_path) as scene_gt_file:
                ...
                if not str(image_num) in data:
                    ...
                    for (
                        obj
                    ) in scene_data:  # add object to annotation_scene object
                        # load object mesh
                        ...
                        obj_geometry, obj_mesh_colored = self._load_object(
                            obj_mesh_path
                        )
                        ...
                        obj_instance = self._obj_instance_count(
                            model_name, active_meshes
                        )
                        obj_name = model_name + "_" + str(obj_instance)
                        translation = np.array(
                            np.array(obj["cam_t_m2c"]), dtype=np.float64
                        )  # distance in mm
                        orientation = np.array(
                            np.array(obj["cam_R_m2c"]), dtype=np.float64
                        )
                    ...
                    self._obj_meshes_colored = all(
                        obj_meshes_colored
                    )  # this is used to determine which ICP to use (colored ir PointToPoint)
    ...
    def _on_next_scene(self):
        ...
        if not os.path.exists(
            os.path.join(
                self.scenes.scenes_path,
                f"{self._annotation_scene.scene_num + 1:06}",
            )
        ):
            self._on_error(
                "There is no next scene with number "
                + f"{self._annotation_scene.scene_num + 1:06}"
            )
            return
    ...
    def _on_previous_scene(self):
        ...
        if self._annotation_scene.scene_num - 1 < 1:
            self._on_error(
                "There is no scene number before scene "
                + f"{self._annotation_scene.scene_num:06}"
            )
            return
        self.scene_load(
...
def main():
    ...
    if os.path.exists(scenes.scenes_path) and os.path.exists(
        scenes.objects_path
    ):
        w.scene_load(
            scenes.scenes_path, p["start_scene_num"], p["start_image_num"]
        )
        w.update_obj_list()
    else:
