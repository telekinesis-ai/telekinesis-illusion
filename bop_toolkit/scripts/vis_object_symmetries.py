...
parser.add_argument(
    "--dataset",
    default="xyzibd",
    help="Name of the BOP dataset. See dataset_params.py for options.",
)
parser.add_argument(
    "--model_type",
    default=None,
    help="Type of the 3D models to use. None = default",
)
parser.add_argument(
    "--renderer_type",
    default="vispy",
    help="Type of the renderer. Options: 'vispy', 'cpp', 'python'",
)
parser.add_argument(
    "--max_sym_disc_step",
    type=float,
    default=0.01,
    help="See misc.get_symmetry_transformations",
)
parser.add_argument(
    "--datasets_path",
    default=config.datasets_path,
    help="Path to the folder containing the BOP datasets.",
)
parser.add_argument(
    "--vis_dir",
    default=os.path.join(config.output_path, "vis_object_symmetries"),
    help="Path to the folder for output visualisations.",
)
parser.add_argument(
    "--render_res",
    type=int,
    default=640,
    help="Resolution of the symmetry renders.",
)
parser.add_argument(
    "--fov_deg",
    type=int,
    default=60,
    help="FOV of the camera used for renders (degrees).",
)
parser.add_argument(
    "--max_portion",
    type=float,
    default=0.9,
    help="Porportion of the image height/width covered by the biggest object.",
)
...
dp_model = dataset_params.get_model_params(
    args.datasets_path, args.dataset, model_type
)
...
symmetry_poses = {
    obj_id: misc.get_symmetry_transformations(
        models_info[obj_id], args.max_sym_disc_step
    )
...
max_vertex_distance_from_origin = max(
    np.max(np.linalg.norm(obj_models[obj_id].vertices, axis=1))
    for obj_id in dp_model["obj_ids"]
)
diameter_pix = (
    args.max_portion * args.render_res
)  # portion of the image covered by the biggest object
...
for obj_id in tqdm(dp_model["obj_ids"], desc="Selecting best views"):
    ...
    for view_id, view in enumerate(views):
        rgb = ren.render_object(obj_id, view["R"], view["t"], fx, fy, cx, cy)[
            "rgb"
        ]
...
for obj_id in tqdm(dp_model["obj_ids"], desc="Rendering object symmetries"):
    ...
    for pose_id, pose_s in enumerate(symmetry_poses[obj_id]):
        ...
        vis_rgb_path = (
            ...
            / args.dataset
            / f"{obj_id:06d}"
            / f"{view_id:06d}_{pose_id:06d}.jpg"
        )
