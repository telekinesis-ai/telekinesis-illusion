...
try:
    ...
    logger.error(
        "torch is required for gpu based evaluation (see bop_toolkit/README.md)"
    )
    raise e
...
for result_filename in p["result_filenames"]:
    ...
    for target in targets:
        if p["eval_mode"] == "localization":
            assert "inst_count" in target, (
                "inst_count is required for localization mode"
            )
    ...
    ests = inout.load_bop_results(
        result_path, max_num_estimates_per_image=max_num_estimates_per_image
    )
    ...
    for scene_id, scene_targets in targets_org.items():
        ...
        for obj_id in est_per_object:
            ...
            for i in range(len(errors)):
                ...
                gt_visib_fract = est_per_object[obj_id]["gt_visib_fract"][
                    i
                ].item()
                ...
                scene_errs[key_name]["gt_visib_fracts"][gt_id] = [
                    gt_visib_fract
                ]
