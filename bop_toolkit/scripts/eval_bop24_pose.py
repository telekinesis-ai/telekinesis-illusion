...
parser.add_argument(
    "--targets_filename", type=str, default=p["targets_filename"]
)
...
for result_filename in result_filenames:
    ...
    result_name, _, dataset, _, _, _ = inout.parse_result_filename(
        result_filename
    )
    ...
    check_passed, check_msg, times, times_available = (
        inout.check_consistent_timings(ests, "im_id")
    )
    if not check_passed:
    ...
    for error in p["errors"]:
        ...
        calc_error_script, is_gpu_script_used = (
            misc.get_eval_calc_errors_script_name(
        ...
        if args.num_workers == 1:
            for calc_scores_cmd in calc_scores_cmds:
                ...
                if subprocess.call(calc_scores_cmd) != 0:
                    raise RuntimeError(
                        "Calculation of performance scores failed."
                    )
        else:
    ...
    logger.info(
        "Evaluation of {} took {}s.".format(result_filename, time_total)
    )
    ...
    final_scores_path = os.path.join(
        args.eval_path, result_name, "scores_bop24.json"
    )
