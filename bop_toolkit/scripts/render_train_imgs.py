...
out_rgb_tpath = os.path.join(
    "{out_path}", "{obj_id:06d}", "rgb", "{im_id:06d}.png"
)
out_depth_tpath = os.path.join(
    "{out_path}", "{obj_id:06d}", "depth", "{im_id:06d}.png"
)
out_scene_camera_tpath = os.path.join(
    "{out_path}", "{obj_id:06d}", "scene_camera.json"
)
...
for obj_id in obj_ids:
    ...
    for radius in radii:
        ...
        for view_id, view in enumerate(views):
            ...
            rgb = cv2.resize(
                rgb, dp_camera["im_size"], interpolation=cv2.INTER_AREA
            )
