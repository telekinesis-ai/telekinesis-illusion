...
dataset_save_dir = os.path.join(
    dp_split["base_path"], f"{p['dataset_split']}_cropped"
)
...
for scene_id in scene_ids:
    ...
    for im_counter, im_id in tqdm(enumerate(im_ids)):
        if im_counter % 10 == 0:
            misc.log(
                "scene: {}, im: {}/{}".format(scene_id, im_counter, len(im_ids))
            )

        # Load the color and depth images and prepare images for rendering.
        if (
            "rgb" in dp_split["im_modalities"]
            or p["dataset_split_type"] == "pbr"
        ):
            rgb_path = dp_split["rgb_tpath"].format(
                scene_id=scene_id, im_id=im_id
            )
        elif "gray" in dp_split["im_modalities"]:
            rgb_path = dp_split["gray_tpath"].format(
                scene_id=scene_id, im_id=im_id
            )
        else:
        ...
        for gt_id, gt in enumerate(scene_gt[im_id]):
            for mask_name in ["mask_tpath", "mask_visib_tpath"]:
                ...
                mask = mask[
                    delta_h : target_height + delta_h,
                    delta_w : target_width + delta_w,
                ]
