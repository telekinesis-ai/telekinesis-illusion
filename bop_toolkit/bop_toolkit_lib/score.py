...
def calc_localization_scores(scene_ids, obj_ids, matches, n_top, do_print=True):
    ...
    if do_print:
        ...
        misc.log(
            "Mean scene recall:  {:.4f}".format(scores["mean_scene_recall"])
        )
...
def calc_pose_detection_scores(
    ...
    for obj_id in obj_ids:
        ...
        for est in ests_info:
            ...
            if (
                est["obj_id"] == obj_id
                and est_signature not in est_signatures_to_ignore
            ):
        ...
        obj_ests_sorted = sorted(
            obj_ests, key=lambda x: x["score"], reverse=True
        )
        ...
        precisions = true_positive_cumsum / (
            true_positive_cumsum + false_positive_cumsum
        )
