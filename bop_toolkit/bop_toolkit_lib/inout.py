...
def load_json(path: Union[str, Path], keys_to_int=False):
    ...
    """
    path = Path(path)
    assert path.as_posix().endswith((".json", ".json.gz")), (
        f"{path} should end with .json or .json.gz extension"
    )
...
def save_json(
    path: Union[str, Path], content: dict, compress=False, verbose=False
):
...
def _camera_as_numpy(camera: dict):
    ...
    if "cam_K" in camera and "cam_model" in camera:
        raise ValueError(
            "Only one of 'cam_K', 'cam_model' field should be present in a scene camera configuration"
        )
    if "cam_K" in camera.keys():
        camera["cam_K"] = np.array(camera["cam_K"], np.float64).reshape((3, 3))
    if "cam_R_w2c" in camera.keys():
        camera["cam_R_w2c"] = np.array(camera["cam_R_w2c"], np.float64).reshape(
            (3, 3)
        )
    if "cam_t_w2c" in camera.keys():
        camera["cam_t_w2c"] = np.array(camera["cam_t_w2c"], np.float64).reshape(
            (3, 1)
        )
    if "cam_model" in camera:
        camera["cam_model"]["projection_params"] = np.array(
            camera["cam_model"]["projection_params"], np.float64
        )
    return camera
...
def _camera_as_json(camera: dict):
    ...
    if "cam_model" in camera:
        camera["cam_model"]["projection_params"] = (
            camera["cam_model"]["projection_params"].flatten().tolist()
        )
    return camera
...
def load_scene_gt(path: Union[str, Path]):
    ...
    for im_id, im_gt in scene_gt.items():
        for gt in im_gt:
            if "cam_R_m2c" in gt.keys():
                gt["cam_R_m2c"] = np.array(gt["cam_R_m2c"], np.float64).reshape(
                    (3, 3)
                )
            if "cam_t_m2c" in gt.keys():
                gt["cam_t_m2c"] = np.array(gt["cam_t_m2c"], np.float64).reshape(
                    (3, 1)
                )
    return scene_gt
...
def load_bop_results(
    path: Union[str, Path], version="bop19", max_num_estimates_per_image=None
):
    ...
    if max_num_estimates_per_image is not None:
        ...
        for im_signature in im_results.keys():
            ...
            num_ignored_estimates += max(
                0, len(im_results[im_signature]) - max_num_estimates_per_image
            )
            filtered_results.extend(
                im_results[im_signature][:max_num_estimates_per_image]
            )
...
def save_bop_results(
    path: Union[str, Path], results: List[dict], version="bop19"
):
...
def check_bop_results(path: Union[str, Path], version="bop19"):
    ...
    try:
        ...
        if version == "bop19":
            check_timings, check_msg_timings, times, times_available = (
                check_consistent_timings(results, "im_id")
            )
...
def check_coco_results(
    path: Union[str, Path],
    version="bop22",
    ann_type="segm",
    enforce_no_segm_if_bbox=False,
):
    ...
    return check_coco_results_(
        results, version, ann_type, enforce_no_segm_if_bbox
    )


def check_coco_results_(
    results: List,
    version="bop22",
    ann_type="segm",
    enforce_no_segm_if_bbox=False,
    ...
    if version == "bop22":
        try:
            for result in results:
                ...
                assert isinstance(result["scene_id"], int), (
                    "'scene_id' value should be of type 'int'"
                )
                assert isinstance(result["image_id"], int), (
                    "'image_id' value should be of type 'int'"
                )
                assert isinstance(result["category_id"], int), (
                    "'category_id' value should be of type 'int'"
                )
                assert isinstance(result["score"], (float, int)), (
                    "'score' value should be of type 'float' or 'int'"
                )
                if ann_type == "bbox":
                    assert "bbox" in result, (
                        "ann_type 'bbox' should contain 'bbox' key in all results"
                    )
                    assert isinstance(result["bbox"], list), (
                        "result['bbox'] should be a list"
                    )
                ...
                if ann_type == "segm":
                    assert "segmentation" in result, (
                        "ann_type 'segm' should contain 'segmentation' key in all results"
                    )
                    assert isinstance(result["segmentation"], dict), (
                        "Segmentation not in RLE format!"
                    )
                    assert "counts" in result["segmentation"], (
                        "Incorrect RLE format!"
                    )
                    assert "size" in result["segmentation"], (
                        "Incorrect RLE format!"
                    )
                if "time" in result:
        ...
        check_timings, check_msg_timings, times, times_available = (
            check_consistent_timings(results, "image_id")
        )
        if not check_timings:
            return False, check_msg_timings
...
def save_coco_results(
    path: Union[str, Path], results: List[dict], version="bop22", compress=False
):
...
def save_ply(path: Union[str, Path], model: dict, extra_header_comments=None):
    ...
    texture_file = (
        model["texture_file"] if "texture_file" in model.keys() else None
    )
...
def get_im_targets(
    im_gt: dict, im_gt_info: dict, visib_gt_min: float, eval_mode="localization"
):
    """
    ...
    for gt_id, gt in enumerate(im_gt):
        ...
        if (
            gt_info["visib_fract"] < visib_gt_min
            and eval_mode == "localization"
        ):
            continue
...
def create_coco_result_filename(
    ...
    :param: optional_id: id that may be attached to uniquely identify result file. Optional.
    """
    return _create_result_filename(
        method, dataset, split, "json", split_type, optional_id
    )
...
def create_pose_result_filename(
    ...
    """
    return _create_result_filename(
        method, dataset, split, "csv", split_type, optional_id
    )
