...
def decode_sample(
    ...
    if decode_gray:
        image_data["im_gray"] = inout.load_im(sample["gray.tiff"]).astype(
            np.uint8
        )
...
def load_image_data(
    ...
    if load_rgb:
        image_data["im_rgb"] = inout.load_im(_load("rgb" + rgb_suffix)).astype(
            np.uint8
        )

    if load_gray:
        image_data["im_gray"] = inout.load_im(_load("gray.tiff")).astype(
            np.uint8
        )
