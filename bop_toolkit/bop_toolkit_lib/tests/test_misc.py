...
class TestMisc(unittest.TestCase):
    ...
    def test_transform_pts_Rt(self):
        ...
        R_np = np.array(
            [transform.random_rotation_matrix()[:3, :3] for _ in range(self.B)]
        )
    ...
    def test_project_pts(self):
        ...
        """
        R_ref = np.array([0, -1, 0, 0, 0, -1, 1, 0, 0]).reshape((3, 3))
        t_ref = np.array([0, 0, 2])
        sa, st = 0.1, 0.1
        R_np = np.array(
            [
                R_ref
                @ transform.euler_matrix(*(sa * np.random.random(3)))[:3, :3]
                for _ in range(self.B)
            ]
        )
        t_np = np.array(
            [t_ref + st * np.random.random(3) for _ in range(self.B)]
        )
        ...
        camera = PinholePlaneCameraModel(
            width=fx, height=fy, f=(fx, fy), c=(cx, cy), distort_coeffs=()
        )
        proj_htt = np.zeros((self.B, self.Np, 2))
        for i in range(self.B):
            proj_htt[i] = pose_error_htt.project_pts_htt(
                self.pts, camera, R_np[i], t_np[i]
            )
    ...
    def test_precomputer(self):
        ...
        pre_Xs1_bis, pre_Ys1_bis = misc.Precomputer.precompute_lazy(
            depth_ima, Ka
        )
