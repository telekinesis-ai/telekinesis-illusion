...
for dataset_method_name, (sub_name, ann_type, compressed) in tqdm(
    ...
    log_file_path = (
        f"{LOGS_PATH}/eval_bop22_coco_test_{dataset_method_name}.txt"
    )
...
for sub_short_name, (sub_name, ann_type, compressed) in tqdm(
    FILE_DICTIONARY.items(), desc="Verifying..."
):
    if sub_short_name in EXPECTED_OUTPUT:
        ...
        for key, expected_score in EXPECTED_OUTPUT[sub_short_name].items():
            ...
            if eval_score is not None:
                ...
                misc.log(
                    f"Please check the log file {log_file_path} and {eval_file_path} for more details."
                )
