...
parser.add_argument(
    "--gt_from_datasets",
    default="",
    help='Comma separated list of dataset names, e.g. "ycbv,tless,lmo"',
    type=str,
)
...
p["gt_from_datasets"] = (
    args.gt_from_datasets.split(",") if len(args.gt_from_datasets) > 0 else []
)
...
FILE_DICTIONARY = {
    ...
    "lmo_gt": (
        "gt-pbrreal-rgb-mmodel_lmo-test_lmo.csv",
        "test_targets_bop19.json",
    ),
    "tless_megaPose": (
    ...
    "tless_gt": (
        "gt-pbrreal-rgb-mmodel_tless-test_tless.csv",
        "test_targets_bop19.json",
    ),
}
...
if len(p["gt_from_datasets"]) > 0:
    ...
    FILE_DICTIONARY = {
        f"{ds}_gt": (
            f"gt-results_{ds}-test_pose.csv",
...
for dataset_method_name, (
    ...
    if args.num_false_positives > 0:
        ...
        inout.save_bop_results(
            f"{RESULT_PATH}/{output_filename}", ests, version="bop19"
        )
...
for dataset_method_name, (file_name, test_targets_name) in tqdm(
    ...
    log_file_path = (
        f"{LOGS_PATH}/eval_bop24_pose_test_{dataset_method_name}.txt"
    )
    ...
    misc.log(
        f"Evaluation time for {dataset_method_name}: {end_time - start_time} seconds"
    )
...
if args.num_false_positives == 0:
    for dataset_name, _ in tqdm(FILE_DICTIONARY.items(), desc="Verifying..."):
        if dataset_name in EXPECTED_OUTPUT:
            log_file_path = (
                f"{LOGS_PATH}/eval_bop24_pose_test_{dataset_name}.txt"
            )
            ...
            for key, expected_value in EXPECTED_OUTPUT[dataset_name].items():
                ...
                if actual_value is not None:
                    ...
                    misc.log(
                        f"Please check the log file {log_file_path} for more details."
                    )
