...
class TestInout(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.json_coco_path = (
            results_dir
            / "zebraposesat-effnetb4_ycbv-test_5ed0eecc-96f8-498b-9438-d586d4d92528.json"
        )

    def test_parse_result_filename(self):
        # works with full path .csv
        result_name, method, dataset, split, split_type, ext = (
            inout.parse_result_filename(self.csv_6d_path)
        )
        self.assertEqual(
            result_name,
            "cnos-fastsammegapose_icbin-test_7c9f443f-b900-41bb-af01-09b8eddfc2c4",
        )
        ...
        result_name, method, dataset, split, split_type, ext = (
            inout.parse_result_filename(self.csv_6d_path.name)
        )
        self.assertEqual(
            result_name,
            "cnos-fastsammegapose_icbin-test_7c9f443f-b900-41bb-af01-09b8eddfc2c4",
        ...
        # .json too
        result_name, method, dataset, split, split_type, ext = (
            inout.parse_result_filename(self.json_coco_path)
        )
        self.assertEqual(
            result_name,
        ...
        # and .json.gz
        result_name, method, dataset, split, split_type, ext = (
            inout.parse_result_filename(self.json_coco_path.as_posix() + ".gz")
        )
        self.assertEqual(
            result_name,
            "zebraposesat-effnetb4_ycbv-test_5ed0eecc-96f8-498b-9438-d586d4d92528",
    ...
    def test_create_result_filename(self):
        ...
        result_filename = inout.create_coco_result_filename(
            method, dataset, split, split_type, optional_id
        )
        ...
        self.assertEqual(
            result_filename, "some-method_tless-val-primesense.json"
        )
        optional_id = "5ed0eecc-96f8-498b-9438-d586d4d92528"
        result_filename = inout.create_coco_result_filename(
            method, dataset, split, split_type, optional_id
        ...
        self.assertEqual(
            result_filename,
            "some-method_tless-val-primesense_5ed0eecc-96f8-498b-9438-d586d4d92528.json",
        )

        result_filename = inout.create_pose_result_filename(
            method, dataset, split, optional_id=optional_id
        )
        self.assertEqual(
            result_filename,
            "some-method_tless-val_5ed0eecc-96f8-498b-9438-d586d4d92528.csv",
        )
        result_filename = inout.create_pose_result_filename(
    ...
    def test_load_save_operations(self):
        ...
        # json
        json_coco_bis_path = self.json_coco_path.parent / (
            self.json_coco_path.stem + "_bis.json"
        )
        json_coco_gz_path = self.json_coco_path.parent / (
            self.json_coco_path.stem + ".json.gz"
        )
        ...
        self.assertTrue(
            len(coco_res) == len(coco_res_from_bis) == len(coco_res_from_gz)
        )
        self.assertTrue(
            coco_res[0].keys()
            == coco_res_from_bis[0].keys()
            == coco_res_from_gz[0].keys()
        )
        self.assertTrue(
            coco_res[0]["time"]
            == coco_res_from_bis[0]["time"]
            == coco_res_from_gz[0]["time"]
        )
        self.assertTrue(
            coco_res[0]["score"]
            == coco_res_from_bis[0]["score"]
            == coco_res_from_gz[0]["score"]
        )
        self.assertTrue(
            coco_res[0]["bbox"]
            == coco_res_from_bis[0]["bbox"]
            == coco_res_from_gz[0]["bbox"]
        )
    ...
    def test_check_coco(self):
        ...
        self.assertFalse(
            inout.check_coco_results_(
                3 * [result_bbox] + [result_bbox_bad_ts], ann_type="bbox"
            )[0]
        )
        self.assertTrue(
            inout.check_coco_results_(3 * [result_bbox], ann_type="bbox")[0]
        )
        self.assertTrue(
            inout.check_coco_results_(3 * [result_bbox_segm], ann_type="bbox")[
                0
            ]
        )
        self.assertTrue(
            inout.check_coco_results_(3 * [result_bbox_segm], ann_type="segm")[
                0
            ]
        )
        self.assertFalse(
        ...
        self.assertTrue(
            inout.check_coco_results_(3 * [result_segm], ann_type="segm")[0]
        )
        ...
        for k in ["bbox"] + common_keys:
            ...
            r_bbox[k] = "wrong_type"
            self.assertFalse(
                inout.check_coco_results_([r_bbox], ann_type="bbox")[0]
            )
            del r_bbox[k]
            self.assertFalse(
                inout.check_coco_results_([r_bbox], ann_type="bbox")[0]
        ...
        for k in ["segmentation"] + common_keys:
            ...
            r_segm[k] = "wrong_type"
            self.assertFalse(
                inout.check_coco_results_([r_segm], ann_type="segm")[0]
            )
            del r_segm[k]
            self.assertFalse(
                inout.check_coco_results_([r_segm], ann_type="segm")[0]
    ...
    def test_loading_and_checking_results(self):
        ...
        check_passed, check_msg, times, times_available = (
            inout.check_consistent_timings(bop_results, "im_id")
        )
        self.assertTrue(check_passed)
        ...
        check_passed, check_msg, times, times_available = (
            inout.check_consistent_timings(coco_results, "image_id")
        )
        self.assertTrue(check_passed)
        self.assertTrue(times_available)
