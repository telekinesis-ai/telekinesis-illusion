...
def generate_random_pose_est(R, t, B):
    R_np = np.array(
        [R @ (transform.random_rotation_matrix()[:3, :3]) for _ in range(B)]
    )
...
class TestPoseErrors(unittest.TestCase):
    def setUp(self) -> None:
        ...
        models_info = load_json(
            model_params["models_info_path"], keys_to_int=True
        )
        ...
        scene_dir = Path(split_params["split_path"]) / "{scene_id:06d}".format(
            scene_id=scene_id
        )
        scene_gt = load_scene_gt(
            split_params["scene_gt_tpath"].format(scene_id=scene_id)
        )
        ...
        # get camera model
        scene_camera = load_scene_camera(scene_dir / "scene_camera.json")[
            scene_id
        ]
        if "cam_K" in scene_camera:
            self.K = scene_camera["cam_K"]
            self.K_ts = (
                torch.Tensor(self.K).unsqueeze(0).repeat(self.B, 1, 1).float()
            )

            fx, fy = self.K[0, 0], self.K[1, 1]
            cx, cy = self.K[0, 2], self.K[1, 2]
            self.camera = PinholePlaneCameraModel(
    ...
    def test_mssd(self):
        ...
        R_np, t_np, R_ts, t_ts = generate_random_pose_est(
            self.R_gt, self.t_gt, self.B
        )

        err_np = np.zeros(self.B)
        for i in range(self.B):
            err_np[i] = pose_error.mssd(
                R_np[i], t_np[i], self.R_gt, self.t_gt, self.pts, self.syms
            )
        err_ts = pose_error_gpu.mssd_by_batch(
            R_ts, t_ts, self.R_gt_ts, self.t_gt_ts, self.pts_ts, self.syms_ts
        )
    ...
    def test_mspd(self):
        ...
        err_ts = pose_error_gpu.mspd_by_batch(
            R_ts,
            t_ts,
            self.R_gt_ts,
            self.t_gt_ts,
