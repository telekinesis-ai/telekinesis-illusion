...
def combine_depth_diffs(
    ...
    if use_clip:
        clip_val = (
            np.percentile(combined, 99.7) if clip_val is None else clip_val
        )
...
def draw_pose_contour(
    ...
    if use_depth:
        ...
        depth2[mask == 0] = 0
        assert depth2.max() > 1e1, (
            f"Ensure the depth has mm-scale, {depth2.max=}"
        )
    ...
    if mesh_bbox is not None:
        ...
        if pose_gt is not None:
            ...
            final_frame = draw_posed_3d_box(
                ...
                K=K,
                bbox=mesh_bbox,
                line_color=bbox_color_gt,
            )
    if extra_text is not None:
        if isinstance(extra_text, list):
            extra_text = "\n".join(extra_text)
        final_frame = draw_text_in_ul(
            final_frame, extra_text, size=1, thickness=3
        )
    return final_frame
...
def draw_xyz_axis(
    ...
    zz[:3] = zz[:3] * scale
    origin = (
        misc.project_pts(np.array([[0, 0, 1.0]]), K, R, t).squeeze().astype(int)
    )
...
def calc_mask_visib_percent(
    mask_visib: np.ndarray, valid_mask: np.ndarray
) -> float:
...
def plot_depth(
    ...
    if include_colorbar:
        ...
        if use_horiz_cbar:
            ...
            cbar.set_label(
                cbar_title, rotation=90, labelpad=15, fontsize=fontsize
            )
