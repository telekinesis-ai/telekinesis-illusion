...
def write_text_on_image(
    im, txt_list, loc=(3, 0), color=(1.0, 1.0, 1.0), size=20
):
    ...
    for info in txt_list:
        ...
        draw.text(
            loc, txt, fill=tuple([int(c * 255) for c in color]), font=font
        )
...
def vis_object_poses(
    ...
    if (
        vis_depth_diff or (vis_rgb and vis_rgb_resolve_visib)
    ) and depth is None:
        raise ValueError(
    ...
    if vis_depth_diff:
        ...
        depth_info = [
            ...
            },
            {
                "name": "median diff",
                "fmt": ":.3f",
                "val": np.median(np.abs(depth_diff_valid)),
            },
            {
                "name": "25 percentile",
                "fmt": ":.3f",
                "val": np.percentile(np.abs(depth_diff_valid), 25),
            },
        ]
...
def render_poses(poses: List[dict], renderer, K) -> List[dict]:
    ...
    renderings = []
    for pose in poses:
        if htt_available and isinstance(
            K, CameraModel
        ):  # hand_tracking_toolkit is used for rendering.
            ren_out = renderer.render_object(
                pose["obj_id"], pose["R"], pose["t"], K
            )
        elif isinstance(K, np.ndarray) and K.shape == (
            3,
            3,
        ):  # pinhole camera model is used for rendering.
            ...
            raise ValueError(
                "Camera model 'K' type {} should be either CameraModel or np.ndarray".format(
                    type(K)
                )
            )
