...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--worker_id", type=int, help="worker id", required=True
    )
    parser.add_argument(
    ...
    for file in all_files:
        vsd_args = POSE_ERROR_VSD_ARGS.from_file(
            os.path.join(worker_folder, file)
        )
        err = pose_error.vsd(
            vsd_args.R_e,
            vsd_args.t_e,
    ...
    misc.log(
        f"Worker {args.worker_id} finished in {time.time() - start_time} s"
    )
