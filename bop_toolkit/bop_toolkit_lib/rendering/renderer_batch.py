...
class BatchRenderer:
    ...
    def _init_renderers(self, num_workers):
        ...
        for worker_id in range(num_workers):
            worker_folder = os.path.join(
                self.tmp_dir, f"vsd_worker_{worker_id}"
            )
    ...
    def run_vsd(self, all_im_errs):
        ...
        out_counter = 0
        for worker_id in range(num_workers_used):
            worker_results = inout.load_json(
                self.tmp_dir + f"/worker_{worker_id}.json"
            )
            for key, value in worker_results.items():
                idx_im, idx_err, gt_id = key.split("_")
                all_im_errs[int(idx_im)][int(idx_err)]["errors"][int(gt_id)] = (
                    value
                )
                out_counter += 1
        assert out_counter == in_counter, (
            "Number of input and output files mismatch"
        )
