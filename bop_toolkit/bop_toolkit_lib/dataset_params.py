...
def get_camera_params(datasets_path, dataset_name, cam_type=None):
    ...
    # hot3d does not have a single camera file, raise an exception
        ...
        raise ValueError(
            "BOP dataset {} does not have a global camera file.".format(
                dataset_name
            )
        )
...
def get_split_params(datasets_path, dataset_name, split, split_type=None):
    ...
    # HOT3D.
        ...
        sensor_modalities_have_separate_annotations = {
            "aria": True,
            "quest3": True,
        }
        p["im_modalities"] = {
            "aria": ["rgb", "gray1", "gray2"],
            "quest3": ["gray1", "gray2"],
        }
        ...
        p["scene_ids"] = {
            "test": p["test_quest3_scene_ids"]
            + p["test_aria_scene_ids"],  # test_quest3 + test_aria
            "train": p["train_quest3_scene_ids"]
            + p["train_aria_scene_ids"],  # train_quest3 + train_aria
        }[split]
        ...
        def hot3d_eval_modality(scene_id):
            if (
                scene_id in p["test_quest3_scene_ids"]
                or scene_id in p["train_quest3_scene_ids"]
            ):
                return p["quest3_eval_modality"]
            elif (
                scene_id in p["test_aria_scene_ids"]
                or scene_id in p["train_aria_scene_ids"]
            ):
                return p["aria_eval_modality"]
            else:
                raise ValueError(
                    "scene_id {} not part of hot3d valid scenes".format(
                        scene_id
                    )
                )
        ...
        p["scene_ids"] = {
            ...
            "val": val_scene_ids,
            "train": [
                i for i in list(range(51)) if i not in missing_pbr_scene_ids
            ],
        }[split]
            ...
            for modality in modalities:
                ...
                if dataset_name == "hot3d":
                    p[f"{modality}_{sensor}_tpath"] = join(
                        ...
                        "{scene_id:06d}",
                        f"{modality}",
                        "{im_id:06d}" + exts[sensor][modality],
                    )
                else:
                    p[f"{modality}_{sensor}_tpath"] = join(
                        split_path,
                ...
                p.update(
                    {
                        # Path template to a file with per-image camera parameters.
                        "scene_camera_{}_{}_tpath".format(
                            modality, sensor
                        ): join(
                            split_path,
                            "{scene_id:06d}",
                            "scene_camera_{}.json".format(gt_file_suffix),
                        ),
                        # Path template to a file with GT annotations.
                        "scene_gt_{}_{}_tpath".format(modality, sensor): join(
                            split_path,
                            "{scene_id:06d}",
                            "scene_gt_{}.json".format(gt_file_suffix),
                        ),
                        # Path template to a file with meta information about the GT annotations.
                        "scene_gt_info_{}_{}_tpath".format(
                            modality, sensor
                        ): join(
                            split_path,
                            "{scene_id:06d}",
                            "scene_gt_info_{}.json".format(gt_file_suffix),
                        ),
                        # Path template to a file with the coco GT annotations.
                        "scene_gt_coco_{}_{}_tpath".format(
                            modality, sensor
                        ): join(
                            split_path,
                            "{scene_id:06d}",
                            "scene_gt_coco_{}.json".format(gt_file_suffix),
                        ),
                        # Path template to a mask of the full object silhouette.
...
def get_scene_sensor_or_modality(
    sm: Union[None, str, Callable], scene_id: Union[None, int]
) -> Union[None, str]:
    """
    ...
    if sm is None or isinstance(sm, str):
        ...
        raise TypeError(
            f"Sensor or modality {sm} should be either None, str or callable, not {type(sm)}"
        )
...
def scene_tpaths_keys(
    ...
    tpath_keys_dic["depth_tpath"] = (
        tpath_keys_dic["rgb_tpath"]
        .replace("rgb", "depth")
        .replace("gray", "depth")
    )
