...
class Precomputer(object):
    ...
    def precompute_lazy(depth_im, K):
        ...
        if depth_im.shape != Precomputer.depth_im_shape or not np.all(
            ...
            Precomputer.K = K
            Precomputer.pre_Xs = (Precomputer.xs - K[0, 2]) / np.float64(
                K[0, 0]
            )
            Precomputer.pre_Ys = (Precomputer.ys - K[1, 2]) / np.float64(
                K[1, 1]
            )
...
def add_argument_bool(
    parser: argparse.ArgumentParser, arg_name: str, default: bool
):
    parser.add_argument(f"--{arg_name}", action="store_true")
    parser.add_argument(
        f"--no_{arg_name}", dest=f"{arg_name}", action="store_false"
    )
