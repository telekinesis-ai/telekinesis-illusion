...
def create_annotation_info(
    ...
    if mask_encoding_format == "rle":
        ...
        raise RuntimeError(
            "Unknown encoding format: {}".format(mask_encoding_format)
        )
...
def merge_coco_results(
    existing_coco_results, new_coco_results, image_id_offset
):
...
def binary_mask_to_rle(binary_mask):
    ...
    if len(mask) > 0:
        ...
        changes_indx = np.where(
            np.concatenate(([True], mask_changes, [True]), 0)
        )[0]
...
def compute_ious(gt, dt, iou_type):
    ...
    if iou_type == "segm":
        ...
        unions = np.sum(
            (np.expand_dims(dt_bin, 1) + np.expand_dims(gt_bin, 0)) > 0,
            axis=(2, 3),
        )
...
def ensure_rle_binary(dataset_coco_results, cocoGt):
    ...
    for ann in dataset_coco_results:
        if "bbox" not in ann:
            ...
            ann["segmentation"] = annotation_to_binary_rle(
                ann, t["height"], t["width"]
            )
