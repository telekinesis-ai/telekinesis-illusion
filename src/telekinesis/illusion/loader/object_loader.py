    ...
    **kwargs,
    ...
    if not os.path.exists(filepath):
        raise FileNotFoundError(
            f"The given filepath does not exist: {filepath}"
        )
    ...
    if filepath.endswith(".obj"):
        ...
        if PLY_TEXTURE_FILE_COMMENT in ply_file_content:
            ...
            material = create_material_from_texture(
                texture_file_path,
                material_name=f"ply_{model_name}_texture_model",
            )
            ...
            tmp_ply_file = os.path.join(
                Utility.get_temporary_directory(), model_name
            )
    ...
    elif filepath.lower().endswith(".glb") or filepath.lower().endswith(
        ".gltf"
    ):
        ...
        raise ValueError(
            f"Unsupported file extension for '{filepath}'. Supported "
            "extensions are: .obj, .ply, .dae, .stl, .fbx, .glb, .gltf, "
            ".usd, .usda, .usdc."
        )
