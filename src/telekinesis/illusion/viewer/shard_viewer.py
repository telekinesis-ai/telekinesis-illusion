...
# Private helper from the stdlib `turtle` module, reused here to auto-forward
# any tk.Canvas method not explicitly defined on ImagePanel to self._canvas.
...
parser.add_argument(
    ...
    type=str,
    metavar="PATH",
    help="path to dataset directory containing an images folder and coco_annotations.json",
)

from pycocotools import mask as mask_utils


def draw_masks(draw_layer: Image.Image, objects, obj_categories, ignore, alpha):
    """
    Draws instance masks using pycocotools decoding (handles polygon + RLE properly).
    We draw into an RGBA layer using Image.paste with a binary mask.
    """
    w, h = draw_layer.size
    ...
    for i, (c, obj) in enumerate(zip(obj_categories, objects)):
        ...
        fill_rgba = tuple(list(c[-1]) + [alpha])

        # Decode segmentation into a binary numpy mask (H,W) in {0,1}
        try:
            if isinstance(segm, list):
                ...
                # rle dict (compressed counts string or uncompressed)
                rle = segm
                if isinstance(rle, dict) and isinstance(rle.get("counts"), list):
                    rle = mask_utils.frPyObjects(rle, h, w)

            m = mask_utils.decode(rle)
            # decode may return (H,W,1) for some inputs
            if m.ndim == 3:
                m = m[:, :, 0]
            m = (m > 0).astype(np.uint8) * 255
        except Exception:
            # If something is malformed, skip instead of killing the UI
            continue

        mask_img = Image.fromarray(m, mode="L")
        # Paste a solid color through the mask onto the RGBA draw_layer
        draw_layer.paste(fill_rgba, (0, 0), mask_img)


class Data:
    """Handles data related stuff."""

    def __init__(self, dataset_dir: str, annotations_file: str):
    ...
    def prepare_image(self, object_based_coloring: bool = False):
        ...
        objects = [
            obj
            for obj in self.instances["annotations"]
...
def draw_bboxes(
    draw, objects, labels, obj_categories, ignore, width, label_size
):
...
class ImagePanel(ttk.Frame):
    ...
    def __init__(
        self, parent, width=768, height=480, canvwidth=600, canvheight=500
    ):
...
class Controller:
    def __init__(
        ...
        self.statusbar.description.configure(
            textvariable=self.description_status
        )
        ...
        self.coloring_on_global = (
            tk.BooleanVar()
        )  # Toggles objects/categories coloring
    ...
    def compose_image(
        ...
        # Draw masks
        if masks_on:
            draw_masks(draw_layer, objects, names_colors, ignore, alpha)
        # Draw bounding boxes
        if bboxes_on:
            draw_bboxes(
                draw,
    ...
    def update_img(self, local=True, width=None, alpha=None, label_size=None):
        ...
        bboxes_on = (
            self.bboxes_on_local if local else self.bboxes_on_global.get()
        )
        labels_on = (
            self.labels_on_local if local else self.labels_on_global.get()
        )
        masks_on = self.masks_on_local if local else self.masks_on_global.get()
        coloring = (
            self.coloring_on_local if local else self.coloring_on_global.get()
        )
        ...
        self.nobjects_status.set(
            f"objects: {len(self.current_img_obj_categories)}"
        )
        self.ncategories_status.set(
            f"categories: {len(self.current_img_categories)}"
        )
    ...
    def toggle_all(self, event=None):
        ...
        # What to toggle
        var_list = [
            self.bboxes_on_local,
            self.labels_on_local,
            self.masks_on_local,
        ]
...
SHARD_ANNOTATIONS = "coco_annotations.json"
MERGED_ANNOTATIONS = "merged_coco_annotations.json"
SPLIT_ANNOTATIONS = "_annotations.coco.json"


class DatasetLayout(NamedTuple):
    annotations_filename: str
    images_subdir: str | None
    layout_name: str  # "shard" | "merged" | "split"


def detect_dataset_layout(dataset_dir: Path) -> DatasetLayout:
    """Inspect a directory and decide whether it holds a shard, merged, or split COCO dataset.

    Precedence: merged > shard > split. Raises FileNotFoundError if none match.
    """
    if (dataset_dir / MERGED_ANNOTATIONS).is_file():
        return DatasetLayout(MERGED_ANNOTATIONS, None, "merged")
    if (dataset_dir / SHARD_ANNOTATIONS).is_file() and (
        dataset_dir / "images"
    ).is_dir():
        return DatasetLayout(SHARD_ANNOTATIONS, "images", "shard")
    if (dataset_dir / SPLIT_ANNOTATIONS).is_file():
        return DatasetLayout(SPLIT_ANNOTATIONS, None, "split")
    raise FileNotFoundError(
        f"No recognizable COCO dataset in {dataset_dir}.\n"
        f"Looked for: {MERGED_ANNOTATIONS} (merged), "
        f"{SHARD_ANNOTATIONS} + images/ (shard), or {SPLIT_ANNOTATIONS} (split)."
    )
...
def view_coco(
    ...
    *,
    images_subdir: str | None = "images",
    annotations_filename: str = "coco_annotations.json",
    title: str = "COCO Viewer",
):
    """
    Launch the COCO viewer UI for a dataset folder.

    Parameters
    ----------
    dataset_dir:
        Path to dataset directory containing the images folder and coco annotations json.
    images_subdir:
        Name of the images subdirectory inside dataset_dir (default: "images").
        Set to ``None`` to skip the images-directory validation — useful when
        image paths in the annotation file are already relative to *dataset_dir*
        (e.g. after merging multiple shards, or for split datasets with flat
        images alongside ``_annotations.coco.json``).
    annotations_filename:
        Name of the annotation json file inside dataset_dir (default: "coco_annotations.json").
        Use "merged_coco_annotations.json" for merged datasets and
        "_annotations.coco.json" for splits — or call :func:`detect_dataset_layout`
        to pick the right values automatically.
    title:
        Window title.
    """
    dataset_dir = Path(dataset_dir).expanduser().resolve()
    annotations_file = dataset_dir / annotations_filename
    ...
    if not dataset_dir.exists() or not dataset_dir.is_dir():
        ...
        messagebox.showerror(
            "Error!", f"Dataset directory does not exist:\n{dataset_dir}"
        )
        root.destroy()
        raise FileNotFoundError(
    ...
    if images_subdir is not None:
        ...
        if not images_dir.exists() or not images_dir.is_dir():
            root.geometry("360x170")
            messagebox.showerror(
                "Error!", f"Images directory not found:\n{images_dir}"
            )
            root.destroy()
            raise FileNotFoundError(f"Images directory not found: {images_dir}")

    if not annotations_file.exists() or not annotations_file.is_file():
        root.geometry("360x170")
        messagebox.showerror(
            "Error!", f"Annotation file not found:\n{annotations_file}"
        )
        root.destroy()
        raise FileNotFoundError(
    ...
    data = Data(str(dataset_dir), str(annotations_file))
...
def main():
    ...
    if not args.dataset:
        ...
        messagebox.showwarning(
            "Warning!", "Nothing to show.\nPlease specify a dataset directory!"
        )
        root.destroy()
        return
    ...
    view_coco(
        ...
        annotations_filename=layout.annotations_filename,
        title=f"COCO Viewer - {layout.layout_name}: {dataset_dir.name}",
    )
