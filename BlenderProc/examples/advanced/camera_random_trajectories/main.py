...
parser.add_argument(
    "output_dir",
    nargs="?",
    default="examples/advanced/camera_random_trajectories/output",
...
camera_shaking_rot_angle = bproc.sampler.random_walk(
    total_length=25,
    dims=1,
    step_magnitude=np.pi / 32,
    window_size=5,
    interval=[-np.pi / 6, np.pi / 6],
    distribution="uniform",
    order=2,
)
camera_shaking_rot_axis = bproc.sampler.random_walk(
    total_length=25, dims=3, window_size=10, distribution="normal"
)
...
for i in range(25):
    ...
    location_cam = np.array(
        [10 * np.cos(i / 25 * np.pi), 10 * np.sin(i / 25 * np.pi), 8]
    )
