...
parser.add_argument(
    "ikea_path",
    nargs="?",
    default="resources/ikea",
    help="Path to the downloaded IKEA dataset, see the /scripts for the download script",
)
parser.add_argument(
    "cc_material_path",
    nargs="?",
...
parser.add_argument(
    "output_dir",
    nargs="?",
    default="examples/advanced/random_room_constructor/output",
...
for i in range(15):
    interior_objects.extend(
        bproc.loader.load_ikea(
            args.ikea_path, ["bed", "chair", "desk", "bookshelf"]
        )
    )
...
while tries < 10000 and poses < 5:
    ...
    if (
        bproc.camera.perform_obstacle_in_view_check(
            cam2world_matrix, {"min": 1.2}, bvh_tree
        )
