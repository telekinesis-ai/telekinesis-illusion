...
parser.add_argument(
    "scene",
    help="Path to the scene.obj file, should be examples/resources/scene.obj",
)
parser.add_argument(
    "config_file", help="Path to the camera calibration config file."
)
parser.add_argument(
    "output_dir",
...
if file_extension == ".blend":
    ...
    raise Exception(
        "The extension of the object file is neither .obj nor .blend ."
    )
...
orig_res_x, orig_res_y, mapping_coords = (
    bproc.camera.set_camera_parameters_from_config_file(
        args.config_file, read_the_extrinsics=True
    )
)
...
if "img1" in os.path.basename(args.config_file):
    ...
    norm_corr_limit = (
        0.660  # low since the real background is large and different
    )
...
if result.argmax() == 24:  # center of the (7,7) correlation window
    ...
    if result.max() > norm_corr_limit:
        print(
            f"The norm. correlation index between generated and real images is {np.round(result.max(), 3)}, "
            f"which is fine."
        )
    else:
        raise Exception(
            "The norm. correlation index between generated and real image is too low. The images do "
            "not match. Choose other object or config file."
        )
else:
    raise Exception(
        "The generated calibration pattern image and the reference real image do not match. Choose other "
