...
parser.add_argument(
    "camera",
    help="Path to the camera file which describes one camera pose per line, here the output of scn2cam from the SUNCGToolbox can be used",
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=int,
    default=2560,
    help="Number of points for random pattern. Not always exact due to rounding errors.",
)
parser.add_argument(
    "output_dir",
    nargs="?",
...
K = np.array([[650.018, 0, 637.962], [0, 650.018, 355.984], [0, 0, 1]])
...
pattern_img = bproc.utility.generate_random_pattern_img(
    WIDTH, HEIGHT, args.points
)
...
with open(args.camera, "r") as f:
    for line in f.readlines():
        ...
        position = bproc.math.change_coordinate_frame_of_point(
            line[:3], ["X", "-Z", "Y"]
        )
        rotation = bproc.math.change_coordinate_frame_of_point(
            line[3:6], ["X", "-Z", "Y"]
        )
        matrix_world = bproc.math.build_transformation_mat(
            position, bproc.camera.rotation_from_forward_vec(rotation)
        )
...
data["stereo-depth"], data["disparity"] = (
    bproc.postprocessing.stereo_global_matching(
        data["colors"], disparity_filter=False
    )
)
