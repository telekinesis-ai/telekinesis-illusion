...
parser.add_argument(
    "urdf_file",
    nargs="?",
    default="examples/resources/medical_robot/miro.urdf",
    help="Path to the .urdf file",
)
parser.add_argument(
    "output_dir",
    nargs="?",
    default="examples/advanced/urdf_loading_and_manipulation/output",
...
robot.set_rotation_euler_fk(
    link=None, rotation_euler=0.2, mode="relative", frame=0
)
robot.set_rotation_euler_fk(
    link=None, rotation_euler=0.2, mode="relative", frame=1
)
robot.set_rotation_euler_fk(
    link=None, rotation_euler=0.2, mode="relative", frame=2
)
...
robot.create_ik_bone_controller(
    link=robot.links[-1], relative_location=[0.0, 0.0, 0.2]
)
robot.set_location_ik(location=[0.0, 0.0, 0.8], frame=4)
robot.set_rotation_euler_ik(
    rotation_euler=[-1.57, 1.57, 0.0], mode="absolute", frame=4
)
