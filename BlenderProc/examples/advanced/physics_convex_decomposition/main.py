...
parser.add_argument(
    "bin_object",
    help="Path to the object file containing the bin, should be examples/advanced/physics_convex_decomposition/bin.obj.",
)
parser.add_argument(
    "shapenet_path",
    help="Path to the downloaded shape net core v2 dataset, get it [here](http://www.shapenet.org/)",
)
parser.add_argument(
    "output_dir",
    nargs="?",
    default="examples/advanced/physics_convex_decomposition/output",
...
parser.add_argument(
    ...
    help="The directory in which vhacd should be installed or is already installed.",
)
...
bproc.object.sample_poses(shapenet_objs, sample_pose_func=sample_pose)
...
bproc.camera.add_camera_pose(
    bproc.math.build_transformation_mat([0, -2.13, 3.22], [0.64, 0, 0])
)
