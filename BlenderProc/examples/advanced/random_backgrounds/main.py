...
parser.add_argument(
    "scene",
    nargs="?",
    default="examples/advanced/random_backgrounds/object.ply",
    help="Path to the object file.",
)
parser.add_argument(
    "output_dir",
    nargs="?",
    default="examples/advanced/random_backgrounds/output",
...
mat.set_principled_shader_value(
    "Base Color", np.random.uniform([0, 0, 0, 1], [1, 1, 1, 1])
)
...
while tries < 10000 and poses < 5:
    ...
    lookat_point = obj.get_location() + np.random.uniform(
        [-0.5, -0.5, -0.5], [0.5, 0.5, 0.5]
    )
    rotation_matrix = bproc.camera.rotation_from_forward_vec(
        lookat_point - location, inplane_rot=np.random.uniform(-0.7854, 0.7854)
    )
