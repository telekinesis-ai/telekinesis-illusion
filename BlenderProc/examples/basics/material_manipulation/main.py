...
parser.add_argument(
    "scene",
    nargs="?",
    default="examples/basics/material_manipulation/scene.obj",
...
parser.add_argument(
    "output_dir",
    nargs="?",
    default="examples/basics/material_manipulation/output",
...
bproc.camera.add_camera_pose(
    bproc.math.build_transformation_mat(
        [1.9488, -6.5202, 0.23291], [1.84, 0, 0.5]
    )
)
...
ground_material.set_displacement_from_principled_shader_value(
    "Base Color", multiply_factor=1.5
)
...
images = list(
    Path(args.image_dir)
    .absolute()
    .rglob("material_manipulation_sample_texture*.jpg")
)
