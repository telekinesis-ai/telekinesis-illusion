...
def generate_collapsible_classlist(
    app, fromdocname, classes, container, caption, module_index, label_prefix
):
    ...
    if module_index is not None:
        ...
        generate_classlist(
            app, fromdocname, toc, classes, label_prefix, level=1
        )
    container += toc
...
def generate_tutorials_sidebar(app, fromdocname, container):
    tutorials_dir = (
        Path(__file__).absolute().parent.parent / "docs" / "tutorials"
    )
    ...
    for tutorial in tutorials:
        ...
        ref = nodes.reference("", "")
        ref["refuri"] = app.builder.get_relative_uri(
            fromdocname, "docs/tutorials/" + tutorial[1]
        )
        ref.append(nodes.Text(tutorial[0]))
        module_item = nodes.list_item(
            "", addnodes.compact_paragraph("", "", ref), classes=["toctree-l1"]
        )
...
def generate_examples_sidebar(app, fromdocname, container):
    ...
    for example_groups in [
        examples / group for group in ["basics", "advanced", "datasets"]
    ]:
        if example_groups.is_dir():
            ...
            ref = nodes.reference("", "")
            ref["refuri"] = app.builder.get_relative_uri(
                fromdocname, "examples/" + example_groups.name + "/README"
            )
            ...
            for example in sorted(
                example_groups.rglob("*/README.md"), key=lambda x: x.parent.name
            ):
                ref = nodes.reference("", "")
                ref["refuri"] = app.builder.get_relative_uri(
                    fromdocname,
                    str(example)
                    .replace(str(examples), "examples")
                    .replace("README.md", "README"),
                )
...
def generate_sidebar(app, fromdocname):
    ...
    classes_per_group = {
        "Entities (bproc.types)": ([], None, ""),
        "api": ([], None, "bproc."),
        "internal": ([], 2, "bproc.python."),
    }  # , "modules (deprecated)": ([], 3, "")}#"modules": ([], 1), "provider": ([], 2),
    for e in classes:
        if (
            e[2] == "module"
            and e[3].startswith("blenderproc.api.")
            or e[2] == "class"
            and not e[3].startswith("blenderproc.api.")
        ):
    ...
    for key, items in classes_per_group.items():
        generate_collapsible_classlist(
            app,
            fromdocname,
            items[0],
            container,
...
def html_page_context(app, pagename, templatename, context, doctree):
    def make_toctree(
        collapse=True, maxdepth=-1, includehidden=True, titles_only=False
    ):
        if (
            "page_source_suffix" in context
