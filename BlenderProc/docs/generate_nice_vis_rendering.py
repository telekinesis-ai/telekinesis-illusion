...
sys.path.append(
    os.path.join(os.path.dirname(__file__), "../blenderproc/scripts")
)
from saveAsImg import convert_hdf

if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Combines up to four images from .png or .hdf5. It requires a Color rendering.  Normal, distance and Semantic Segmentation are optional."
    )
    parser.add_argument(
        "-f",
        "--file_path",
        nargs="*",
        help="File path to the list of color.png or .hdf5 file.",
        required=True,
    )
    parser.add_argument(
        "-o",
        "--output",
        help="Folder path where the resulting image/s should be saved.",
        type=str,
    )
    parser.add_argument(
        "-b",
        "--border",
        help="Adds a border around the images in white.",
        type=int,
        default=0,
    )
    ...
    def convert_png_to_multiples(image_path, org_path):
        if "colors" in image_path:
            ...
            if os.path.exists(distance_path) and os.path.exists(depth_path):
                raise Exception(
                    "This can only work with one of the two, either distance or depth!"
                )
            ...
            if used_imgs:
                ...
                if len(used_imgs) == 1:
                    ...
                    final_img = np.ones(
                        (
                            img_size[0] * 2 + border * 3,
                            img_size[1] * 2 + border * 3,
                            img_size[2],
                        )
                    )

                final_img[
                    border : img_size[0] + border,
                    border : img_size[1] + border,
                    :,
                ] = used_imgs[0]

                if len(used_imgs) == 2:
                    final_img[
                        2 * border + img_size[0] : -border,
                        border : border + img_size[1],
                        :,
                    ] = used_imgs[1]
                if len(used_imgs) == 3:
                    ...
                    final_img[
                        border : img_size[0] + border,
                        2 * border + img_size[1] : -border,
                        :,
                    ] = used_imgs[1]
                    final_img[
                        2 * border + img_size[0] : -border,
                        start_val : start_val + img_size[1],
                        :,
                    ] = used_imgs[2]
                if len(used_imgs) == 4:
                    final_img[
            ...
            if ".png" in org_path:
                ...
                if final_img.shape[2] == 3:
                    resulting_file_name = org_path.replace(
                        ".hdf5", "_rendering.jpg"
                    )
            ...
            if args.output:
                resulting_file_name = os.path.join(
                    args.output, resulting_file_name
                )
            ...
            raise Exception(
                "The file path must point to the colors image: {}".format(
                    image_path
                )
            )
        ...
        if os.path.exists(file_path):
            if file_path.endswith(".png"):
                ...
                color_img_path = os.path.join(
                    tmp_path,
                    os.path.basename(file_path).split(".")[0] + "_colors.png",
                )
                ...
                raise Exception(
                    "This file format is not supported: {}".format(file_path)
                )
        else:
            raise Exception(
                "The given file does not exist: {}".format(args.file_path)
            )
