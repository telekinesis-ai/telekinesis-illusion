...
class UnitTestCheckUtility(unittest.TestCase):
    def test_convert_to_entity_subclass(self):
        ...
        bproc.clean_up(True)
        cube = convert_to_entity_subclass(
            bproc.object.create_primitive("CUBE").blender_obj
        )
        empty = convert_to_entity_subclass(
            bproc.object.create_empty("empty_test").blender_obj
        )
    ...
    def test_scene_graph_hierarchy_changes(self):
        ...
        self.assertTrue(
            (grandchild.get_local2world_mat()[:3, 3] == [4, 0, 0]).all()
        )
    ...
    def test_duplicate_linked(self):
        ...
        self.assertEqual(
            light.blender_obj.data, duplicate_light.blender_obj.data
        )
    ...
    def test_duplicate_hierarchy(self):
        ...
        # world location
        self.assertTrue(
            (duplicate_root.get_local2world_mat()[:3, 3] == [0, 0, 0]).all()
        )
        self.assertTrue(
            (duplicate_child.get_local2world_mat()[:3, 3] == [1, 1, 1]).all()
        )
        self.assertTrue(
            (
                duplicate_grandchild.get_local2world_mat()[:3, 3] == [2, 2, 2]
            ).all()
        )
        ...
        self.assertTrue(
            (duplicate_grandchild.get_location() == [1, 1, 1]).all()
        )
