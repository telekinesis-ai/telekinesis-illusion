...
class UnitTestCheckCamera(unittest.TestCase):
    def test_camera_add_camera_pose(self):
        ...
        cam2world_matrix = np.array(
            [
                [-0.5285266, -0.8057487, 0.26726118, 1.0],
        ...
        for x, y in zip(
            np.reshape(cam2world_matrix, -1).tolist(),
            np.reshape(cam2world_matrix_calc, -1).tolist(),
        ):
    ...
    def test_camera_rotation_from_forward_vec(self):
        ...
        correct_roation_matrix = np.array(
            [
                [-8.94427180e-01, -3.58568549e-01, 2.67261177e-01],
        ...
        calc_rotation_matrix = bproc.camera.rotation_from_forward_vec(
            poi - location, inplane_rot=rotation
        )

        for x, y in zip(
            np.reshape(correct_roation_matrix, -1).tolist(),
            np.reshape(calc_rotation_matrix, -1).tolist(),
        ):
