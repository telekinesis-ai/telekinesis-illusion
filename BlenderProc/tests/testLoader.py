...
class UnitTestCheckLoader(unittest.TestCase):
    ...
    def test_create_material_from_texture(self):
        ...
        def perform_material_checks(
            used_material: bproc.types.Material, used_path: Path
        ):
            # check if the material has a texture loaded
            found_nodes = used_material.get_nodes_with_type(
                "ShaderNodeTexImage"
            )
        ...
        material = bproc.material.create_material_from_texture(
            texture_path, material_name="new_mat"
        )
