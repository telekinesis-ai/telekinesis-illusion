...
def cli():
    ...
    parser.add_argument(
        "download_mp_script",
        help="Path to the download script, this script is only available after "
        "accepting the Terms of Use.",
    )
    parser.add_argument(
        "scans_txt_path",
        help="Path to the scans.txt data, this file is only available after "
        "accepting the Terms of Use.",
    )
    parser.add_argument(
    ...
    if not scan_file.exists():
        raise FileNotFoundError(
            "The scans.txt file could not be found check argument."
        )

    with scan_file.open("r", encoding="utf-8") as file:
        current_ids = [
            id_val for id_val in file.read().split("\n") if id_val.strip()
        ]

    download_mp_file = Path(args.download_mp_script).absolute()
    if not download_mp_file.exists():
        raise FileNotFoundError(
            f"The download_mp script could not be found: {download_mp_file}"
        )
