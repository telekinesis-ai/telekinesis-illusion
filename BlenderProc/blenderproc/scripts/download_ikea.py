...
def split_object_according_to_groups(file_path, folder):
    ...
    with open(file_path, "r", encoding="utf-8") as file:
        ...
        list_of_split_ids = [
            i for i, line in enumerate(lines) if line.strip().startswith("g ")
        ]
        ...
        for current_i in list_of_split_ids[1:]:
            ...
            face_lines = np.array(
                [
                    [[int(e) for e in eles.split("/")] for eles in l]
                    for l in face_lines
                ]
            )
            ...
            for line in current_lines:
                if line.strip().startswith("f "):
                    ...
                    values = [
                        np.array([int(e) for e in eles.split("/")])
                        - face_offset
                        for eles in blocks
                    ]
                    f_line = "f " + " ".join(
                        [
                            "/".join([str(int(e)) for e in eles])
                            for eles in values
                        ]
                    )
            ...
            if amount_of_faces > 10:
                current_text += "\n".join(final_lins)
                file_path_obj = os.path.join(
                    folder, f"{os.path.basename(folder)}_{group_counter}.obj"
                )
...
def cli():
    ...
    parser.add_argument(
        "output_dir", help="Determines where the data is going to be saved."
    )
        ...
        if no_jitter_folders:
            for no_jitter_folder in no_jitter_folders:
                ...
                folders_in_no_jitter = [
                    f
                    for f in glob.glob(os.path.join(no_jitter_folder, "*"))
                    if os.path.isdir(f)
                ]
    ...
    delete_obj_file(path)
    path = os.path.join(
        ikea_dir, "IKEA_table_JOKKMOKK", "jokkmokk_table_2_obj0_object.obj"
    )
    delete_obj_file(path)
    path = os.path.join(
        ikea_dir,
    ...
    split_object_according_to_groups(
        first_path, os.path.join(ikea_dir, "IKEA_sofa_VRETA")
    )
