...
def download_blendkit_assets(asset_types: List[str], output_dir: str):
    ...
    for asset_type in asset_types:
        ...
        while True:
            ...
            try:
                url_path = (
                    f"https://www.blenderkit.com/api/v1/search/?query=asset_type:{asset_type}+order:_score+"
            ...
            for file in asset["files"]:
                if file["fileType"] == "blend":
                    if download_url is not None:
                        print(
                            f"Warning: asset {asset['id']} has more than one blend file in downloads."
                        )
                    download_url = file["downloadUrl"]

            if download_url is None:
                print(
                    f"Warning: asset {asset['id']} has no blend file in downloads."
                )
                continue

            # Download metadata for blend file
            with urllib.request.urlopen(
                download_url + "?scene_uuid=" + scene_uuid
            ) as url:
                data = json.loads(url.read().decode())
...
def cli():
    ...
    parser = argparse.ArgumentParser(
        "Downloads materials and models from blenderkit"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--asset_types",
        nargs="+",
        help="Which type of assets to download",
        default=["material", "model"],
        choices=["material", "model"],
    )
