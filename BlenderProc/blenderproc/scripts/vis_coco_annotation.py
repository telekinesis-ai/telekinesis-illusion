...
def cli():
    ...
    parser.add_argument(
        ...
        dest="image_index",
        default=0,
        help="image over which to annotate, uses the rgb rendering",
        type=int,
    )
    parser.add_argument(
        "-b",
        ...
        help="path to folder with coco_annotation.json and images",
        type=str,
    )
    parser.add_argument(
        "--save",
    ...
    def get_category(_id):
        category = [
            category["name"] for category in categories if category["id"] == _id
        ]
        if category:
            return str(category[0])
        raise RuntimeError(
            f"Category {_id} is not defined in {os.path.join(base_path, conf)}"
        )
    ...
    for annotation in annotations:
        if annotation["image_id"] == image_idx:
            ...
            draw.text(
                (bb[0] + 2, bb[1] + 2),
                get_category(annotation["category_id"]),
                font=font,
            )
            if isinstance(annotation["segmentation"], dict):
                ...
                draw_ov.bitmap(
                    (0, 0),
                    item,
                    fill=(rand_color[0], rand_color[1], rand_color[2], 128),
                )
                ...
                for item in annotation["segmentation"]:
                    ...
                    pdraw.polygon(
                        item,
                        fill=(rand_color[0], rand_color[1], rand_color[2], 127),
                        outline=(255, 255, 255, 255),
                    )
                    im.paste(poly, mask=poly)
    if save:
        im.save(
            os.path.join(base_path, f"coco_annotated_{image_idx}.png"), "PNG"
        )
    im.show()
