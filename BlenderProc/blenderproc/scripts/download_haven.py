...
def cli():
    ...
    parser.add_argument(
        "output_folder", help="Determines where the data is going to be saved."
    )
    parser.add_argument(
        "--resolution",
        help="Desired resolution for the hdr images. Be aware that bigger resolutions, "
        "take a lot of disc space.",
        default="2k",
    )
    parser.add_argument(
        "--format",
        help="Desired download format for the images.",
        default="jpg",
    )
    parser.add_argument(
        "--tags", nargs="+", help="Filter by asset tag.", default=None
    )
    parser.add_argument(
        "--categories", nargs="+", help="Filter by asset category.", default=[]
    )
    parser.add_argument(
        "--threads",
        nargs="?",
    ...
    parser.add_argument(
        "--types",
        nargs="+",
        help="Only download the given types",
        default=None,
        choices=["textures", "hdris", "models"],
    )
    ...
    def download_items(
        item_type: str,
        output_dir: Path,
        item_download_func: Callable[[str, Path], None],
    ):
        # Filter for type
        ...
        response = requests.get(
            f"https://api.polyhaven.com/assets?t={item_type}"
        ...
        if args.tags:
            data = {
                ...
                if any(tag_list in args.tags for tag_list in value.get("tags"))
            }
        ...
        missing_item_ids = list(
            filter(item_file_exists, map(lambda item_id: item_id, data.keys()))
        )

        # Skip download if no items are missing
        if not missing_item_ids:
            print(
                "Skipping download of\t" + output_dir.name + " All files exist"
            )
            return
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for item_id in missing_item_ids:
                ...
                futures.append(
                    executor.submit(item_download_func, item_id, item_output)
                )
    ...
    def download_texture(item_id: str, output_dir: Path):
        request = requests.get(
            f"https://api.polyhaven.com/files/{item_id}", timeout=30
        )
        data = request.json()
        ...
        for key in data.keys():
            ...
            if key in [
                ...
                if args.resolution not in data[key]:
                    print(
                        f"Skipping {key} texture {item_id} as the desired resolution is not available."
                    )
                    continue
                ...
                if args.format not in data[key][args.resolution]:
                    print(
                        f"Skipping {key} texture {item_id} as the desired format is not available."
                    )
                    continue
                ...
                download_file(
                    download_url, output_dir / download_url.split("/")[-1]
                )
    ...
    def download_hdri(item_id: str, output_dir: Path):
        ...
        if args.resolution not in data["hdri"]:
            print(
                f"Skipping hdri {item_id} as the desired resolution is not available."
            )
            return
    ...
    def download_model(item_id: str, output_dir: Path):
        ...
        if args.resolution not in data["blend"]:
            print(
                f"Skipping model {item_id} as the desired resolution is not available."
            )
            return
        ...
        download_file(
            blend_data["url"], output_dir / blend_data["url"].split("/")[-1]
        )
