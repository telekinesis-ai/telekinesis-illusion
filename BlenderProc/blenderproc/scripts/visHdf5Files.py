...
def vis_data(
    ...
    if key_matches(key, flow_keys):
        try:
            ...
            raise ImportError(
                "Using .hdf5 containers, which contain flow images needs opencv-python to be "
                "installed!"
            ) from e
            ...
            if full_hdf5_data is not None and segcolormap_key in full_hdf5_data:
                ...
                if len(segcolormap) > 0:
                    ...
                    for colormap_key, colormap_value in segcolormap[0].items():
                        if (
                            colormap_key.startswith("channel_")
                            and colormap_value.isdigit()
                        ):
            ...
            with h5py.File(path, "r") as data:
                ...
                if keys_to_visualize is not None:
                    keys = [
                        ...
                        if key_matches(key, keys_to_visualize)
                    ]
                else:
                    keys = list(data.keys())
                ...
                for key in keys:
                    ...
                    if save_to_path is not None:
                        save_to_file = os.path.join(
                            save_to_path,
                            str(os.path.basename(path)).split(".", maxsplit=1)[
                                0
                            ]
                            + f"_{key}.png",
                        )
...
def cli():
    ...
    parser.add_argument(
        "--keys",
        nargs="+",
        help="Keys that should be visualized. If none is given, "
        "all keys are visualized.",
    ...
    parser.add_argument(
        "--segcolormap_keys",
        nargs="+",
        help="Keys that point to the segmentation color maps "
    ...
    parser.add_argument(
        "--depth_keys",
        nargs="+",
        help="Keys that contain additional non-RGB data which should be "
        "visualized using a jet color map.",
        default=default_depth_keys,
    )
    parser.add_argument("--depth_max", type=float, default=default_depth_max)
    parser.add_argument(
        "--save", default=None, type=str, help="Saves visualizations to file."
    )
