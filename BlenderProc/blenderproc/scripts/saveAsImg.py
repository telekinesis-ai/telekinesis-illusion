...
def convert_hdf(base_file_path: str, output_folder: Optional[str] = None):
    ...
    if os.path.exists(base_file_path):
        if os.path.isfile(base_file_path):
            base_name = str(os.path.basename(base_file_path)).split(
                ".", maxsplit=1
            )[0]
            ...
            with h5py.File(base_file_path, "r") as data:
                ...
                for key, val in data.items():
                    val = np.array(val)
                    if (
                        np.issubdtype(val.dtype, np.string_)
                        or len(val.shape) == 1
                    ):
                        pass  # metadata
                    else:
                        ...
                        if val.shape[0] != 2:
                            ...
                            for image_index, image_value in enumerate(val):
                                file_path = (
                                    f"{base_name}_{key}_{image_index}.png"
                                )
...
def cli():
    ...
    parser = argparse.ArgumentParser(
        "Script to save images out of a hdf5 files."
    )
