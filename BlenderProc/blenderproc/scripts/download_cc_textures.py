...
def cli():
        ...
        if "foundAssets" in json_data and len(json_data["foundAssets"]) > 0:
            for asset in json_data["foundAssets"]:
                if (
                    ...
                    and "downloadFiletypeCategories"
                    in asset["downloadFolders"]["default"]
                ):
                    ...
                    if (
                        ...
                        for download_attr in current_download_dict["zip"][
                            "downloads"
                        ]:
                            if (
                                ...
                                data[asset["assetId"]] = (
                                    download_attr["downloadLink"],
                                    download_attr["zipContent"],
                                )
                    else:
                        print(
                            f"No zip or downloads found for asset: {asset['assetId']}"
                        )
                else:
                    print(
    ...
    for index, (asset, content) in enumerate(data.items()):
        ...
        if not current_folder.exists():
            ...
            files_in_asset_folder = [
                file_path.name for file_path in current_folder.iterdir()
            ]
            delete_folder = False
            ...
            if delete_folder:
                print(
                    f"Redownload the asset: {asset}, not all files are present after download"
                )
