"""
This script extracts all id labels used in 3D Front, as they change every few months, this can be used to regenerate
them. Results will be saved in 3D_front_mapping_generated.csv
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "This detects all labels which are currently used in the 3D Front dataset."
    )
    parser.add_argument(
        "front_folder",
        help="Folder path to the 3D FRONT dataset. The folder should contain "
        "the json files.",
    )
    parser.add_argument(
        "future_folder", help="Folder path to the 3D FUTURE model dataset."
    )
    ...
    print(
        f"There are {len(not_used_models)} models which have been defined but not found on disc!"
    )
    ...
    with open(
        os.path.join(
            os.path.dirname(__file__), "3D_front_mapping_generated.csv"
        ),
        "w",
        encoding="utf-8",
    ) as file:
