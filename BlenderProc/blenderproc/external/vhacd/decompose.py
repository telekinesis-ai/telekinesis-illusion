...
def convex_decomposition(
    ...
    name_template: str = "?_hull_#",
    remove_doubles: bool = True,
    apply_modifiers: bool = True,
    apply_transforms: str = "NONE",
    depth: int = 20,
    ...
    if platform not in ["linux", "linux2"]:
        raise RuntimeError(
            f"Convex decomposition is at the moment only available on linux: {platform}"
        )
    ...
    if not os.path.exists(os.path.join(vhacd_path, "v-hacd")):
        ...
        if "NO_OPENCL" in os.environ and os.environ["NO_OPENCL"] == "1":
            os.system(
                os.path.join(os.path.dirname(__file__), "build_linux.sh")
                + " "
                + os.path.join(vhacd_path, "v-hacd")
                + " -DNO_OPENCL=ON"
            )
        else:
            os.system(
    ...
    if apply_modifiers:
        mesh = obj.blender_obj.evaluated_get(
    ...
    # Apply transforms
    translation, quaternion, scale = Matrix(
    ...
    if cache_dir is None or not os.path.exists(
        os.path.join(cache_dir, str(mesh_hash) + ".obj")
    ):
        vhacd_binary = os.path.join(vhacd_path, "v-hacd", "app", "TestVHACD")
        if not os.path.exists(vhacd_binary):
            raise FileNotFoundError(
                "The vhacd binary was not found, the build script probably failed!"
            )
        ...
        with Popen(
            cmd_line, bufsize=-1, close_fds=True, shell=True, cwd=temp_dir
        ) as vhacd_process:
            vhacd_process.wait()
        ...
        if not os.path.exists(out_file_name):
            raise RuntimeError(
                f"No output produced by convex decomposition of object {obj.get_name()}"
            )
