...
def cli():
    ...
    parser = argparse.ArgumentParser(
        description="BlenderProc: A procedural Blender pipeline for "
        "photorealistic image generation.",
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument(
        "-v", "--version", action="store_true", help="Version of BlenderProc"
    )
    subparsers = parser.add_subparsers(
        dest="mode", help="Select a BlenderProc command to run:"
    )

    # Setup different modes
    parser_run = subparsers.add_parser(
        "run", help="Runs the BlenderProc pipeline in normal mode."
    )
    ...
    parser_debug = subparsers.add_parser(
        "debug",
        help="Runs the BlenderProc pipeline in debug mode. This will open "
        "the Blender UI, so the 3D scene created by the pipeline "
        "can be visually inspected.",
    )
    parser_vis = subparsers.add_parser(
        "vis",
        help=f"Visualize the content of BlenderProc output files. \n"
        f"Options: {', '.join(options['vis'])}",
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser_download = subparsers.add_parser(
        "download",
        help="Download datasets, materials or 3D models to run "
    ...
    parser_extract = subparsers.add_parser(
        "extract",
        help="Extract the raw images from generated containers such "
        f"as hdf5. \nOptions: {', '.join(options['extract'])}",
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser_pip = subparsers.add_parser(
        "pip",
        help="Can be used to install/uninstall pip packages in the Blender "
    ...
    parser_pip.add_argument(
        ...
        choices=options["pip"],
        help="\n".join(
            f"{key}: {value}" for key, value in options["pip"].items()
        ),
    )
    parser_pip.add_argument(
        "pip_packages",
        metavar="pip_packages",
        nargs="*",
        help="A list of pip packages that should be installed/uninstalled. "
        "Packages versions can be determined via the `==` notation.",
    )
    parser_pip.add_argument(
        "--not-use-custom-package-path",
        dest="not_use_custom_package_path",
        action="store_true",
    ...
    for subparser in [parser_run, parser_debug, parser_quickstart]:
        if subparser != parser_quickstart:
            subparser.add_argument(
                "file",
                help="The path to a python file which uses BlenderProc via the API.",
            )

        subparser.add_argument(
            "--reinstall-blender",
            dest="reinstall_blender",
            action="store_true",
    ...
    if args.version:
        ...
        if is_using_external_bpy_module():
            if args.mode == "run":
                ...
                print(
                    "USE_EXTERNAL_BPY_MODULE is set, debug mode is not supported."
                )
            elif args.mode == "quickstart":
                path_src_run = os.path.join(
                    repo_root_directory,
                    "blenderproc",
                    "scripts",
                    "quickstart.py",
                ...
                print(
                    f"USE_EXTERNAL_BPY_MODULE is set, quickstart is not supported, instead run:\n\n"
        ...
        if args.mode == "quickstart":
            path_src_run = os.path.join(
                repo_root_directory, "blenderproc", "scripts", "quickstart.py"
            )
            args.file = path_src_run
            print(
                f"'blenderproc quickstart' is an alias for 'blenderproc run {path_src_run}'"
            )
        else:
        ...
        # Setup env vars
        used_environment = dict(
            os.environ, PYTHONPATH=repo_root_directory, PYTHONNOUSERSITE="1"
        )
        # this is done to enable the import of blenderproc inside the blender internal python environment
        used_environment[
            "INSIDE_OF_THE_INTERNAL_BLENDER_PYTHON_ENVIRONMENT"
        ] = "1"

        # Install blender, if not already done
        custom_blender_path, blender_install_path = (
            InstallUtility.determine_blender_install_path(args)
        )
        blender_run_path, major_version = (
            InstallUtility.make_sure_blender_is_installed(
                custom_blender_path,
        ...
        if args.force_pip_update:
            SetupUtility.clean_installed_packages_cache(
                os.path.dirname(blender_run_path), major_version
            )
        ...
        if args.mode == "debug":
            ...
            p = subprocess.Popen(
                [
                    ...
                    "--python",
                    os.path.join(
                        repo_root_directory, "blenderproc/debug_startup.py"
                    ),
                    "--",
                    path_src_run,
                    temp_dir,
    ...
    # Import the required entry point
        ...
        if args.mode == "vis" and args.vis_mode == "hdf5":
            ...
            from blenderproc.scripts.vis_coco_annotation import (
                cli as current_cli,
            )
            ...
            from blenderproc.scripts.download_blenderkit import (
                cli as current_cli,
            )
        elif args.mode == "download" and args.download_mode == "cc_textures":
            from blenderproc.scripts.download_cc_textures import (
                cli as current_cli,
            )
            ...
            from blenderproc.scripts.download_matterport3d import (
                cli as current_cli,
            )
        else:
            raise RuntimeError(
                f"There is no linked script for the command: {args.mode}. "
        ...
        if args.pip_mode == "install":
            SetupUtility.setup_pip(
                user_required_packages=args.pip_packages,
            ...
            SetupUtility.uninstall_pip_packages(
                args.pip_packages,
                blender_path=blender_path,
                major_version=major_version,
            )
