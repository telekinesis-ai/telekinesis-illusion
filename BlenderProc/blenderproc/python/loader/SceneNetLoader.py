...
def load_scenenet(
    ...
    unknown_texture_folder: Optional[str] = None,
) -> List[MeshObject]:
    ...
    _SceneNetLoader.random_sample_materials_for_each_obj(
        loaded_objects, texture_folder, unknown_texture_folder
    )
...
class _SceneNetLoader:
    @staticmethod
    def random_sample_materials_for_each_obj(
        loaded_objects: List[MeshObject],
        texture_folder: str,
            ...
            for material in obj.get_materials():
                ...
                if not texture_nodes or len(texture_nodes) == 1:
                    ...
                    image_paths = glob.glob(
                        os.path.join(texture_folder, mat_name, "*")
                    )
                    if not image_paths:
                        if not os.path.exists(unknown_texture_folder):
                            raise FileNotFoundError(
                                f"The unknown texture folder does not exist: "
                                f"{unknown_texture_folder}, check if it was set correctly "
                                f"via the config."
                            )
                        image_paths = glob.glob(
                            os.path.join(unknown_texture_folder, "*")
                        )
                        if not image_paths:
                            raise FileNotFoundError(
                                f"The unknown texture folder did not contain any "
                    ...
                    if os.path.exists(image_path):
                        ...
                        raise FileNotFoundError(
                            f"No image was found for this entity: {obj.get_name()}, "
        ...
        for obj in loaded_objects:
            ...
            if (
                "wall" in obj_name
                or "floor" in obj_name
                or "ceiling" in obj_name
            ):
    ...
    def set_category_ids(
        loaded_objects: List[MeshObject], label_mapping: LabelIdMapping
    ):
        """
        ...
        for obj in loaded_objects:
            ...
            # Check whether the object's name without suffixes like 's', '1' or '2' exist in the mapping.
                ...
                obj.set_cp(
                    "category_id", label_mapping.id_from_label(obj_name[:-1])
                )
            elif "painting" in obj_name:
                obj.set_cp(
                    "category_id", label_mapping.id_from_label("picture")
                )
            else:
                print(
                    f"This object was not specified: {obj_name} use objects for it."
                )
                obj.set_cp(
                    "category_id",
