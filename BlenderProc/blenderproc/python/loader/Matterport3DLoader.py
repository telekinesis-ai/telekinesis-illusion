...
def load_matterport3d(
    data_folder: Union[Path, str],
    used_house_id: Optional[str] = None,
    compare_floor_angle_in_degrees: float = 15.0,
    ...
    all_object_files = [
        object_file
        for object_file in all_object_files
        if "aayBHfsNo7d" not in str(object_file)
    ]
    ...
    if used_house_id is not None:
        loaded_house_file = [
            object_file
            for object_file in all_object_files
            if used_house_id in str(object_file)
        ]
        if not loaded_house_file:
            raise ValueError(
                f"The used house id: {used_house_id} does not appear in the downloaded .obj files!"
            )
        if len(loaded_house_file) == 1:
            loaded_house_file = loaded_house_file[0]
        else:
            raise ValueError(
                f"The used house id: {used_house_id} does appear more than once in the downloaded "
                f".obj files!"
            )
    else:
    ...
    if len(loaded_obj) == 1:
        ...
        raise RuntimeError(
            f"At this point only one object should be loaded, not more or less: {len(loaded_obj)}"
        )
    ...
    for material in obj.get_materials():
        ...
        material.link(
            textures.outputs["Color"], background_color_node.inputs["Color"]
        )
    ...
    floor_obj = slice_faces_with_normals(
        obj, compare_angle_degrees=compare_floor_angle_in_degrees
    )
