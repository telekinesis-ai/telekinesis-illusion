...
class RockEssentialsRockLoader:
    ...
    def load_rocks(
        ...
        with bpy.data.libraries.load(path) as (data_from, _):
            ...
            if not objects:
                ...
                for obj_type in obj_types:
                    obj_list += [
                        obj for obj in data_from.objects if obj_type in obj
                    ]
            else:
        ...
        for i in range(amount):
            ...
            bpy.ops.wm.append(
                filepath=os.path.join(path, "Object", obj),
                filename=obj,
    ...
    @staticmethod
    def set_rocks_properties(
        objects: List[MeshObject],
        physics: bool = False,
        render_levels: int = 3,
        high_detail_mode: bool = False,
        scale: Optional[Union[Vector, np.ndarray, list]] = None,
        reflection_amount: Optional[float] = None,
        reflection_roughness: Optional[float] = None,
        ...
        for obj in objects:
            ...
            if obj.has_cp("01) High Detail Mode"):
                ...
                print(
                    "High Detail Mode is unavailable for "
                    + str(obj.get_name())
                    + ", omitting."
                )
