...
def load_replica_segmented_mesh(
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "This function needs the plyfile lib, install it via:"
    ...
    if not current_folder.exists():
        raise Exception(
            f'The dataset folder: "{current_folder}" does not exist!'
        )
    ...
    if not ply_segmented_path.exists():
        raise Exception(
            f'Could not find "{ply_segmented_path}", the path was created automatically.'
        )
    ...
    colors = np.concatenate(
        [
            vertex_data[:, 6:].astype(float) / 255.0,
            np.ones((vertex_data.shape[0], 1)),
        ],
        axis=-1,
    )
    ...
    for current_class_id in used_class_ids:
        ...
        mesh.vertices.foreach_set(
            "co", vertices[current_face_indices].reshape(-1)
        )
        mesh.vertices.foreach_set(
            "normal", normals[current_face_indices].reshape(-1)
        )
        ...
        color_layer.data.foreach_set(
            "color", colors[current_face_indices].reshape(-1)
        )
...
def load_replica(
    data_path: str, data_set_name: str, use_smooth_shading: bool = False
) -> List[MeshObject]:
