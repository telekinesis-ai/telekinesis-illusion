...
def load_AMASS(
    ...
    supported_mocap_datasets = _AMASSLoader.get_supported_mocap_datasets(
        taxonomy_file_path, data_path
    )

    # selected_obj = self._files_with_fitting_ids
    pose_body, betas = _AMASSLoader.get_pose_parameters(
    ...
    body_model, faces = _AMASSLoader.load_parametric_body_model(
    ...
    generated_obj = _AMASSLoader.write_body_mesh_to_obj_file(
...
class _AMASSLoader:
    ...
    def get_pose_parameters(
        supported_mocap_datasets: dict,
        num_betas: int,
        used_sub_dataset_id: str,
        used_subject_id: str,
        used_sequence_id: int,
        used_frame_id: int,
        ...
        if used_sub_dataset_id in supported_mocap_datasets:
            ...
            if not used_subject_id:
                ...
                possible_subject_ids = glob.glob(
                    os.path.join(sub_dataset_path, "*")
                )
                possible_subject_ids.sort()
                if len(possible_subject_ids) > 0:
                    used_subject_id_str = os.path.basename(
                        random.choice(possible_subject_ids)
                    )
                else:
                    raise FileNotFoundError(
                        f"No subjects found in folder: {sub_dataset_path}"
                    )
            else:
            ...
            if used_sequence_id < 0:
                ...
                possible_sequence_ids = glob.glob(
                    os.path.join(sub_dataset_path, used_subject_id_str, "*")
                )
                possible_sequence_ids.sort()
                if len(possible_sequence_ids) > 0:
                    used_sequence_id = os.path.basename(
                        random.choice(possible_sequence_ids)
                    )
                    used_sequence_id = used_sequence_id[
                        used_sequence_id.find("_") + 1 : used_sequence_id.rfind(
                            "_"
                        )
                    ]
                else:
                    raise FileNotFoundError(
                        f"No sequences found in folder: "
                        f"{os.path.join(sub_dataset_path, used_subject_id_str)}"
                    )
            subject_path = os.path.join(sub_dataset_path, used_subject_id_str)
            used_subject_id_str_reduced = (
                used_subject_id_str[: used_subject_id_str.find("_")]
                if "_" in used_subject_id_str
                else used_subject_id_str
            )
            ...
            if os.path.exists(sequence_path):
                ...
                if frame_id in range(0, no_of_frames_per_sequence):
                    ...
                    pose_body = torch.Tensor(
                        sequence_body_data["poses"][
                            frame_id : frame_id + 1, 3:66
                        ]
                    ).to(comp_device)
                    # parameters that control the body shape
                    betas = torch.Tensor(
                        sequence_body_data["betas"][:num_betas][np.newaxis]
                    ).to(comp_device)
                    return pose_body, betas
                raise RuntimeError(
                    f"Requested frame id is beyond sequence range, for the selected sequence, choose "
                    f"frame id within the following range: [0, {no_of_frames_per_sequence}]"
                )
            raise RuntimeError(
                f"Invalid sequence/subject: {used_subject_id} category identifiers, please choose a "
                f"valid one. Used path: {sequence_path}"
            )
        raise RuntimeError(
            f"The requested mocap dataset is not yest supported, please choose another one "
            f"from the following supported datasets: {list(supported_mocap_datasets.keys())}"
        )

    @staticmethod
    def load_parametric_body_model(
        data_path: str,
        used_body_model_gender: str,
        num_betas: int,
        num_dmpls: int,
    ) -> Tuple["BodyModel", np.array]:
        ...
        if not os.path.exists(bm_path) or not os.path.exists(dmpl_path):
            raise FileNotFoundError(
                "Parametric Body model doesn't exist, please follow download "
    ...
    @staticmethod
    def get_supported_mocap_datasets(
        taxonomy_file_path: str, data_path: str
    ) -> dict:
            ...
            with open(taxonomy_file_path, "r", encoding="utf-8") as f:
                ...
                for block in loaded_data:
                    if "sub_data_id" in block:
                        ...
                        supported_mocap_datasets[sub_dataset_id] = os.path.join(
                            data_path, block["path"]
                        )
        else:
            raise FileNotFoundError(
                f"The taxonomy file could not be found: {taxonomy_file_path}"
            )

        return supported_mocap_datasets

    @staticmethod
    def write_body_mesh_to_obj_file(
        body_representation: "torch.Tensor", faces: np.array, temp_dir: str
    ) -> str:
        ...
        with open(outmesh_path, "w", encoding="utf-8") as fp:
            fp.write(
                "".join(
                    [
                        f"v {v[0]:f} {v[1]:f} {v[2]:f}\n"
                        for v in body_representation.v[0].detach().cpu().numpy()
                    ]
                )
            )
        ...
        for obj in objects:
            ...
            for material in obj.get_materials():
                ...
                principled_bsdf.inputs[
                    "Base Color"
                ].default_value = mathutils.Vector([*skin_tone_rgb, 1.0])
                ...
                principled_bsdf.inputs[
                    "Subsurface Radius"
                ].default_value = mathutils.Vector([1.0, 0.2, 0.1])
                principled_bsdf.inputs["Subsurface IOR"].default_value = 2.5

                # darker skin looks better when made less specular
                principled_bsdf.inputs["Specular IOR Level"].default_value = (
                    np.mean(skin_tone_rgb) / 255.0
                )
                ...
                if texture_nodes and len(texture_nodes) > 1:
                    ...
                    for node_links in principled_bsdf.inputs["Alpha"].links:
                        if (
                            "ShaderNodeTexImage"
                            in node_links.from_node.bl_idname
                        ):
