...
from blenderproc.python.types.BoneUtility import (
    set_location_constraint,
    set_rotation_constraint,
...
def load_urdf(
    urdf_file: str,
    weight_distribution: str = "rigid",
    fk_offset: Optional[Union[List[float], Vector, np.array]] = None,
    ik_offset: Optional[Union[List[float], Vector, np.array]] = None,
) -> URDFObject:
    ...
    base_joints = get_joints_which_have_link_as_parent(
        urdf_tree.base_link.name, urdf_tree.joints
    )
    ...
    # load links
    links = load_links(
        urdf_tree.links, urdf_tree.joints, armature, urdf_path=urdf_file
    )
...
def get_joints_which_have_link_as_parent(
    link_name: str, joint_trees: List["urchin.Joint"]
) -> List["urchin.Joint"]:
    ...
    return [
        joint_tree
        for i, joint_tree in enumerate(joint_trees)
        if joint_tree.parent == link_name
    ]


def get_joints_which_have_link_as_child(
    link_name: str, joint_trees: List["urchin.Joint"]
) -> Optional["urchin.Joint"]:
    ...
    if not valid_joint_trees:  # happens for the very first link
        warnings.warn(
            f"WARNING: There is no joint defined for the link {link_name}!"
        )
        return None
    if len(valid_joint_trees) == 1:
        return valid_joint_trees[0]
    raise NotImplementedError(
        f"More than one ({len(valid_joint_trees)}) joints map onto a single link with name "
        f"{link_name}"
    )
...
def create_bone(
    ...
    joint_tree: "urchin.Joint",
    all_joint_trees: List["urchin.Joint"],
    parent_bone_name: Optional[str] = None,
    create_recursive: bool = True,
    ...
    if joint_tree.joint_type == "fixed":
        ...
        set_rotation_constraint(
            bone=bone, x_limits=[0, 0], y_limits=limits, z_limits=[0, 0]
        )
        set_rotation_constraint(
            bone=fk_bone, x_limits=[0, 0], y_limits=limits, z_limits=[0, 0]
        )
        set_rotation_constraint(
            bone=ik_bone, x_limits=[0, 0], y_limits=limits, z_limits=[0, 0]
        )
        set_copy_rotation_constraint(
            bone=bone,
            target=armature,
            target_bone=fk_bone.name,
            custom_constraint_name="copy_rotation.fk",
        )
        set_copy_rotation_constraint(
            bone=bone,
            target=armature,
            target_bone=ik_bone.name,
            custom_constraint_name="copy_rotation.ik",
            influence=0.0,
        )  # start in fk mode per default
    else:
        warnings.warn(
            f"WARNING: No constraint implemented for joint type '{joint_tree.joint_type}'!"
        )

    if create_recursive:
        child_joints = get_joints_which_have_link_as_parent(
            link_name=joint_tree.child, joint_trees=all_joint_trees
        )
...
def load_links(
    link_trees: List["urchin.Link"],
    joint_trees: List["urchin.Joint"],
    armature: bpy.types.Armature,
    urdf_path: str,
) -> List[Link]:
    ...
    for link_tree in link_trees:
        ...
        if link_tree.visuals:
            visuals = [
                load_visual_collision_obj(
                    visual_tree,
                    name=f"{link_tree.name}_visual",
                    urdf_path=urdf_path,
                )
        ...
        if link_tree.collisions:
            collisions = [
                load_visual_collision_obj(
                    collision_tree,
                    name=f"{link_tree.name}_collision",
                    urdf_path=urdf_path,
                )
        ...
        if link_tree.inertial:
            inertial = load_inertial(
                link_tree.inertial, name=f"{link_tree.name}_inertial"
            )

        # determine bone name
        corresponding_joint = get_joints_which_have_link_as_child(
            link_tree.name, joint_trees
        )

        # create link and set attributes
        link = Link(
            bpy_object=create_with_empty_mesh(link_tree.name).blender_obj
        )
        link.set_armature(armature)
        link.set_visuals(visuals)
        link.set_visual_local2link_mats(
            [Matrix(obj.get_local2world_mat()) for obj in visuals]
        )
        link.set_collisions(collisions)
        link.set_collision_local2link_mats(
            [Matrix(obj.get_local2world_mat()) for obj in collisions]
        )
        link.set_inertial(inertial)
        link.set_inertial_local2link_mat(
            Matrix(inertial.get_local2world_mat())
        ...
        if corresponding_joint is not None:
            ...
            link.set_fk_bone(
                armature.pose.bones.get(corresponding_joint.name + ".fk")
            )
            link.set_ik_bone(
                armature.pose.bones.get(corresponding_joint.name + ".ik")
            )
...
def propagate_pose(
    ...
    """
    child_link = one_by_attr(
        elements=links, attr_name="name", value=joint_tree.child
    )
    parent_link = one_by_attr(
        elements=links, attr_name="name", value=joint_tree.parent
    )
    ...
    for obj in child_link.get_all_objs():
        obj.set_local2world_mat(
            Matrix(child_link.get_local2world_mat())
            @ Matrix(obj.get_local2world_mat())
        )
    ...
    if recursive:
        child_joint_trees = get_joints_which_have_link_as_parent(
            child_link.get_name(), joint_trees
        )
        for child_joint_tree in child_joint_trees:
            propagate_pose(
                links, child_joint_tree, joint_trees, armature, recursive=True
            )


def load_geometry(
    geometry_tree: "urchin.Geometry", urdf_path: Optional[str] = None
) -> MeshObject:
    ...
    if geometry_tree.mesh is not None:
        if os.path.isfile(geometry_tree.mesh.filename):
            ...
            relative_path = os.path.join(
                "/".join(urdf_path.split("/")[:-1]), geometry_tree.mesh.filename
            )
            if os.path.isfile(relative_path):
                # load in default coordinate system
                obj = load_obj(
                    filepath=relative_path, forward_axis="Y", up_axis="Z"
                )[0]
            else:
                warnings.warn(
                    f"Couldn't load mesh file for {geometry_tree} (filename: {geometry_tree.mesh.filename}; "
                    f"urdf filename: {urdf_path})"
                )
        else:
            raise NotImplementedError(
                f"Couldn't load mesh file for {geometry_tree} (filename: "
        ...
        obj = create_primitive(
            shape="CYLINDER",
            radius=geometry_tree.cylinder.radius,
            depth=geometry_tree.cylinder.length,
        )
    elif geometry_tree.sphere is not None:
        obj = create_primitive(
            shape="SPHERE", radius=geometry_tree.sphere.radius
        )
    else:
        raise NotImplementedError(
            f"Unknown geometry in urdf_tree {geometry_tree}"
        )
...
def load_visual_collision_obj(
    viscol_tree: Union["urchin.Visual", "urchin.Collision"],
    name: str,
    ...
    if hasattr(viscol_tree, "material") and viscol_tree.material is not None:
        ...
        if viscol_tree.material.texture is not None:
            ...
            mat = MaterialLoaderUtility.create(
                name=viscol_tree.material.name + "_texture"
            )
            nodes = mat.nodes
            links = mat.links
            ...
            if not os.path.exists(viscol_tree.material.texture.filename):
                raise Exception(
                    f"Couldn't load texture image for {viscol_tree} from "
                    f"{viscol_tree.material.texture.filename}"
                )
            color_image.image = bpy.data.images.load(
                viscol_tree.material.texture.filename, check_existing=True
            )
            ...
            links.new(
                color_image.outputs["Color"], principled.inputs["Base Color"]
            )
...
def load_inertial(inertial_tree: "urchin.Inertial", name: str) -> Inertial:
    ...
    primitive.blender_obj.dimensions = Vector(
        [0.03, 0.03, 0.03]
    )  # just small values to keep cubes small for debugging
    primitive.persist_transformation_into_mesh(
        location=True, rotation=True, scale=True
    )
    return inertial
...
def get_size_from_geometry(geometry: "urchin.Geometry") -> Optional[float]:
    ...
    if geometry.mesh is not None:
        if (
            hasattr(geometry.geometry, "scale")
            and geometry.geometry.scale is not None
        ):
            return max(geometry.geometry.scale)
        if (
            hasattr(geometry.geometry, "size")
    ...
    print(
        f"Warning: Failed to derive size from geometry model {geometry}. Setting scale to 0.2!"
    )
    return None
