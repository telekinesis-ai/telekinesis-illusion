...
def identify_base_color_image_path(
    texture_map_paths: List[str],
...
def identify_texture_maps(
    texture_folder_path: Union[str, Path],
    ...
    texture_map_paths = [
        str(path.absolute()) for path in texture_folder_path.glob("*.jpg")
    ]
    color_path, color_identifier = identify_base_color_image_path(
        texture_map_paths
    )
    ...
    for type_val in texture_map_types:
        for identifier in _texture_map_identifiers[type_val]:
            texture_map_path_lowercase = color_path.replace(
...
def load_haven_mat(
    folder_path: Union[str, Path] = "resources/haven",
    used_assets: Optional[List[str]] = None,
    preload: bool = False,
    fill_used_empty_materials: bool = False,
    add_cp: Optional[Dict[str, Any]] = None,
    return_random_element: bool = False,
    ...
    if not haven_folder.exists():
        raise FileNotFoundError(
            f"The given haven folder does not exist: {haven_folder}"
        )
    ...
    if used_assets:
        texture_names = [
            ...
            if any(texture_name.startswith(asset) for asset in used_assets)
        ]
        if not texture_names:
            raise FileNotFoundError(
                f"No texture folders found in {haven_folder} for which used_assets "
    ...
    for texture_name in texture_names:
        ...
        texture_map_paths_by_type = identify_texture_maps(
            str(texture_folder_path)
        )
        ...
        if (
            ...
            and not MaterialLoaderUtility.is_material_used(new_mat)
        ):
    ...
    if return_random_element:
        if len(materials) != 1:
            raise RuntimeError(
                f"The amount of loaded materials is not one: {materials}, this should not happen!"
            )
        return materials[0]
...
class HavenMaterialLoader:
    ...
    def create_material(
        ...
        alpha_node = MaterialLoaderUtility.add_alpha(
            nodes, links, alpha_image_path, principled_bsdf
        )
        ...
        if not os.path.exists(normal_image_path):
            bump_node = MaterialLoaderUtility.add_bump(
                nodes, links, bump_image_path, principled_bsdf
            )
        ...
        MaterialLoaderUtility.connect_uv_maps(
            nodes, links, collection_of_texture_nodes
        )
