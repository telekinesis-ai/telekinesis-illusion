...
def load_front3d(
    ...
    created_objects = _Front3DLoader.create_mesh_objects_from_file(
        data,
    ...
    all_loaded_furniture = _Front3DLoader.load_furniture_objs(
...
class _Front3DLoader:
    ...
    @staticmethod
    def extract_hash_nr_for_texture(
        given_url: str, front_3D_texture_path: str
    ) -> str:
        """
    ...
    @staticmethod
    def get_used_image(
        hash_folder_path: str, saved_image_dict: Mapping[str, bpy.types.Texture]
    ) -> bpy.types.Texture:
        """
    ...
    @staticmethod
    def create_mesh_objects_from_file(
        data: dict,
        front_3D_texture_path: str,
        ...
        for mesh_data in data["mesh"]:
            ...
            if used_mat:
                if used_mat["texture"]:
                    ...
                    if (
                        hash_folder in used_materials_based_on_texture
                        and "ceiling" not in used_obj_name.lower()
                    ):
                        ...
                        if used_mat["color"]:
                            principled_node.inputs[
                                ...
                                mathutils.Vector(used_mat["color"]) / 255.0
                            )
                        ...
                        if used_mat["normaltexture"]:
                            ...
                            # extract the has folder is from the url and download it if necessary
                            hash_folder = (
                                _Front3DLoader.extract_hash_nr_for_texture(
                                    used_mat["normaltexture"],
                                    front_3D_texture_path,
                                )
                            )
                            used_image = _Front3DLoader.get_used_image(
                            ...
                            normal_texture = (
                                MaterialLoaderUtility.create_image_node(
                ...
                # if there is a normal color used
                    ...
                    if (
                        used_hash in used_materials_based_on_color
                        and "ceiling" not in used_obj_name.lower()
                    ):
            ...
            if (
                ...
                for obj in objs:
                    ...
                    for mat in obj.get_materials():
                        ...
                        principled_node.inputs["Emission Color"].default_value[
                            :3
                        ] = [0, 0, 0]
    ...
    @staticmethod
    def move_and_duplicate_furniture(
        data: dict, all_loaded_furniture: list
    ) -> List[MeshObject]:
        """
                ...
                if "furniture" in child["instanceid"]:
                    ...
                    for obj in all_loaded_furniture:
                        if obj.get_cp("uid") == child["ref"]:
                            ...
                            rotation_mat = (
                                mathutils.Quaternion(child["rot"])
                                .to_euler()
                                .to_matrix()
                                .to_4x4()
                            )
