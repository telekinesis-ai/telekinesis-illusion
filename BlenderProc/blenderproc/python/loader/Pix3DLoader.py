...
def load_pix3d(
    used_category: str, data_path: str = "resources/pix3d"
) -> List[MeshObject]:
    ...
    files_with_fitting_category = _Pix3DLoader.get_files_with_category(
        used_category, data_path
    )
...
class _Pix3DLoader:
    ...
    def get_files_with_category(used_category: str, data_path: str) -> list:
        ...
        if os.path.exists(path_to_annotation_file):
            files = []
            path_to_category_file = os.path.join(
                data_path, f"category_{used_category.strip()}_paths.txt"
            )
        ...
        raise FileNotFoundError(
            f"The annotation file could not be found: {path_to_annotation_file}"
        )
