...
def load_bop_objs(
    ...
    if mm2m is not None:
        warnings.warn(
            "WARNING: `mm2m` is deprecated, please use `object_model_unit='mm'` instead!"
        )
        scale = 0.001
    ...
    if sample_objects:
        ...
        while loaded_amount != num_of_objs_to_sample:
            ...
            if (
                obj_instances_limit == -1
                or loaded_ids[random_id] < obj_instances_limit
            ):
                cur_obj = _BopLoader.load_mesh(
                    random_id, model_p, bop_dataset_name, scale
                )
        ...
        for obj_id in obj_ids:
            cur_obj = _BopLoader.load_mesh(
                obj_id, model_p, bop_dataset_name, scale
            )
            loaded_objects.append(cur_obj)
            ...
            for inst in insts:
                cur_objs.append(
                    _BopLoader.load_mesh(
                        inst["obj_id"], model_p, bop_dataset_name, scale
                    )
                )
...
class _BopLoader:
    ...
    def compute_camera_to_world_trafo(
        cam_H_m2w_ref: np.array,
        cam_H_m2c_ref: np.array,
        source_frame: List[str],
    ) -> np.ndarray:
    ...
    def get_ref_cam_extrinsics_intrinsics(
        ...
        cam_H_m2c_ref[:3, 3] = (
            np.array(insts[0]["cam_t_m2c"]).reshape(3) * scale
        )
    ...
    @staticmethod
    def load_mesh(
        obj_id: int, model_p: dict, bop_dataset_name: str, scale: float = 1
    ) -> MeshObject:
        ...
        cur_obj.get_materials()[-1].set_name(
            "bop_" + bop_dataset_name + "_vertex_col_material"
        )
        cur_obj.set_scale(Vector((scale, scale, scale)))
