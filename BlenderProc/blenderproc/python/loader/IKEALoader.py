...
def load_ikea(
    data_dir: str = "resources/IKEA",
    obj_categories: Optional[Union[List[str], str]] = None,
    obj_style: Optional[str] = None,
) -> List[MeshObject]:
    ...
    if obj_categories is not None and obj_style is not None:
        ...
        if not object_lst:
            ...
            warnings.warn(
                f"Could not find object of type: {obj_categories}, and style: "
                f"{obj_style}. Selecting random object...",
                category=Warning,
            )
        else:
        ...
        for obj_category in obj_categories:
            object_lst.extend(
                _IKEALoader.get_object_by_type(obj_category, obj_dict)
            )
        selected_obj = random.choice(object_lst)
        ...
        for line in first_lines:
            if "File units" in line:
                ...
                if file_unit not in [
                    ...
                    raise RuntimeError(
                        f"The file unit type could not be found, check the selected "
                        f"file: {selected_obj}"
                    )
                break
    ...
    for obj in loaded_obj:
        ...
        if file_unit == "inches":
            ...
            raise RuntimeError(
                f"The file unit type: {file_unit} is not defined"
            )
        if scale != 1.0:
...
class _IKEALoader:
    ...
    def generate_object_dict(data_dir: str) -> Dict[str, List[str]]:
        ...
        print(
            f"Found {counter} object files in dataset belonging to {len(obj_dict)} categories"
        )
        if not obj_dict:
            raise RuntimeError(
                "No obj file was found, check if the correct folder is provided!"
            )
    ...
    def get_object_by_type(
        obj_type: str, obj_dict: Dict[str, List[str]]
    ) -> List[str]:
        """
        ...
        """
        object_lst = [
            obj[0] for (key, obj) in obj_dict.items() if obj_type in key
        ]
        if not object_lst:
            warnings.warn(
                f"There were no objects found matching the type: {obj_type}.",
                category=Warning,
            )
        return object_lst

    @staticmethod
    def get_object_by_style(
        obj_style: str, obj_dict: Dict[str, List[str]]
    ) -> List[str]:
        """
        ...
        object_lst = [
            obj[0]
            for (key, obj) in obj_dict.items()
            if obj_style in key.lower()
        ]
        if not object_lst:
            warnings.warn(
                f"There were no objects found matching the style: {obj_style}.",
                category=Warning,
            )
        return object_lst
