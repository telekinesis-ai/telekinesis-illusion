"""Provides functionality to load objects from the ShapeNetCore.v2 dataset."""
...
def load_shapenet(
    ...
    files_with_fitting_synset = _ShapeNetLoader.get_files_with_synset(
    ...
    if len(loaded_objects) != 1:
        raise RuntimeError(
            "The ShapeNetLoader expects every .obj file to contain exactly one object, "
            "however the file "
            + selected_obj
            + " contained "
            + str(len(loaded_objects))
            + " objects."
        )
...
class _ShapeNetLoader:
    @staticmethod
    def get_files_with_synset(
        used_synset_id: str,
        used_source_id: str,
        path_to_taxonomy_file: str,
        data_path: str,
    ) -> list:
        ...
        if os.path.exists(path_to_taxonomy_file):
            ...
            with open(path_to_taxonomy_file, "r", encoding="utf-8") as f:
                ...
                parent_synset_id = _ShapeNetLoader.find_parent_synset_id(
                    data_path, used_synset_id, loaded_data
                )
                ...
                if not used_source_id:
                    files.extend(
                        glob.glob(
                            os.path.join(
                                id_path, "*", "models", "model_normalized.obj"
                            )
                        )
                    )
                else:
                    if not os.path.exists(
                        os.path.join(id_path, used_source_id)
                    ):
                        raise ValueError(
                            f"The used_source_id {used_source_id} is not correct"
                        )
        ...
        raise FileNotFoundError(
            f"The taxonomy file could not be found: {path_to_taxonomy_file}"
        )
    ...
    def find_parent_synset_id(data_path, synset_id, json_data):
        ...
        for block in json_data:
            if synset_id in block["children"]:
                ...
                return _ShapeNetLoader.find_parent_synset_id(
                    data_path, parent_synset_id, json_data
                )

        raise ValueError(
            f"The used_synset_id {synset_id} does not exists in the taxonomy file"
        )
