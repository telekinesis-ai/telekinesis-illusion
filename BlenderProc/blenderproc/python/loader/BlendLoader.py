...
from blenderproc.python.utility.BlenderUtility import (
    collect_all_orphan_data_blocks,
)
from blenderproc.python.types.EntityUtility import (
    Entity,
    convert_to_entity_subclass,
)
...
def load_blend(
    path: str,
    obj_types: Optional[Union[List[str], str]] = None,
    name_regrex: Optional[str] = None,
    data_blocks: Union[List[str], str] = "objects",
    link: bool = False,
) -> List[Entity]:
    """
    ...
    data_blocks = _BlendLoader.validate_and_standardizes_configured_list(
        data_blocks, _BlendLoader.valid_data_blocks, "data block"
    )
    obj_types = _BlendLoader.validate_and_standardizes_configured_list(
        obj_types, _BlendLoader.valid_object_types, "object type"
    )
    ...
    with bpy.data.libraries.load(path, link=link) as (data_from, data_to):
        for data_block in data_blocks:
            ...
            if hasattr(data_from, data_block):
                ...
                print(
                    "Imported " + str(len(data_to_entities)) + " " + data_block
                )
            else:
...
class _BlendLoader:
    valid_data_blocks = [
        ...
        if isinstance(
            getattr(bpy.data, collection), bpy.types.bpy_prop_collection
        )
    ]
    valid_object_types = [
        "mesh",
    ...
    @staticmethod
    def validate_and_standardizes_configured_list(
        config_value: Union[list, str],
    ...
    def purge_added_orphans(orphans_before, data_to):
        ...
        while purge_orphans:
            ...
            for collection_name, orphan_block in orphans_after.items():
                ...
                for orphan in orphan_block.difference(
                    orphans_before[collection_name]
                ):
