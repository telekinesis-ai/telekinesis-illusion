...
def set_world_background_hdr_img(
    ...
    if not os.path.exists(path_to_hdr_file):
        raise FileNotFoundError(
            f"The given path does not exists: {path_to_hdr_file}"
        )
    ...
    texture_node.image = bpy.data.images.load(
        path_to_hdr_file, check_existing=True
    )
    ...
    links.new(
        tex_coords_node.outputs["Generated"], mapping_node.inputs["Vector"]
    )
...
def get_random_world_background_hdr_img_path_from_haven(data_path: str) -> str:
    ...
    if os.path.exists(data_path):
        ...
        if not os.path.exists(data_path):
            raise FileNotFoundError(
                f"The folder: {data_path} does not contain a folder name hdfris. "
