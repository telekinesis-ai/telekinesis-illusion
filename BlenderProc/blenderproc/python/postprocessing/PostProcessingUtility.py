...
def dist2depth(
    dist: Union[List[np.ndarray], np.ndarray],
...
def depth2dist(
    depth: Union[List[np.ndarray], np.ndarray],
) -> Union[List[np.ndarray], np.ndarray]:
    """
    ...
    if (
        isinstance(depth, list)
        or hasattr(depth, "shape")
...
def remove_segmap_noise(
    ...
    for index in noise_indices:
        neighbors = _PostProcessingUtility.get_pixel_neighbors(
            image, index[0], index[1]
        )  # Extracting the indices surrounding 3x3 neighbors
        curr_val = image[index[0]][index[1]][
            0
        ]  # Current value of the noisy pixel

        neighbor_vals = [
            image[neighbor[0]][neighbor[1]] for neighbor in neighbors
        ]  # Getting the values of the neighbors
        # Getting the unique values only
        neighbor_vals = np.unique(
            np.array([np.array(index) for index in neighbor_vals])
        )
...
def oil_paint_filter(
    ...
    if rgb:
        if (
            ...
            return [
                oil_paint_filter(img, filter_size, edges_only, rgb)
                for img in image
            ]
        ...
        neighbors = np.array(
            _PostProcessingUtility.get_pixel_neighbors_stacked(
                image, filter_size, return_list=True
            )
        )
        neighbors_intensity = (
            _PostProcessingUtility.get_pixel_neighbors_stacked(
                intensity_img, filter_size
            )
        )

        mode_intensity = stats.mode(neighbors_intensity, axis=2)[0].reshape(
            image.shape[0], image.shape[1]
        )

        # keys here would match all instances of the mode value
        mode_keys = np.argwhere(
            neighbors_intensity == np.expand_dims(mode_intensity, axis=3)
        )
        # Remove the duplicate keys, since they point to the same value, and to be able to use them for indexing
        _, unique_indices = np.unique(
            mode_keys[:, 0:2], axis=0, return_index=True
        )
        unique_keys = mode_keys[unique_indices]

        filtered_img = neighbors[
            unique_keys[:, 2], unique_keys[:, 0], unique_keys[:, 1], :
        ].reshape(image.shape[0], image.shape[1], image.shape[2])

        if edges_only:
            edges = cv2.Canny(
                image, 0, np.max(image)
            )  # Assuming "image" is an uint8 array.
        ...
        filtered_img = stats.mode(
            _PostProcessingUtility.get_pixel_neighbors_stacked(
                image, filter_size
            ),
            axis=2,
        )[0]
        filtered_img = filtered_img.reshape(
            filtered_img.shape[0], filtered_img.shape[1]
        )
        ...
        if edges_only:
            ...
            _max = (
                np.max(_image)
                if np.max(_image) != np.inf
                else np.unique(_image)[-2]
            )
...
def add_kinect_azure_noise(
    ...
    if (
        ...
        assert len(color) == len(depth), (
            "Enter same number of depth and color images"
        )
        return [
            add_kinect_azure_noise(d, c, missing_depth_darkness_thres)
    ...
    depth += (
        5 / 10000 + np.maximum((depth - 0.5) * 1 / 1000, 0)
...
def trim_redundant_channels(
    ...
    if (
        ...
        image = image[
            :, :, 0
        ]  # All channels have the same value, so just extract any single channel

    return image


def segmentation_mapping(
    ...
    return_dict: Dict[
        str, Union[np.ndarray, List[np.ndarray], List[Dict[str, Any]]]
    ] = {}
    ...
    if isinstance(image, list):
        ...
        if (
            hasattr(image[0], "shape")
            and len(image[0].shape) == 2
            and not is_stereo_case
        ):
            # convert list of images to np.ndarray
            image = np.array(image)[
                :, np.newaxis, :, :
            ]  # reshape for the stereo case
    elif hasattr(image, "shape") and len(image.shape) == 3:
        if not is_stereo_case:
            image = np.array(image)[
                :, np.newaxis, :, :
            ]  # reshape for stereo case
        else:
        ...
        if is_stereo_case:
            raise RuntimeError(
                "The amount of dimensions for an image must be higher than two in stereo mode!"
            )
        ...
        for stereo_image in frame_image:
            ...
            for map_by_attribute in map_by:
                ...
                resulting_map = np.zeros(
                    (stereo_image.shape[0], stereo_image.shape[1]),
                    dtype=np.float64,
                )
                ...
                if map_by_attribute == "instance":
                    mapped_results_stereo_dict.setdefault(
                        f"{map_by_attribute}_segmaps", []
                    ).append(stereo_image)
                else:
                    ...
                    for object_id in object_ids:
                        ...
                        # if the current object has a custom property with that name -> get it
                            ...
                            if current_attribute == "cf_basename":
                                ...
                                raise ValueError(
                                    f'The given attribute is a custom function: "cf_", but it is not '
                            ...
                            raise RuntimeError(
                                f'The object "{current_obj.name}" does not have the '
                        ...
                        if isinstance(
                            value, (mathutils.Vector, mathutils.Matrix)
                        ):
                            value = np.array(value)

                        if object_id in non_image_attributes:
                            non_image_attributes[object_id][
                                current_attribute
                            ] = value
                        else:
                            non_image_attributes[object_id] = {
                    ...
                    if found_dtype is not None:
                        ...
                        raise ValueError(
                            f'The map_by key "{map_by_attribute}" requires that the instance map is '
...
class _PostProcessingUtility:
    ...
    def get_pixel_neighbors(data: np.ndarray, i: int, j: int) -> np.ndarray:
        ...
        for p in range(max(0, i - 1), min(data.shape[0], i + 2)):
            for q in range(max(0, j - 1), min(data.shape[1], j + 2)):
                if not (
                    p == i and q == j
                ):  # We don't want the current pixel, just the neighbors
                    neighbors.append([p, q])

        return np.array(neighbors)

    @staticmethod
    def get_pixel_neighbors_stacked(
        img: np.ndarray, filter_size: int = 3, return_list: bool = False
    ) -> Union[list, np.ndarray]:
        """
        ...
        for p in range(_min, _max):
            for q in range(_min, _max):
                ...
                shifted[
                    max(p, 0) : min(rows, rows + p),
                    max(q, 0) : min(cols, cols + q),
                ] = img[
    ...
    def determine_noisy_pixels(image: np.ndarray) -> np.ndarray:
        ...
        noise_indices = np.argwhere(
            _PostProcessingUtility.is_in(image, noise_vals)
        )
