...
def stereo_global_matching(
    ...
    if depth_max is None:
        depth_max = (
            bpy.context.scene.world.mist_settings.start
            + bpy.context.scene.world.mist_settings.depth
        )
    ...
    for color_image in color_images:
        depth, disparity = _StereoGlobalMatching.stereo_global_matching(
            color_image[0],
            color_image[1],
            baseline,
            depth_max,
...
class _StereoGlobalMatching:
    ...
    def stereo_global_matching(
        ...
        if disparity_filter:
            filteredImg = wls_filter.filter(
                displ, left_color_image, None, dispr
            ).astype(np.float32)
    ...
    def fill_in_fast(
        ...
        if extrapolate:
            ...
            top_pixel_values = depth_map[
                top_row_pixels, range(depth_map.shape[1])
            ]
