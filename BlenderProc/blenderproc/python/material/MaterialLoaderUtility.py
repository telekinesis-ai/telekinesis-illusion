...
def convert_to_materials(
    blender_materials: List[Optional[bpy.types.Material]],
    ...
    """
    return [
        (None if obj is None else Material(obj)) for obj in blender_materials
    ]


def find_cc_material_by_name(
    material_name: str, custom_properties: Dict[str, Any]
) -> bpy.types.Material:
    """
...
def create_new_cc_material(
    material_name: str, add_custom_properties: dict
) -> bpy.types.Material:
    """
...
def add_base_color(
    ...
    if os.path.exists(base_image_path):
        base_color = create_image_node(
            nodes, base_image_path, False, _x_texture_node, _y_texture_node
        )
        links.new(
            base_color.outputs["Color"], principled_bsdf.inputs["Base Color"]
        )
        return base_color
    return None
...
def add_ambient_occlusion(
    ...
    if os.path.exists(ambient_occlusion_image_path):
        ...
        links.new(
            math_node.outputs["Color"], principled_bsdf.inputs["Base Color"]
        )
...
def add_metal(
    ...
    if os.path.exists(metalness_image_path):
        metallic = create_image_node(
            nodes, metalness_image_path, True, _x_texture_node, 0
        )
...
def add_roughness(
    ...
    if os.path.exists(roughness_image_path):
        roughness_texture = create_image_node(
            nodes,
...
def add_specular(
    ...
    if os.path.exists(specular_image_path):
        specular_texture = create_image_node(
            nodes, specular_image_path, True, _x_texture_node, 0
        )
        links.new(
            specular_texture.outputs["Color"],
            principled_bsdf.inputs["Specular IOR Level"],
        )
...
def add_alpha(
    ...
    if os.path.exists(alpha_image_path):
        alpha_texture = create_image_node(
            nodes, alpha_image_path, True, _x_texture_node, _y_texture_node * -2
        )
        links.new(
            alpha_texture.outputs["Color"], principled_bsdf.inputs["Alpha"]
        )
...
def add_normal(
    ...
    if os.path.exists(normal_image_path):
        normal_texture = create_image_node(
            nodes, normal_image_path, True, _x_texture_node, normal_y_value
        )
        if invert_y_channel:
            ...
            links.new(
                normal_texture.outputs["Color"], separate_rgba.inputs["Image"]
            )
...
def add_bump(
    ...
    if os.path.exists(bump_image_path):
        bump_texture = create_image_node(
            nodes, bump_image_path, True, _x_texture_node, bump_y_value
        )
...
def add_displacement(
    ...
    if os.path.exists(displacement_image_path):
        displacement_texture = create_image_node(
            nodes,
...
def connect_uv_maps(
    nodes: bpy.types.Nodes,
    links: bpy.types.NodeLinks,
    collection_of_texture_nodes: list,
):
    """
        ...
        for slot in obj.material_slots:
            ...
            if texture_node is not None:
                ...
                node_connected_to_the_output, material_output = (
                    Utility.get_node_connected_to_the_output_and_unlink_it(
                        material
                    )
                )
                ...
                if node_connected_to_the_output is not None:
                    ...
                    if blurry_edges:
                        ...
                        links.new(
                            step_function_node.outputs["Value"],
                            mix_node.inputs["Fac"],
                        )

                    links.new(
                        node_connected_to_the_output.outputs[0],
                        mix_node.inputs[2],
                    )
                    ...
                    raise RuntimeError(
                        f"Could not find shader node, which is connected to the material "
        ...
        for slot in used_object.material_slots:
            ...
            if not emission_shader:
                if use_alpha_channel:
                    slot.material = add_alpha_texture_node(
...
def create_procedural_texture(
    ...
    if pattern_name is None:
        ...
        if pattern_name not in possible_patterns:
            raise RuntimeError(
                f"There is no such pattern: {pattern_name}. Allowed patterns are: {possible_patterns}"
            )

    return bpy.data.textures.new(f"ct_{pattern_name}", pattern_name)


def create_material_from_texture(
    texture: Union[Path, str, bpy.types.Image], material_name: str
) -> Material:
    """
    ...
    if isinstance(texture, (Path, str)):
        ...
        raise TypeError(
            f"The given type of texture must be either [str, Path, bpy.types.Image] "
    ...
    if texture_path:
        if texture_path.exists():
            ...
            raise FileNotFoundError(
                f'The given texture path could not be found: "{texture_path}"'
            )
    ...
    if isinstance(texture, bpy.types.Image):
        ...
        return bp_mat
    raise TypeError(
        "The texture variable should be a bpy.types.Image at this point!"
    )
