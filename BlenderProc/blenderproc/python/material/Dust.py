...
def add_dust(
    material: Material,
    strength: float,
    texture_nodes: List[bpy.types.Texture] = None,
    ...
    group = bpy.data.node_groups.new(
        name="Dust Material", type="ShaderNodeTree"
    )
    ...
    links.new(
        separate_z_normal.outputs["B"], mix_with_layer_weight.inputs["Color1"]
    )
    links.new(
        layer_weight.outputs["Facing"], mix_with_layer_weight.inputs["Color2"]
    )
    ...
    links.new(
        mix_with_layer_weight.outputs["Color"], gamma_node.inputs["Color"]
    )
    ...
    if texture_nodes is not None and texture_nodes:
        ...
        if not texture_nodes:
            warnings.warn(
                "No texture was found, check the config. Random generated texture is used."
            )
        ...
        links.new(
            noise_node.outputs["Color"], separate_r_channel.inputs["Image"]
        )
    ...
    group.interface.new_socket(
        mix_shader.outputs[0].name,
        in_out="OUTPUT",
        socket_type=mix_shader.outputs[0].bl_idname,
    )
    group.interface.new_socket(
        mix_shader.inputs[1].name,
        in_out="INPUT",
        socket_type=mix_shader.inputs[1].bl_idname,
    )
    group.interface.new_socket(
        "Dust strength",
        in_out="INPUT",
        socket_type=multiply_node.inputs[1].bl_idname,
    )
    ...
    links.new(
        group_input.outputs["Texture scale"], mapping_node.inputs["Scale"]
    )
    ...
    node_connected_to_the_output, material_output = (
        material.get_node_connected_to_the_output_and_unlink_it()
    )

    # place the group node above the material output
    group_node.location = (
        material_output.location.x - x_diff,
        material_output.location.y + y_diff,
    )
