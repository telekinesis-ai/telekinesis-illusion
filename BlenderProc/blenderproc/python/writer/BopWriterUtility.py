...
def write_bop(
    ...
    if not os.path.exists(dataset_dir):
        ...
        raise FileExistsError(
            f"The output folder already exists: {dataset_dir}"
        )
    ...
    if target_objects is not None:
        ...
        for obj in dataset_objects:
            if obj.is_hidden():
                print(
                    f"WARNING: The given object {obj.get_name()} is hidden. However, the bop writer will still add "
    ...
    if not dataset_objects:
        raise RuntimeError(
            f"The scene does not contain any object from the specified dataset: {dataset}. "
    ...
    if calc_mask_info_coco:
        ...
        if last_chunk_dir:
            ...
            chunk_gt = _BopWriterUtility.load_json(
                last_chunk_gt_fpath, keys_to_int=True
            )
    ...
    assert annotation_unit in ["m", "dm", "cm", "mm"], (
        f"Invalid annotation unit: `{annotation_unit}`. Supported "
        f"are 'm', 'dm', 'cm', 'mm'"
    )
    annotation_scale = {"m": 1.0, "dm": 10.0, "cm": 100.0, "mm": 1000.0}[
        annotation_unit
    ]
    if m2mm is not None:
        warnings.warn(
            "WARNING: `m2mm` is deprecated, please use `annotation_scale='mm'` instead!"
        )
        annotation_scale = 1000.0
    _BopWriterUtility.write_frames(
        chunks_dir,
    ...
    if calc_mask_info_coco:
        ...
        SetupUtility.setup_pip(
            ["git+https://github.com/thodan/bop_toolkit", "PyOpenGL==3.1.0"]
        )
        ...
        chunk_dirs = chunk_dirs[
            chunk_dir_ids.index(f"{starting_chunk_id:06d}") :
        ]
        ...
        for obj in dataset_objects:
            ...
            if isinstance(obj, Link):
                ...
                if len(obj.visuals) > 1:
                    warnings.warn(
                        "BOP Writer only supports saving annotations of one visual mesh per Link"
                    )
            ...
            if not np.all(
                np.isclose(
                    np.array(obj.blender_obj.scale), obj.blender_obj.scale[0]
                )
            ):
                print(
                    "WARNING: the scale is not the same across all dimensions, writing bop_toolkit annotations with "
...
def bop_pose_to_pyrender_coordinate_system(
    cam_R_m2c: np.ndarray, cam_t_m2c: np.ndarray
) -> np.ndarray:
    ...
    return change_target_coordinate_frame_of_transformation_matrix(
        bop_pose, ["X", "-Y", "-Z"]
    )
...
class _BopWriterUtility:
    ...
    def save_depth(path: str, im: np.ndarray):
        ...
        w_depth = png.Writer(
            im.shape[1], im.shape[0], greyscale=True, bitdepth=16
        )
    ...
    @staticmethod
    def get_frame_gt(
        dataset_objects: List[bpy.types.Mesh],
        unit_scaling: float,
        ...
        for obj in dataset_objects:
            if isinstance(obj, Link):
                ...
                if len(obj.visuals) > 1:
                    warnings.warn(
                        "BOP Writer only supports saving poses of one visual mesh per Link"
                    )
                H_m2w = Matrix(obj.get_visual_local2world_mats()[0])
            else:
                H_m2w = Matrix(obj.get_local2world_mat())
                assert obj.has_cp("category_id"), (
                    f"{obj.get_name()} object has no custom property 'category_id'"
                )
            ...
            if not np.linalg.norm(list(cam_t_m2c)) > ignore_dist_thres:
                ...
                frame_gt.append(
                    {
                        "cam_R_m2c": list(cam_R_m2c[0])
                        + list(cam_R_m2c[1])
                        + list(cam_R_m2c[2]),
                        "cam_t_m2c": cam_t_m2c,
                        "obj_id": obj.get_cp("category_id")
                        if not isinstance(obj, Link)
                        else obj.visuals[0].get_cp("category_id"),
                    }
                )
            else:
                print(
    ...
    def write_frames(
        ...
        rgb_tpath = os.path.join(
            chunks_dir, "{chunk_id:06d}", "rgb", "{im_id:06d}" + "{im_type}"
        )
        depth_tpath = os.path.join(
            chunks_dir, "{chunk_id:06d}", "depth", "{im_id:06d}" + depth_ext
        )
        chunk_camera_tpath = os.path.join(
            chunks_dir, "{chunk_id:06d}", "scene_camera.json"
        )
        chunk_gt_tpath = os.path.join(
            chunks_dir, "{chunk_id:06d}", "scene_gt.json"
        )
        ...
        for frame_id in range(
            ...
            if curr_frame_id == 0:
                ...
                os.makedirs(
                    os.path.dirname(
                        rgb_tpath.format(
                            chunk_id=curr_chunk_id, im_id=0, im_type="PNG"
                        )
                    )
                )
                os.makedirs(
            ...
            chunk_gt[curr_frame_id] = _BopWriterUtility.get_frame_gt(
                dataset_objects, annotation_scale, ignore_dist_thres
            )
            chunk_camera[curr_frame_id] = _BopWriterUtility.get_frame_camera(
                save_world2cam, depth_scale, annotation_scale
            )
            ...
            if color_file_format == "PNG":
                rgb_fpath = rgb_tpath.format(
                    chunk_id=curr_chunk_id, im_id=curr_frame_id, im_type=".png"
                )
                cv2.imwrite(rgb_fpath, color_bgr)
            elif color_file_format == "JPEG":
                rgb_fpath = rgb_tpath.format(
                    chunk_id=curr_chunk_id, im_id=curr_frame_id, im_type=".jpg"
                )
                cv2.imwrite(
                    rgb_fpath,
                    color_bgr,
            ...
            depth_fpath = depth_tpath.format(
    ...
    def _pyrender_init(
        ...
        renderer = pyrender.OffscreenRenderer(
            viewport_width=ren_width, viewport_height=ren_height
        )
        # Create renderer for calc_gt_info
        renderer_large = pyrender.OffscreenRenderer(
            viewport_width=ren_width * 3, viewport_height=ren_height * 3
        )
        ...
        for key in trimesh_objects.keys():
            ...
            dataset_objects[key] = pyrender.Mesh.from_trimesh(
                mesh=trimesh_objects[key], material=material
            )
    ...
    def _calc_gt_masks_iteration(
        ...
        chunk_dir: str,
        im_id: int,
        gt_data: Tuple[int, Dict[str, int]],
    ):
        ...
        camera = pyrender.IntrinsicsCamera(
            fx=fx, fy=fy, cx=cx, cy=cy, znear=0.1, zfar=100000
        )
        ...
        pose = bop_pose_to_pyrender_coordinate_system(
            cam_R_m2c=np.array(gt["cam_R_m2c"]).reshape(3, 3), cam_t_m2c=t
        )
        ...
        mask_path = os.path.join(
            chunk_dir, "mask", "{im_id:06d}_{gt_id:06d}.png"
        ).format(im_id=im_id, gt_id=gt_id)
        inout.save_im(mask_path, 255 * mask.astype(np.uint8))

        mask_visib_path = os.path.join(
            chunk_dir, "mask_visib", "{im_id:06d}_{gt_id:06d}.png"
        ).format(im_id=im_id, gt_id=gt_id)
    ...
    def calc_gt_masks(
        ...
        for dir_counter, chunk_dir in enumerate(chunk_dirs):
            ...
            last_chunk_camera_fpath = os.path.join(
                chunk_dir, "scene_camera.json"
            )
            scene_gt = _BopWriterUtility.load_json(
                last_chunk_gt_fpath, keys_to_int=True
            )
            scene_camera = _BopWriterUtility.load_json(
                last_chunk_camera_fpath, keys_to_int=True
            )

            # Create folders for the output masks (if they do not exist yet).
            mask_dir_path = os.path.dirname(
                os.path.join(chunk_dir, "mask", "000000_000000.png")
            )
            misc.ensure_dir(mask_dir_path)

            mask_visib_dir_path = os.path.dirname(
                os.path.join(chunk_dir, "mask_visib", "000000_000000.png")
            )
            ...
            for im_counter, im_id in enumerate(im_ids):
                if im_counter % 100 == 0:
                    misc.log(
                        f"Calculating GT masks - {chunk_dir}, {im_counter}"
                    )
    ...
    def calc_gt_info(
        ...
        for dir_counter, chunk_dir in enumerate(chunk_dirs):
            ...
            if dir_counter == 0 and starting_frame_id > 0:
                misc.log(
                    f"Loading gt info from existing chunk dir - {chunk_dir}"
                )
                scene_gt_info = _BopWriterUtility.load_json(
                    os.path.join(chunk_dir, "scene_gt_info.json"),
                    keys_to_int=True,
                )
            else:
                scene_gt_info = {}
    ...
    def calc_gt_coco(
        ...
        for dir_counter, chunk_dir in enumerate(chunk_dirs):
            ...
            CATEGORIES = list(
                {frozenset(item.items()): item for item in CATEGORIES}.values()
            )
            ...
            if dir_counter == 0 and starting_frame_id > 0:
                misc.log(
                    f"Loading coco annotations from existing chunk dir - {chunk_dir}"
                )
                coco_scene_output = _BopWriterUtility.load_json(
                    os.path.join(chunk_dir, "scene_gt_coco.json")
                )
                if coco_scene_output["annotations"]:
                    segmentation_id = (
                        coco_scene_output["annotations"][-1]["id"] + 1
                    )
                else:
                    segmentation_id = 1
            else:
            ...
            last_chunk_gt_info_fpath = os.path.join(
                chunk_dir, "scene_gt_info.json"
            )
            scene_gt_info = inout.load_json(
                last_chunk_gt_info_fpath, keys_to_int=True
            )
            # Output coco path
            ...
            for scene_view, inst_list in scene_gt.items():
                ...
                img_path = os.path.join(
                    chunk_dir, "rgb", "{im_id:06d}.jpg"
                ).format(im_id=im_id)
                relative_img_path = os.path.relpath(
                    img_path, os.path.dirname(coco_gt_path)
                )
                im_size = (
                    bpy.context.scene.render.resolution_x,
                    bpy.context.scene.render.resolution_y,
                ...
                for idx, inst in enumerate(inst_list):
                    ...
                    mask_visib_p = os.path.join(
                        chunk_dir, "mask_visib", "{im_id:06d}_{gt_id:06d}.png"
                    ).format(im_id=im_id, gt_id=idx)
                    mask_full_p = os.path.join(
                        chunk_dir, "mask", "{im_id:06d}_{gt_id:06d}.png"
                    ).format(im_id=im_id, gt_id=idx)

                    binary_inst_mask_visib = inout.load_depth(
                        mask_visib_p
                    ).astype(bool)
