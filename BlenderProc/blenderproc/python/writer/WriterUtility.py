...
def write_hdf5(
    ...
    for frame in range(
        ...
        hdf5_path = os.path.join(
            output_dir_path, str(frame + frame_offset) + ".hdf5"
        )
        with h5py.File(hdf5_path, "w") as file:
                ...
                if adjusted_frame < len(data_block):
                    ...
                    if stereo_separate_keys and (
                        ...
                        _WriterUtility.write_to_hdf_file(
                            file, key + "_0", data_block[adjusted_frame][0]
                        )
                        _WriterUtility.write_to_hdf_file(
                            file, key + "_1", data_block[adjusted_frame][1]
                        )
                    else:
                        _WriterUtility.write_to_hdf_file(
                            file, key, data_block[adjusted_frame]
                        )
                else:
                    raise Exception(
                        f"There are more frames {adjusted_frame} then there are blocks of information "
            ...
            if blender_proc_version is not None:
                _WriterUtility.write_to_hdf_file(
                    ...
                    np.bytes_(blender_proc_version),
                )


class _WriterUtility:
    @staticmethod
    def load_registered_outputs(
        keys: Set[str], keys_with_alpha_channel: Set[str] = None
        ...
        for reg_out in reg_outputs:
            if reg_out["key"] in keys:
                key_has_alpha_channel = (
                    keys_with_alpha_channel is not None
                    and reg_out["key"] in keys_with_alpha_channel
                )
                if "%" in reg_out["path"]:
                    # per frame outputs
                    for frame_id in range(
                        ...
                        if os.path.exists(output_path):
                            output_file = _WriterUtility.load_output_file(
                                output_path, key_has_alpha_channel
                            )
                        else:
                            # check for stereo files
                            output_paths = _WriterUtility.get_stereo_path_pair(
                                output_path
                            )
                            ...
                            output_file = np.array(
                                [
                                    _WriterUtility.load_output_file(
                                        path, key_has_alpha_channel
                                    )
                        ...
                        if (
                            "trim_redundant_channels" in reg_out
                            and reg_out["trim_redundant_channels"]
                        ):
                            output_file = trim_redundant_channels(output_file)
                        if (
                            "convert_to_depth" in reg_out
                            and reg_out["convert_to_depth"]
                        ):
                            output_file = dist2depth(output_file)
                        if (
                            "convert_to_distance" in reg_out
                            and reg_out["convert_to_distance"]
                        ):
                            output_file = depth2dist(output_file)

                        # semantic seg must be last
                        if (
                            "is_semantic_segmentation" in reg_out
                            and reg_out["is_semantic_segmentation"]
                            and "semantic_segmentation_mapping" in reg_out
                            and "semantic_segmentation_default_values"
                            in reg_out
                        ):
                            output_file = segmentation_mapping(
                                output_file,
                            ...
                            for key, output_info in output_file.items():
                                output_data_dict.setdefault(key, []).append(
                                    output_info
                                )
                        else:
                            output_data_dict.setdefault(
                                reg_out["key"], []
                            ).append(output_file)
                else:
                    # per run outputs
    ...
    def load_output_file(
        file_path: str, load_alpha_channel: bool = False, remove: bool = True
        ...
        if file_ending in ["exr", "png", "jpg"]:
            ...
            output = load_image(
                file_path, num_channels=3 + (1 if load_alpha_channel else 0)
            )
    ...
    def get_common_attribute(
        ...
        if local_frame_change != ["X", "Y", "Z"] and attribute_name in [
            ...
            print(
                "Warning: The local_frame_change parameter is at the moment only supported by "
        ...
        if attribute_name == "location":
            return change_coordinate_frame_of_point(
        ...
        if attribute_name == "matrix_world":
            ...
            matrix_world = (
                change_source_coordinate_frame_of_transformation_matrix(
                    Entity(item).get_local2world_mat(), local_frame_change
                )
            )
            matrix_world = (
                change_target_coordinate_frame_of_transformation_matrix(
    ...
    def get_cam_attribute(
        ...
        if attribute_name == "cam2world_matrix":
            return _WriterUtility.get_common_attribute(
                cam_ob, "matrix_world", local_frame_change, world_frame_change
            )
        return _WriterUtility.get_common_attribute(
            cam_ob, attribute_name, local_frame_change, world_frame_change
        )

    @staticmethod
    def get_light_attribute(
        ...
        return _WriterUtility.get_common_attribute(
            light, attribute_name, local_frame_change, world_frame_change
        )

    @staticmethod
    def _get_shapenet_attribute(
        shapenet_obj: bpy.types.Object,
        attribute_name: str,
        local_frame_change: Union[None, List[str]] = None,
        world_frame_change: Union[None, List[str]] = None,
    ):
        ...
        return _WriterUtility.get_common_attribute(
            shapenet_obj, attribute_name, local_frame_change, world_frame_change
        )

    @staticmethod
    def write_to_hdf_file(
        file,
        key: str,
