...
def write_gif_animation(
    ...
    _GifWriterUtility.write_to_gif(
        to_animate,
        output_dir_path,
...
class _GifWriterUtility:
    ...
    def cache_png(
        ...
        if not set(keys_to_use) <= set(output_data_dict.keys()):
            raise ValueError(
                "The keys_to_use list must be contained in the list of keys from the output_data_dict!"
            )
            ...
            if value.shape[0] == 2:
                ...
                for index, perspective in enumerate(["_L", "_R"]):
                    ...
                    for number, frame in enumerate(output_data_dict[key]):
                        ...
                        vis_data(
                            key=key, data=frame[index], save_to_file=file_path
                        )
            else:
                # non stereo images
                to_animate[key] = []
                for number, frame in enumerate(output_data_dict[key]):
                    file_path = os.path.join(
                        Utility.get_temporary_directory(), f"{number}_{key}.png"
                    )
                    to_animate[key].append(file_path)
                    vis_data(key=key, data=frame, save_to_file=file_path)
        return to_animate

    @staticmethod
    def look_for_existing_output(
        output_dir_path: str, append_to_existing_output: bool, name_ending: str
    ) -> int:
        """
    ...
    def write_to_gif(
        ...
        for key, frame_list in to_animate.items():
            ...
            gif_number = _GifWriterUtility.look_for_existing_output(
                output_dir_path,
                append_to_existing_output,
