...
def random_walk(
    ...
    window_size: int = 1,
    interval: Optional[List[np.ndarray]] = None,
    ...
    if distribution == "uniform":
        ...
        raise RuntimeError(
            f'Unknown distribution: {distribution}. Choose between "normal" and "uniform"'
        )

        # Sample random steps
    random_steps = (
        step_magnitude
        * np.random.choice([-1, 1], (total_length, dims))
        * dist_fun(total_length, dims) ** order
    )
    ...
    if interval is not None:
        ...
        cumulative_steps = (
            np.abs((cumulative_steps - left_bound + size) % (2 * size) - size)
            + left_bound
        )
    ...
    if window_size > 1:
        initial_padding = (
            np.ones((window_size - 1, dims)) * cumulative_steps[:1, :]
        )
        cumulative_steps_padded = np.vstack((initial_padding, cumulative_steps))
        for i in range(dims):
            cumulative_steps[:, i] = np.convolve(
                cumulative_steps_padded[:, i],
