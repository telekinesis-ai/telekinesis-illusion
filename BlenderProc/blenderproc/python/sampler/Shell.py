...
def shell(
    center: Union[Vector, np.ndarray, List[float]],
    radius_min: float,
    radius_max: float,
    ...
    assert -180 <= azimuth_min <= 180, (
        "azimuth_min must be in range [-180, 180]"
    )
    assert -180 <= azimuth_max <= 180, (
        "azimuth_max must be in range [-180, 180]"
    )
    assert -90 <= elevation_min <= 90, (
        "elevation_min must be in range [-90, 90]"
    )
    assert -90 <= elevation_min <= 90, (
        "elevation_max must be in range [-90, 90]"
    )
    assert azimuth_min < azimuth_max, (
        "azimuth_min must be smaller than azimuth_max"
    )
    ...
    if uniform_volume:
        radius = radius_min + (radius_max - radius_min) * np.cbrt(
            np.random.rand()
        )
        ...
        while not constr_fulfilled:
            ...
            elev_constraint = (
                np.deg2rad(elevation_min)
                < elevation
                < np.deg2rad(elevation_max)
            )
            azim_constraint = (
                np.deg2rad(azimuth_min) < azimuth < np.deg2rad(azimuth_max)
            )
            constr_fulfilled = elev_constraint and azim_constraint
    else:
        el_sampled = np.deg2rad(
            elevation_min + (elevation_max - elevation_min) * np.random.rand()
        )
        az_sampled = np.deg2rad(
            azimuth_min + (azimuth_max - azimuth_min) * np.random.rand()
        )
        # spherical to cartesian coordinates
        direction_vector = np.array(
            [
                np.sin(np.pi / 2 - el_sampled) * np.cos(az_sampled),
