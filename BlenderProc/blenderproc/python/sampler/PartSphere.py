...
def part_sphere(
    ...
    part_sphere_dir_vector: Optional[
        Union[Vector, np.ndarray, List[float]]
    ] = None,
) -> np.ndarray:
    """Samples a point from the surface or from the interior of solid sphere which is split in two parts.
    ...
    if part_sphere_dir_vector is None:
        ...
        part_sphere_dir_vector = np.array(part_sphere_dir_vector).astype(
            np.float32
        )
    ...
    if dist_above_center >= radius:
        raise ValueError(
            "The dist_above_center value is bigger or as big as the radius!"
        )
