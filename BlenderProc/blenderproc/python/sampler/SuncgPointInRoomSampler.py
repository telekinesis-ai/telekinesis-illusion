...
class SuncgPointInRoomSampler:
    ...
    def __init__(self, suncg_objects: List[MeshObject]):
        ...
        for room_obj in suncg_objects:
            ...
            if (
                room_obj.has_cp("suncg_type")
                and room_obj.get_cp("suncg_type") == "Room"
                and room_obj.has_cp("bbox")
            ):
    ...
    def sample(
        self, height: float, max_tries: int = 1000
        ...
        for _ in range(max_tries):
            ...
            point = np.array(
                [
                    random.uniform(
                        room_obj.get_cp("bbox")["min"][0],
                        room_obj.get_cp("bbox")["max"][0],
                    ),
                    random.uniform(
                        room_obj.get_cp("bbox")["min"][1],
                        room_obj.get_cp("bbox")["max"][1],
                    ),
        ...
        raise Exception(
            "Cannot sample any point inside the loaded suncg rooms."
        )

    def _find_floor(
        self, suncg_objects: List[MeshObject], room_obj: MeshObject
    ) -> Optional[MeshObject]:
