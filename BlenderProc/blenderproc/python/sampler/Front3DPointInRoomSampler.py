...
class Front3DPointInRoomSampler:
    ...
    def __init__(
        self,
        front3d_objects: List[MeshObject],
        amount_of_objects_needed_per_room: int = 2,
    ):
        ...
        """
        front3d_objects = [
            obj for obj in front3d_objects if obj.has_cp("is_3D_future")
        ]

        floor_objs = [
            obj
            for obj in front3d_objects
            if obj.get_name().lower().startswith("floor")
        ]
        ...
        for obj in front3d_objects:
            ...
            for floor_obj in floor_objs:
                is_above = floor_obj.position_is_above_object(
                    obj.get_location()
                )
                if is_above:
        ...
        self.used_floors = [
            ...
            if floor_obj_counters[obj.get_name()]
    ...
    def sample(self, height: float, max_tries: int = 1000) -> np.ndarray:
        ...
        raise RuntimeError(
            "Cannot sample any point inside the loaded front3d rooms."
        )
