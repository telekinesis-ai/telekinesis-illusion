...
def disk(
    ...
    if sample_from in ["arc", "sector"]:
        ...
        if not all(
            [start_angle < end_angle, abs(start_angle - end_angle) <= 180]
        ):
            raise Exception(
                "Sector's/arch's start and end points are defined wrong! Boundaries to comply with:"
                "1. start_angle < end_angle; 2. abs(start_angle - end_angle) <= 180."
            )
        # transform to 2d vectors
        start_vec = [
            np.cos(np.deg2rad(start_angle)),
            np.sin(np.deg2rad(start_angle)),
        ]
    ...
    if sample_from in ["arc", "sector"]:
        while not all(
            [
                not _Disk.is_clockwise(start_vec, sampled_point),
                _Disk.is_clockwise(end_vec, sampled_point),
            ]
        ):
            sampled_point = _Disk.sample_point(magnitude)
...
class _Disk:
    ...
    def sample_point(magnitude: float) -> np.ndarray:
        ...
        sampled_point = np.append(
            list(map(lambda x: magnitude * x / norm, direction)), 0
        )
    ...
    @staticmethod
    def is_clockwise(
        rel_point: Union[Vector, np.ndarray, List[float]],
        sampled_point: Union[Vector, np.ndarray, List[float]],
    ) -> bool:
        ...
        """
        return (
            -rel_point[0] * sampled_point[1] + rel_point[1] * sampled_point[0]
        ) > 0
