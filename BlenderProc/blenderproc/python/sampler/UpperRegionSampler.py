...
def upper_region(
    objects_to_sample_on: Union[MeshObject, List[MeshObject]],
    face_sample_range: Optional[Union[Vector, np.ndarray, List[float]]] = None,
    ...
    for obj in objects_to_sample_on:
        ...
        if selected_face is not None:
            ...
            raise RuntimeError(
                f"Couldn't find a face, for this obj: {obj.get_name()}"
            )

    if regions and len(regions) == len(objects_to_sample_on):
        selected_region_id = random.randint(0, len(regions) - 1)
        selected_region, obj = (
            regions[selected_region_id],
            objects_to_sample_on[selected_region_id],
        )
        ...
        while True:
            ...
            if use_ray_trace_check:
                ...
                c_dir = inv_world_matrix @ np.concatenate(
                    (dir_val * -1.0, [0]), 0
                )
        ...
        return np.array(ret)
    raise RuntimeError(
        "The amount of regions is either zero or does not match the amount of objects!"
    )
