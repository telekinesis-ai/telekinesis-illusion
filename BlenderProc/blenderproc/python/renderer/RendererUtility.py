...
def set_denoiser(denoiser: Optional[str], denoiser_configs: Optional[dict]):
    ...
    if denoiser is None:
        ...
        bpy.context.scene.cycles.denoising_input_passes = denoiser_configs[
            "denoising_input_passes"
        ]
        bpy.context.scene.cycles.denoising_prefilter = denoiser_configs[
            "denoising_prefilter"
        ]
        bpy.context.scene.cycles.denoising_quality = denoiser_configs[
            "denoising_quality"
        ]
        bpy.context.scene.cycles.denoising_use_gpu = denoiser_configs[
            "denoising_use_gpu"
        ]
        ...
        links.new(
            render_layer_node.outputs["DiffCol"], denoise_node.inputs["Albedo"]
        )
        links.new(
            render_layer_node.outputs["Normal"], denoise_node.inputs["Normal"]
        )
    else:
...
def set_simplify_subdivision_render(
    simplify_subdivision_render: int, texture_limit_render: str
):
    ...
    if simplify_subdivision_render > 0:
        ...
        bpy.context.scene.cycles.texture_limit_render = texture_limit_render
    else:
...
def enable_normals_output(
    ...
    links.new(
        render_layer_node.outputs["Normal"], separate_rgba.inputs["Image"]
    )
    ...
    for row_index, channel in enumerate(c_channels):
        ...
        for column in range(3):
            ...
            multiply.inputs[
                1
            ].default_value = 0  # setting at the end for all frames
    ...
    for frame in range(
        ...
        used_rotation_matrix = (
            CameraUtility.get_camera_pose(frame) @ rot_around_x_axis
        )
        for row_index in range(3):
            for column_index in range(3):
                current_multiply = multiplication_values[row_index][
                    column_index
                ]
...
def _progress_bar_thread(
    ...
    with Progress(
        *columns, console=Console(file=stdout), transient=True
    ) as progress:
        complete_task = progress.add_task(
            "[green]Total", total=total_frames, status=""
        )
        ...
        while True:
            ...
            if char == "\n":
                ...
                if current_line.startswith("Fra:"):
                    ...
                    status_columns = [
                        col.strip() for col in current_line.split("|")
                    ]
                    if "Scene, ViewLayer" in status_columns:
                        ...
                        status = " | ".join(
                            status_columns[
                                status_columns.index("Compositing") :
                            ]
                        )
...
def render(
    ...
    if bpy.context.scene.frame_end != bpy.context.scene.frame_start:
        ...
        if load_keys:
            ...
            logger.info(
                f"Rendering {total_frames} frames of {', '.join(keys_to_render)}..."
            )
    ...
    return (
        _WriterUtility.load_registered_outputs(
            load_keys, keys_with_alpha_channel
        )
        if return_data
        else {}
    )
                ...
                for i, device in enumerate(devices):
                    ...
                    if desired_gpu_ids is None or i in desired_gpu_ids:
                        if device.type != "CPU":
                            ...
                            device.use = True
                            found = True
                        if not use_only_gpu and device.type == "CPU":
                            print(
                                f"Device {device.name} of type {device.type} found and used."
                            )
                            device.use = True
...
def set_tiling(use_auto_tile: bool, tile_size: int):
    """
    Set tile size for the cycles renderer.
    """
    bpy.context.scene.cycles.use_auto_tile = use_auto_tile
