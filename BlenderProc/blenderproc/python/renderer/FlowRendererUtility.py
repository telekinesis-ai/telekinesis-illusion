...
def render_optical_flow(
    output_dir: str = None,
    temp_dir: str = None,
    get_forward_flow: bool = True,
    get_backward_flow: bool = True,
    ...
    if get_forward_flow is False and get_backward_flow is False:
        raise RuntimeError(
            "Take the FlowRenderer Module out of the config if both forward and "
    ...
    with UndoAfterExecution():
        ...
        _FlowRendererUtility.output_vector_field(
            get_forward_flow, get_backward_flow, output_dir
        )
        ...
        print(
            f"Rendering {bpy.context.scene.frame_end - bpy.context.scene.frame_start} frames of optical flow..."
        )
        RendererUtility.render(
            temp_dir,
            "img_flow_temp_ignore_me_",
            None,
            load_keys=set(),
            verbose=verbose,
        )
        ...
        for frame in range(
            ...
            if get_forward_flow:
                file_path = (
                    temporary_fwd_flow_file_path + f"{frame:04d}" + ".exr"
                )
                fwd_flow_field = load_image(file_path, num_channels=4).astype(
                    np.float32
                )
                ...
                file_name = (
                    os.path.join(output_dir, forward_flow_output_file_prefix)
                    + f"{frame:04d}"
                )
                forward_flow = (
                    fwd_flow_field * -1
                )  # invert forward flow to point at next frame
                np.save(file_name + ".npy", forward_flow[:, :, :2])

            if get_backward_flow:
                file_path = (
                    temporary_bwd_flow_file_path + f"{frame:04d}" + ".exr"
                )
                bwd_flow_field = load_image(file_path, num_channels=4).astype(
                    np.float32
                )
                ...
                file_name = (
                    os.path.join(output_dir, backward_flow_output_file_prefix)
                    + f"{frame:04d}"
                )
    ...
    return (
        _WriterUtility.load_registered_outputs(load_keys) if return_data else {}
    )


class _FlowRendererUtility:
    @staticmethod
    def output_vector_field(
        forward_flow: bool, backward_flow: bool, output_dir: str
    ):
        ...
        links.new(
            render_layer_node.outputs["Vector"], separate_rgba.inputs["Image"]
        )
        ...
        if forward_flow:
            ...
            links.new(
                combine_fwd_flow.outputs["Image"],
                fwd_flow_output_file.inputs["Image"],
            )
        ...
        if backward_flow:
            ...
            links.new(
                combine_bwd_flow.outputs["Image"],
                bwd_flow_output_file.inputs["Image"],
            )
