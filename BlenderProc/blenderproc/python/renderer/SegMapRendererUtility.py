...
def render_segmap(
    ...
    segcolormap_output_file_prefix: str = "instance_attribute_map_",
    segcolormap_output_key: str = "segcolormap",
    ...
    with UndoAfterExecution():
        ...
        result = _colorize_objects_for_instance_segmentation(
            objs_with_mats,
            use_alpha_channel,
        ...
        if isinstance(attributes, str):
            ...
            raise RuntimeError(
                f"The type of this is not supported here: {attributes}"
            )
        ...
        for frame in range(
            ...
            for suffix in suffixes:
                ...
                for channel_id in range(result_channels):
                    ...
                    resulting_map = np.zeros(
                        (segmap.shape[0], segmap.shape[1]), dtype=optimal_dtype
                    )
                    was_used = False
                    ...
                    if current_attribute == "instance":
                        ...
                        if (
                            current_attribute in default_values
                            or attribute in default_values
                        ):
                        ...
                        for object_id in object_ids:
                            ...
                            # if the current object has a custom property with that name -> get it
                                ...
                                current_attribute.startswith("cp_")
                                and attribute in current_obj
                            ):
                                ...
                                raise RuntimeError(
                                    f"The obj: {current_obj.name} does not have the "
                            ...
                            if object_id in save_in_csv_attributes:
                                save_in_csv_attributes[object_id][attribute] = (
                                    value
                                )
                            else:
                    ...
                    if was_used and num_default_values < len(object_ids):
                        ...
                        return_dict.setdefault(
                            f"{org_attribute}_segmaps{suffix}", []
            ...
            if there_was_an_instance_rendering:
                ...
                return_dict.setdefault("instance_attribute_maps", []).append(
                    mappings
                )
                ...
                csv_file_path = os.path.join(
                    output_dir,
                    segcolormap_output_file_prefix + f"{frame:04d}.csv",
                )
                with open(
                    ...
                    for (
                        obj_idx,
                        object_element,
                    ) in save_in_csv_attributes.items():
                ...
                if len(list_of_attributes) > 0:
                    raise RuntimeError(
                        f"There were attributes specified in the may_by, which could not be saved as "
    ...
    Utility.register_output(
        output_dir, file_prefix, output_key, ".npy", "2.0.0"
    )
...
def _colorize_object(
    obj: bpy.types.Object, color: mathutils.Vector, use_alpha_channel: bool
):
    ...
    if len(obj.material_slots) > 0:
        for i, material_slot in enumerate(obj.material_slots):
            if use_alpha_channel:
                obj.data.materials[i] = (
                    MaterialLoaderUtility.add_alpha_texture_node(
