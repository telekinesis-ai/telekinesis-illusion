...
def render_nocs(
    ...
    with UndoAfterExecution():
        ...
        RendererUtility.set_output_format(
            "OPEN_EXR", 16, enable_transparency=True
        )
        # Render and ret
        return RendererUtility.render(
            output_dir,
            file_prefix,
            output_key,
            load_keys={output_key},
            return_data=return_data,
            keys_with_alpha_channel={output_key},
            verbose=verbose,
        )
...
class _NOCSRendererUtility:
    ...
    def create_nocs_material() -> Material:
        ...
        nocs_material.link(
            tex_coords_node.outputs["Object"], scale_node.inputs[0]
        )
        ...
        nocs_material.link(
            add_node.outputs["Vector"], output_node.inputs["Surface"]
        )
