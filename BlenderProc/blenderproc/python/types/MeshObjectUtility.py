...
from blenderproc.python.utility.BlenderUtility import (
...
class MeshObject(Entity):
    ...
    def get_materials(self) -> List[Optional[Material]]:
        ...
        """
        return MaterialLoaderUtility.convert_to_materials(
            self.blender_obj.data.materials
        )
    ...
    def move_origin_to_bottom_mean_point(self):
        ...
        bpy.ops.transform.translate(
            value=[-bb_center[0], -bb_center[1], -bb_min_z_value]
        )
    ...
    def get_bound_box(self, local_coords: bool = False) -> np.ndarray:
        ...
        if not local_coords:
            ...
            return np.array(
                [
                    local2world @ Vector(cord)
                    for cord in self.blender_obj.bound_box
                ]
            )
        return np.array([Vector(cord) for cord in self.blender_obj.bound_box])

    def persist_transformation_into_mesh(
        self, location: bool = True, rotation: bool = True, scale: bool = True
    ):
        """
        ...
        with bpy.context.temp_override(
            selected_editable_objects=[self.blender_obj]
        ):
    ...
    def set_origin(
        self, point: Union[list, np.ndarray, Vector] = None, mode: str = "POINT"
    ) -> np.ndarray:
        ...
        with bpy.context.temp_override(
            ...
            if mode == "POINT":
                if point is None:
                    raise Exception(
                        "The parameter point is not given even though the mode is set to POINT."
                    )
    ...
    def disable_rigidbody(self):
        ...
        if self.has_rigidbody_enabled():
            ...
            warnings.warn(
                f"MeshObject {self.get_name()} has no rigid_body component enabled"
            )
    ...
    def mesh_as_bmesh(self, return_copy=False) -> bmesh.types.BMesh:
        ...
        if return_copy:
            ...
            if bpy.context.mode != "EDIT_MESH":
                raise Exception(
                    f"The object: {self.get_name()} is not in EDIT mode before calling mesh_as_bmesh()"
                )
    ...
    def join_with_other_objects(self, objects: List["MeshObject"]):
        ...
        context["selected_objects"] = context["selected_editable_objects"] = [
            obj.blender_obj for obj in objects
        ] + [self.blender_obj]
    ...
    def position_is_above_object(
        ...
        if check_no_objects_in_between:
            ...
            hit, _, _, _, hit_object, _ = scene_ray_cast(
                position, down_direction
            )
        ...
        hit, _, _, _ = self.blender_obj.ray_cast(
            world2local @ Vector(position),
    ...
    def ray_cast(
        ...
        """
        result, location, normal, index = self.blender_obj.ray_cast(
            Vector(origin), Vector(direction), distance=max_distance
        )
    ...
    def add_uv_mapping(self, projection: str, overwrite: bool = False):
        ...
        if not self.has_uv_mapping() or overwrite:
            ...
            if projection == "cube":
                ...
                raise RuntimeError(
                    f"Unknown projection: '{projection}'. Please use 'cube', 'cylinder', "
                    f"'smart' or 'sphere'."
                )
            self.object_mode()
    ...
    def has_uv_mapping(self) -> bool:
        ...
        if len(self.blender_obj.data.uv_layers) > 1:
            raise Exception(
                "This only support objects which only have one uv layer."
            )
    ...
    def scale_uv_coordinates(self, factor: float):
        ...
        if not self.has_uv_mapping():
            raise Exception(
                "Cannot scale UV coordinates of a MeshObject that has no UV mapping."
            )
    ...
    def add_displace_modifier(
        ...
        return self.add_modifier(
            "DISPLACE", texture=texture, mid_level=mid_level, strength=strength
        )
    ...
    def get_modifiers(
        self,
    ) -> Dict[str, bpy.types.Modifier] | List[bpy.types.Modifier]:
    ...
    def add_auto_smooth_modifier(
        self, angle: float = 30.0
    ) -> bpy.types.Modifier:
        ...
        if not path.exists():
            raise RuntimeError(
                f"Could not find the path to the 'ESSENTIALS' asset folder expected at {path}"
            )
        ...
        existing_node_group = bpy.data.node_groups.get(
            smooth_by_angle_node_group_name, None
        )
        ...
        for existing_mod in self.get_modifiers():
            if (
                existing_mod.type == "NODES"
                and existing_mod.node_group == existing_node_group
            ):
                modifier = modifier
                break
        ...
        if modifier is None:
            modifier = self.blender_obj.modifiers.new(
                name=smooth_by_angle_node_group_name, type="NODES"
            )
    ...
    def mesh_as_trimesh(self) -> Trimesh:
        ...
        if not all(
            len(f.vertices[:]) == len(mesh.polygons[0].vertices[:])
...
def create_from_blender_mesh(
    blender_mesh: bpy.types.Mesh, object_name: str = None
) -> MeshObject:
    ...
    obj = bpy.data.objects.new(
        blender_mesh.name if object_name is None else object_name, blender_mesh
    )
...
def create_with_empty_mesh(
    object_name: str, mesh_name: str = None
) -> MeshObject:
...
def create_from_point_cloud(
    ...
    point_color: Tuple[float, float, float] = (1, 0, 0),
) -> MeshObject:
    ...
    if add_geometry_nodes_visualization:
        ...
        mesh_to_points_node = geometry_nodes.nodes.new(
        ...
        # Link nodes
        input_node = Utility.get_the_one_node_with_type(
            geometry_nodes.nodes, "NodeGroupInput"
        )
        output_node = Utility.get_the_one_node_with_type(
            geometry_nodes.nodes, "NodeGroupOutput"
        )
        geometry_nodes.links.new(
            input_node.outputs["Geometry"], mesh_to_points_node.inputs["Mesh"]
        )
        geometry_nodes.links.new(
            mesh_to_points_node.outputs["Points"],
            material_node.inputs["Geometry"],
        )
        geometry_nodes.links.new(
            material_node.outputs["Geometry"], output_node.inputs["Geometry"]
        )
...
def scene_ray_cast(
    ...
    direction: Union[Vector, list, np.ndarray],
    max_distance: float = 1.70141e38,
) -> Tuple[bool, np.ndarray, np.ndarray, int, MeshObject, np.ndarray]:
    ...
    :return: Whether the ray successfully hit any geometry
             ...
             Some 4x4 matrix.
    """
    hit, location, normal, index, hit_object, matrix = (
