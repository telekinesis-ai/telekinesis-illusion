...
def set_rotation_constraint(
    ...
    constraint = add_constraint_if_not_existing(
        bone, constraint_name="Limit Rotation"
    )
...
def set_ik_limits_from_rotation_constraint(
    bone: bpy.types.PoseBone, constraint: Optional[bpy.types.Constraint] = None
):
...
def copy_constraints(
    ...
    for c in source_bone.constraints:
        if (
            constraints_to_be_copied is not None
            and c.name not in constraints_to_be_copied
        ):
            continue
        c_copy = add_constraint_if_not_existing(
            target_bone, constraint_name=c.name
        )
        for prop in dir(c):
...
def set_copy_rotation_constraint(
    bone: bpy.types.PoseBone,
    ...
    :param influence: Influence of the constraint.
    """
    c = add_constraint_if_not_existing(
        bone,
...
def set_copy_location_constraint(
    bone: bpy.types.PoseBone,
    target: bpy.types.Armature,
    target_bone: str,
    custom_constraint_name: Optional[str] = None,
...
def set_location_constraint(
    ...
    # add new constraint if it doesn't exist
    constraint = add_constraint_if_not_existing(
        bone, constraint_name="Limit Location"
    )
...
def get_constraint(
    bone: bpy.types.PoseBone, constraint_name: str = ""
) -> Optional[bpy.types.Constraint]:
