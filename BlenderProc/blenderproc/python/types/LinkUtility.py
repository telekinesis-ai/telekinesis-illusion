...
class Link(Entity):
    ...
    def _set_rotation_euler(
        ...
        if mode == "absolute":
            if isinstance(rotation_euler, float):
                ...
                rotation_euler = self._clip_value_from_constraint(
                    bone=bone,
                    value=rotation_euler,
                    constraint_name="Limit Rotation",
                    axis=axis,
                )
                current_rotation_euler = bone.rotation_euler
                current_rotation_euler[["X", "Y", "Z"].index(axis)] = (
                    rotation_euler
                )
                bone.rotation_euler = current_rotation_euler
                print(
                    f"Set rotation_euler of bone {bone.name} to {rotation_euler}"
                )
            else:
                bone.rotation_euler = Vector(
                    [
                        self._clip_value_from_constraint(
                            bone=bone,
                            value=rot_euler,
                            constraint_name="Limit Rotation",
                            axis=axis,
                        )
                        for rot_euler, axis in zip(
            ...
            if isinstance(rotation_euler, float):
                ...
                if axis is not None:
                    ...
                    for axis in ["X", "Y", "Z"]:
                        bone.rotation_euler.rotate_axis(axis, rotation_euler)
                print(
                    f"Relatively rotated bone {bone.name} around axis {axis} for {rotation_euler} radians"
                )
            else:
                for axis, rotation in zip(["X", "Y", "Z"], rotation_euler):
                    bone.rotation_euler.rotate_axis(axis, rotation)
                print(
                    f"Relatively rotated {bone.name} for {rotation_euler} radians"
                )

    def set_rotation_euler(self, *args, **kwargs):
        raise NotImplementedError(
            "Please use 'set_rotation_euler_fk()' or set_rotation_euler_ik()'!"
        )
    ...
    def set_location_ik(self, location: Union[List[float], np.array, Vector]):
        ...
        assert self.ik_bone_controller is not None, (
            "No ik bone chain created. Please run "
            "'urdf_object.create_ik_bone_controller()' first!"
        )

        # first: determine offset to base frame
        self.ik_bone_controller.location = Vector([0.0, 0.0, 0.0])
        self.ik_bone_controller.rotation_mode = "XYZ"
        self.ik_bone_controller.rotation_euler = Vector([0.0, 0.0, 0.0])
    ...
    def _determine_rotation_axis(
        ...
        if c is None:
            print(
                "WARNING: No rotation constraint set. Will rotate all axes relatively!"
            )
            return None
        ...
        assert len(axes) == 1, (
            f"Constraints are set wrong for a rotation around a single axis. Only one axis should "
    ...
    def _clip_value_from_constraint(
        self,
        bone: bpy.types.PoseBone,
        value: float = 0,
        constraint_name: str = "",
    ...
    def get_visual_local2world_mats(
        self, parent2world_matrix: Optional[Matrix] = None
        ...
        if parent2world_matrix is None:
            parent2world_matrix = Matrix(
                Entity(self.armature).get_local2world_mat()
            )
        ...
        if self.visuals:
            link2base_mats = [
                bone_mat
                @ (self.link2bone_mat.inverted() @ visual_local2link_mat)
                for visual_local2link_mat in self.visual_local2link_mats
            ]
            return [parent2world_matrix @ mat for mat in link2base_mats]
        return None

    def get_collision_local2world_mats(
        self, parent2world_matrix: Optional[Matrix] = None
    ) -> Optional[List[Matrix]]:
        ...
        if self.collisions:
            link2base_mats = [
                bone_mat
                @ (self.link2bone_mat.inverted() @ collision_local2link_mat)
                for collision_local2link_mat in self.collision_local2link_mats
            ]
            return [parent2world_matrix @ mat for mat in link2base_mats]
        return None

    def get_inertial_local2world_mat(
        self, parent2world_matrix: Optional[Matrix] = None
    ) -> Optional[Matrix]:
        ...
        if self.inertial is not None:
            link2base_mat = bone_mat @ (
                self.link2bone_mat.inverted() @ self.inertial_local2link_mat
            )
    ...
    def get_all_objs(self):
        ...
        return (
            self.visuals
            + self.collisions
            + ([self.inertial] if self.inertial is not None else [])
        )
    ...
    def parent_with_bone(self, weight_distribution="rigid"):
        ...
        if self.bone is None:
            # only set parent
            print(
                f"WARNING: Link {self.get_name()} does not have a bone to be parented with (usually because it's the "
                f"first link)!"
            )
    ...
    def create_ik_bone_controller(
        ...
        chain_length: int = 0,
    ) -> Tuple[bpy.types.PoseBone, bpy.types.PoseBone, Matrix]:
        ...
        ik_bone_controller.parent = edit_bones[self.bone.name].parent_recursive[
            -1
        ]
        ...
        ik_bone_constraint.head = ik_bone_constraint.tail - (
            self.ik_bone.tail - self.ik_bone.head
        )
        # we need to re-parent the pre- and successor of the constraint bone
        ik_bone_constraint.parent = edit_bones[
            self.ik_bone.name
        ].parent_recursive[0]
        edit_bones[self.ik_bone.name].parent = edit_bones[
            ik_bone_constraint.name
        ]

        # add the bones to the link
        bpy.ops.object.mode_set(mode="POSE")
        self.set_ik_bone_constraint(
            self.armature.pose.bones.get(self.ik_bone.name + ".constraint")
        )
        self.set_ik_bone_controller(
            self.armature.pose.bones.get(self.ik_bone.name + ".controller")
        )

        # add ik constraint
        set_ik_constraint(
            self.ik_bone_constraint,
            self.armature,
            self.ik_bone_controller.name,
        ...
        bpy.context.view_layer.update()
        offset = (
            self.ik_bone_constraint.matrix.inverted()
            @ self.ik_bone_controller.matrix
        )
    ...
    def switch_fk_ik_mode(self, mode: str = "fk", keep_pose: bool = True):
        ...
        if (
            mode == "fk"
        ):  # turn off copy rotation constraints of fk bone and base bone
            if self.get_fk_ik_mode() == "fk":
                return None
            bpy.context.view_layer.update()
            ...
            if self.joint_type == "revolute":
                self.bone.constraints["copy_rotation.fk"].influence = 1.0
                self.bone.constraints["copy_rotation.ik"].influence = 0.0

            if "copy_rotation.ik" in self.bone.constraints.keys():
                self.bone.constraints[
                    "copy_rotation.ik"
                ].influence = 0.0  # otherwise skip

            if self.ik_bone_controller is not None:  # switch off ik constraint
                self.ik_bone_constraint.constraints["IK"].influence = 0.0
            ...
            if self.joint_type == "revolute":
                self.bone.constraints["copy_rotation.fk"].influence = 0.0
                self.bone.constraints["copy_rotation.ik"].influence = 1.0
            ...
            if self.ik_bone_constraint is not None:
                self.ik_bone_constraint.constraints["IK"].influence = 1.0
                fk_bone_mat = np.array(self.fk_bone.matrix)
                fk_bone_mat[:3, -1] = np.array(self.ik_bone_controller.matrix)[
                    :3, -1
                ]
