...
class Entity(Struct):
    ...
    def set_rotation_mat(
        self,
        rotation_mat: Union[Matrix, np.ndarray],
        frame: Optional[int] = None,
    ):
    ...
    def get_rotation(self, frame: Optional[int] = None) -> np.ndarray:
        ...
        """
        warnings.warn(
            "This function will be deprecated. Use get_rotation_euler() instead."
        )
    ...
    def apply_T(self, transform: Union[np.ndarray, Matrix]):
        ...
        """
        self.blender_obj.matrix_world = Matrix(
            self.get_local2world_mat()
        ) @ Matrix(transform)
    ...
    def duplicate(
        self, duplicate_children: bool = True, linked: bool = False
    ) -> "Entity":
        ...
        if type(duplicate_obj) != type(self):
            warnings.warn(
                f"Duplication is only partly supported for {type(self)}"
            )

        if duplicate_children:
            for child in self.get_children():
                duplicate_child = child.duplicate(
                    duplicate_children=duplicate_children, linked=linked
                )
                duplicate_child.set_parent(duplicate_obj)

                duplicate_child.blender_obj.matrix_basis = (
                    child.blender_obj.matrix_basis.copy()
                )
                duplicate_child.blender_obj.matrix_parent_inverse = (
                    child.blender_obj.matrix_parent_inverse.copy()
                )
    ...
    def set_parent(self, new_parent: "Entity"):
        ...
        self.blender_obj.matrix_parent_inverse = Matrix(
            new_parent.get_local2world_mat()
        ).inverted()
    ...
    def get_parent(self) -> Optional["Entity"]:
        ...
        """
        return (
            convert_to_entity_subclass(self.blender_obj.parent)
            if self.blender_obj.parent is not None
            else None
        )

    def get_children(
        self, return_all_offspring: bool = False
    ) -> List["Entity"]:
        ...
        def collect_offspring(
            entity: bpy.types.Object,
    ...
    def delete(self, remove_all_offspring: bool = False):
        ...
        if remove_all_offspring:
            selected_objects.extend(
                self.get_children(return_all_offspring=True)
            )
        with bpy.context.temp_override(
            selected_objects=[e.blender_obj for e in selected_objects]
        ):
    ...
    def hide(self, hide_object: bool = True):
        ...
        self.blender_obj.hide_set(hide_object)
    ...
    def __setattr__(self, key, value):
        if key != "blender_obj":
            raise RuntimeError(
                "The entity class does not allow setting any attribute. Use the corresponding "
...
def create_empty(entity_name: str, empty_type: str = "plain_axes") -> "Entity":
    ...
    if empty_type.lower() in [
        ...
        raise RuntimeError(
            f'Unknown basic empty type "{empty_type}"! Available types: "plain_axes".'
        )
...
def convert_to_entities(
    blender_objects: list, convert_to_subclasses: bool = False
) -> List["Entity"]:
...
def delete_multiple(
    entities: List[Union["Entity"]], remove_all_offspring: bool = False
):
    ...
    if remove_all_offspring:
        ...
        with bpy.context.temp_override(
            selected_objects=[e.blender_obj for e in all_nodes]
        ):
            bpy.ops.object.delete()
    else:
        with bpy.context.temp_override(
            selected_objects=[e.blender_obj for e in entities]
        ):
            bpy.ops.object.delete()
