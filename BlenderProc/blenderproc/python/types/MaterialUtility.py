...
class Material(Struct):
    ...
    def __init__(self, material: bpy.types.Material):
        ...
        if not material.use_nodes:
            raise RuntimeError(
                f"The given material {material.name} does not have nodes enabled and can "
    ...
    def get_the_one_node_with_type(
        self, node_type: str, created_in_func: str = ""
    ) -> bpy.types.Node:
        ...
        :return: The node.
        """
        return Utility.get_the_one_node_with_type(
            self.nodes, node_type, created_in_func
        )
    ...
    def get_nodes_with_type(
        ...
        """
        return Utility.get_nodes_with_type(
            self.nodes, node_type, created_in_func
        )

    def get_nodes_created_in_func(
        self, created_in_func: str
    ) -> List[bpy.types.Node]:
        """Returns all nodes which are of the given node_type
    ...
    def map_vertex_color(
        self, layer_name: str = "Col", active_shading: bool = True
    ):
        ...
        if active_shading:
            ...
            self.links.new(
                attr_node.outputs["Color"], principled_bsdf.inputs["Base Color"]
            )
        else:
            # create new vertex color shade node
            vcol = self.nodes.new(type="ShaderNodeVertexColor")
            vcol.layer_name = layer_name
            result = Utility.get_node_connected_to_the_output_and_unlink_it(
                self.blender_obj
            )
            ...
            if "Color" in background_color_node.inputs:
                self.links.new(
                    vcol.outputs["Color"], background_color_node.inputs["Color"]
                )
                self.links.new(
                    background_color_node.outputs["Background"],
                    material_output.inputs["Surface"],
                )
            else:
                raise RuntimeError(
                    f"Material '{self.blender_obj.name}' has no node connected to the output, "
    ...
    def make_emissive(
        ...
        if not replace:
            mix_node = self.new_node(
                "ShaderNodeMixShader", self.make_emissive.__name__
            )
            if non_emissive_color_socket is None:
                principled_bsdf = self.get_the_one_node_with_type(
                    "BsdfPrincipled"
                )
                non_emissive_color_socket = principled_bsdf.outputs["BSDF"]
            self.insert_node_instead_existing_link(
                non_emissive_color_socket,
                mix_node.inputs[2],
            ...
            light_path_node = self.new_node(
                "ShaderNodeLightPath", self.make_emissive.__name__
            )
            self.link(
                light_path_node.outputs["Is Camera Ray"], mix_node.inputs["Fac"]
            )
            output_socket = mix_node.inputs[1]
        else:
            output_socket = output_node.inputs["Surface"]

        emission_node = self.new_node(
            "ShaderNodeEmission", self.make_emissive.__name__
        )
        ...
        if emission_color is None:
            ...
            if len(principled_bsdf.inputs["Base Color"].links) == 1:
                ...
                socket_connected_to_the_base_color = (
                    principled_bsdf.inputs["Base Color"].links[0].from_socket
                )
                self.link(
                    socket_connected_to_the_base_color,
    ...
    def set_principled_shader_value(
        self,
        input_name: str,
        value: Union[float, bpy.types.Image, bpy.types.NodeSocket],
    ):
    ...
    def get_principled_shader_value(
        self, input_name: str
    ) -> Union[float, bpy.types.NodeSocket]:
        """
        ...
        if input_name in principled_bsdf.inputs:
            ...
            if principled_bsdf.inputs[input_name].links:
                if len(principled_bsdf.inputs[input_name].links) == 1:
                    ...
                    return (
                        principled_bsdf.inputs[input_name].links[0].from_socket
                    )
                raise RuntimeError(
                    f"The input socket has more than one input link: "
        ...
        raise RuntimeError(
            f"The input name could not be found in the inputs: {input_name}"
        )
    ...
    def infuse_texture(
        ...
        if used_mode not in ["overlay", "mix", "set"]:
            raise Exception(
                f'This mode is unknown here: {used_mode}, only ["overlay", "mix", "set"]!'
            )
        ...
        if used_connector not in principled_bsdf.inputs:
            raise Exception(
                f"The {used_connector} is not an input to Principled BSDF!"
            )
        ...
        for link in principled_bsdf.inputs[used_connector].links:
            ...
            self.links.remove(link)
        if (
            node_socket_connected_to_the_connector is not None
            or used_mode == "set"
        ):
            ...
            self.link(
                texture_coords.outputs["UV"], mapping_node.inputs["Vector"]
            )
            self.link(
            ...
            if (
                ...
                self.link(
                    node_socket_connected_to_the_connector,
                    mix_node.inputs["Color1"],
                )
                self.link(
                    mix_node.outputs["Color"],
                    principled_bsdf.inputs[used_connector],
                )
            elif used_mode == "set":
                self.link(
                    texture_node_output, principled_bsdf.inputs[used_connector]
                )

    def infuse_material(
        self, material: "Material", mode: str = "mix", mix_strength: float = 0.5
    ):
        """
        ...
        if used_mode not in ["add", "mix"]:
            raise Exception(
                f'This mode is unknown here: {used_mode}, only ["mix", "add"]!'
            )

        # move the copied material inside of a group
        group_node = self.new_node("ShaderNodeGroup")
        group = BlenderUtility.add_nodes_to_group(
            material.nodes, f"{used_mode.title()}_{material.get_name()}"
        )
        ...
        for mat_output_input in material_output.inputs:
            if len(mat_output_input.links) > 0:
                if (
                    "Float" in mat_output_input.bl_idname
                    or "Vector" in mat_output_input.bl_idname
                ):
                    # For displacement
                    ...
                    if used_mode == "mix":
                        ...
                        raise Exception(
                            f"This mode is not supported here: {used_mode}!"
                        )
                ...
                for link in mat_output_input.links:
                    self.link(
                        link.from_socket, infuse_node.inputs[input_offset]
                    )
                self.link(
                    group_node.outputs[mat_output_input.name],
                    infuse_node.inputs[input_offset + 1],
                )
                self.link(infuse_output, mat_output_input)

    def set_displacement_from_principled_shader_value(
        self, input_name: str, multiply_factor: float
    ):
        ...
        if not isinstance(input_socket, bpy.types.NodeSocket):
            raise Exception(
                f"The input {input_name} of the principled shader does not have any incoming connection."
            )
    ...
    def __setattr__(self, key, value):
        if key not in ["links", "nodes", "blender_obj"]:
            raise RuntimeError(
                "The API class does not allow setting any attribute. Use the corresponding method or "
