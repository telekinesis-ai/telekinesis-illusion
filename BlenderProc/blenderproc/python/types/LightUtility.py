...
class Light(Entity):
    ...
    def set_radius(self, radius: float, frame: Optional[int] = None):
        ...
        Utility.insert_keyframe(
            self.blender_obj.data, "shadow_soft_size", frame
        )
    ...
    def setup_as_projector(
        self, pattern: np.ndarray, frame: Optional[int] = None
    ):
        ...
        if pattern.dtype == np.uint8:
            pattern = (
                pattern / 255.0
            )  # manual cast to range [0,1] to avoid integer casting issues below
        ...
        # Set Up Links
        links.new(
            node_pattern.outputs["Color"], node_ox.inputs["Color"]
        )  # Link Image Texture to Emission
