...
class Armature(Entity):
    ...
    def set_rotation_euler(
        self,
        rotation_euler: Union[float, list, Euler, np.ndarray],
        ...
        if mode == "absolute":
            if isinstance(rotation_euler, float):
                ...
                print(
                    f"Set rotation_euler of armature {self.get_name()} to {rotation_euler}"
                )
            else:
        ...
        # in relative mode we add the rotation to the current value
            ...
            if isinstance(rotation_euler, float):
                ...
                print(
                    f"Relatively rotated armature {self.get_name()} around axis {axis} for {rotation_euler} radians"
                )
            else:
                for axis, rotation in zip(["X", "Y", "Z"], rotation_euler):
                    bone.rotation_euler.rotate_axis(axis, rotation)
                print(
                    f"Relatively rotated armature {self.get_name()} for {rotation_euler} radians"
                )
    ...
    def _determine_rotation_axis(self):
        ...
        assert c is not None, (
            "Tried to determine the single rotation axis but no rotation constraints are set!"
        )
    ...
    def _clip_value_from_constraint(
        self, value: float, constraint_name: str, axis: str
    ) -> float:
        """
    ...
    def add_constraint_if_not_existing(
        ...
        if (
            constraint_name
            not in self.blender_obj.pose.bones["Bone"].constraints.keys()
        ):
            self.blender_obj.pose.bones["Bone"].constraints.new(
                constraint_name.upper().replace(" ", "_")
            )
        return self.blender_obj.pose.bones["Bone"].constraints[constraint_name]

    def set_rotation_constraint(
        self,
        ...
        constraint = self.add_constraint_if_not_existing(
            constraint_name="Limit Rotation"
        )
    ...
    def set_location_constraint(
        ...
        # add new constraint if it doesn't exist
        constraint = self.add_constraint_if_not_existing(
            constraint_name="Limit Location"
        )
    ...
    def get_constraint(
        ...
        if (
            constraint_name
            in self.blender_obj.pose.bones["Bone"].constraints.keys()
        ):
            return self.blender_obj.pose.bones["Bone"].constraints[
                constraint_name
            ]
        return None
