...
class URDFObject(Entity):
    ...
    def __init__(
        ...
        links: List[Link],
        xml_tree: Optional["urchin.URDF"] = None,
    ):
    ...
    def get_all_collision_objs(self) -> List[MeshObject]:
        ...
        return [
            obj
            for obj in self.get_all_urdf_objs()
            if "collision" in obj.get_name()
        ]
    ...
    def get_all_inertial_objs(self) -> List[Inertial]:
        ...
        """
        return [
            obj for obj in self.get_all_urdf_objs() if isinstance(obj, Inertial)
        ]
    ...
    def hide_links_and_collision_inertial_objs(self):
        ...
        for link in self.links:
            for obj in link.get_all_objs():
                if (
                    "collision" in obj.get_name()
                    or "inertial" in obj.get_name()
                ):
                    obj.hide()

    def set_ascending_category_ids(
    ...
    def get_all_visual_local2world_mats(self) -> np.array:
        ...
        return np.stack(
            [
                link.get_visual_local2world_mats(
                    Matrix(self.get_local2world_mat())
                )
                for link in self.links
            ]
        )
    ...
    def get_all_collision_local2world_mats(self) -> np.array:
        ...
        return np.stack(
            [
                link.get_collision_local2world_mats(
                    Matrix(self.get_local2world_mat())
                )
                for link in self.links
            ]
    ...
    def get_all_inertial_local2world_mats(self) -> np.array:
        ...
        return np.stack(
            [
                link.get_inertial_local2world_mat(
                    Matrix(self.get_local2world_mat())
                )
                for link in self.links
            ]
    ...
    def create_ik_bone_controller(
        ...
        if link is None:
            link = self.links[-1]
        ik_bone_controller, ik_bone_constraint, offset = (
            link.create_ik_bone_controller(
    ...
    def set_rotation_euler_fk(
        ...
        if link is not None:
            ...
            if isinstance(rotation_euler, list) and len(revolute_joints) == len(
                ...
                for revolute_joint, rotation in zip(
                    revolute_joints, rotation_euler
                ):
    ...
    def set_rotation_euler_ik(
        ...
        self.ik_link.set_rotation_euler_ik(
            rotation_euler=rotation_euler, mode=mode
        )
        self._set_keyframe(name="rotation_euler", frame=frame)
    ...
    def has_reached_ik_pose(
        ...
        """
        curr_offset = (
            self.ik_bone_controller.matrix.inverted()
            @ self.ik_bone_constraint.matrix
        )
