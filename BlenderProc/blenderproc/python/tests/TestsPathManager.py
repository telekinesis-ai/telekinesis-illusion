...
class TestsPathManager:
    ...
    def __init__(self):
        ...
        self.example_resources = abspath(
            join(self._main_folder, "examples/resources")
        )

        # for cc materials
        self._add_parameter(
            "cc_materials", "resources/cctextures", "BP_CC_MATERIALS_PATH"
        )

        self._add_parameter("haven", "resources/haven", "BP_HAVEN_PATH")

    def _add_parameter(
        self, param_name: str, default_path: str, environment_key: str
    ):
        """
        ...
        """
        setattr(
            self, param_name, abspath(join(self._main_folder, default_path))
        )
        if not exists(getattr(self, param_name)):
            if environment_key in os.environ:
                setattr(
                    self, param_name, resolve_path(os.environ[environment_key])
                )
            if not exists(getattr(self, param_name)):
                raise Exception(
                    f'The env variable: "{environment_key}" is empty or does not exist and the default '
