...
def light_suncg_scene(
    ...
    for obj in get_all_mesh_objects():
        if obj.has_cp("modelId"):
            ...
            if obj_id in lights:
                _SuncgLighting.make_lamp_emissive(
                    obj,
                    lights[obj_id],
                    collection_of_mats,
            ...
            if obj.get_name().startswith("Ceiling#"):
                _SuncgLighting.make_ceiling_emissive(
                    obj, collection_of_mats, ceiling_emission_strength
                )
...
class _SuncgLighting:
    ...
    @staticmethod
    def make_lamp_emissive(
        obj: MeshObject,
        light: Tuple[List[str], List[str]],
        collection_of_mats: Dict[str, Dict[str, Material]],
        lightbulb_emission_strength: float = 15,
        lampshade_emission_strength: float = 7,
    ):
        ...
        for i, m in enumerate(obj.get_materials()):
            ...
            if mat_name in light[0] or mat_name in light[1]:
                ...
                if old_mat_name in collection_of_mats["lamp"]:
                    ...
                    obj.set_material(
                        i, collection_of_mats["lamp"][old_mat_name]
                    )
                    continue
    ...
    @staticmethod
    def make_window_emissive(
        obj: MeshObject, collection_of_mats: Dict[str, Dict[str, Material]]
    ):
            ...
            if alpha < 1:
                ...
                if not m.get_nodes_with_type("Emission"):
                    ...
                    m.make_emissive(
                        ...
                        non_emissive_color_socket=transparent_node.outputs[
                            "BSDF"
                        ],
                    )
    ...
    def make_ceiling_emissive(
        ...
        for i, material in enumerate(obj.get_materials()):
            ...
            if not material.get_nodes_with_type(
                "Emission"
            ) and material.get_nodes_with_type("BsdfPrincipled"):
                material.make_emissive(
                    emission_strength=ceiling_emission_strength,
                    emission_color=[1, 1, 1, 1],
                )
