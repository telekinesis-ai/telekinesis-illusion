...
from blenderproc.python.camera.CameraUtility import (
    get_camera_frustum,
    is_point_inside_camera_frustum,
    get_camera_pose,
...
def _default_light_pose_sampling(frustum_vertices: np.ndarray) -> np.ndarray:
    ...
    cube_diag = (
        np.linalg.norm(
            np.max(frustum_vertices, axis=0) - np.min(frustum_vertices, axis=0)
        )
        * 0.5
    )
    sampled_light_pose = part_sphere(
...
def _default_look_at_pose_sampling(
    frustum_vertices: np.ndarray, _sampled_light_pose: np.ndarray
) -> np.ndarray:
...
def add_intersecting_spot_lights_to_camera_poses(
    clip_start: float,
    clip_end: float,
    perform_look_at_intersection_check: bool = True,
    perform_look_at_pose_visibility_check: bool = True,
    light_pose_sampling: Optional[Callable[[np.ndarray], np.ndarray]] = None,
    look_at_pose_sampling: Optional[
        Callable[[np.ndarray, np.ndarray], np.ndarray]
    ] = None,
    max_tries_per_cam_pose: int = 10000,
) -> Light:
    ...
    bvh_tree = None
    if (
        perform_look_at_pose_visibility_check
        or perform_look_at_intersection_check
    ):
    ...
    for frame_id in range(
        ...
        with KeyFrame(frame_id):
            ...
            for _ in range(max_tries_per_cam_pose):
                ...
                if not is_point_inside_camera_frustum(
                    sampled_pose
                ) and is_point_inside_camera_frustum(look_at_point):
                    ...
                    if perform_look_at_pose_visibility_check:
                        ...
                        _, _, _, dist = bvh_tree.ray_cast(
                            look_at_point, look_dir, np.linalg.norm(look_dir)
                        )
                        if dist is not None:
                    ...
                    if perform_look_at_intersection_check:
                        ...
                        _, _, _, dist = bvh_tree.ray_cast(
                            sampled_pose, look_dir, np.linalg.norm(look_dir)
                        )
                        if dist is not None:
            ...
            if not found_pose:
                raise RuntimeError(
                    "No pose found, increase the start and end clip or increase the amount of tries."
                )
    return new_light
