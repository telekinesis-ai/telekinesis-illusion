...
def all_with_type(
    elements: List[Struct], filtered_data_type: Type[Struct] = None
) -> List[Struct]:
    ...
    if filtered_data_type is not None:
        return list(
            filter(lambda x: isinstance(x, filtered_data_type), elements)
        )
    return elements
...
def by_attr(
    ...
    attr_name: str,
    value: Any,
    filtered_data_type: Type[Struct] = None,
    regex: bool = False,
) -> List[Struct]:
    ...
    return list(
        filter(
            lambda struct: _Filter.check_equality(
                struct.get_attr(attr_name), value, regex
            ),
            elements,
        )
    )
...
def by_cp(
    ...
    return list(
        filter(
            lambda struct: (
                struct.has_cp(cp_name)
                and _Filter.check_equality(struct.get_cp(cp_name), value, regex)
            ),
            elements,
        )
    )
...
def by_attr_in_interval(
    ...
    return list(
        filter(
            lambda struct: (
                (min_value is None or min_value <= struct.get_attr(attr_name))
                and (
                    max_value is None or max_value >= struct.get_attr(attr_name)
                )
            ),
            elements,
        )
...
class _Filter:
    ...
    def check_list_has_length_one(elements: List[Any]) -> Any:
        ...
        if len(elements) > 1:
            raise Exception(
                "More than one element with the given condition has been found."
            )
        if len(elements) == 0:
            raise Exception(
                "No element with the given condition has been found."
            )
        return elements[0]

    @staticmethod
    def check_equality(
        attr_value: Any, filter_value: Any, regex: bool = False
    ) -> bool:
        ...
        try:
            ...
            raise RuntimeError(
                f"Could not broadcast attribute {attr_value} with shape {np.array(attr_value).shape} "
                f"to filter_value {filter_value} with shape {np.array(filter_value).shape}!"
            ) from e
