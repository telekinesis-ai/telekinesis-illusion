...
def change_coordinate_frame_of_point(
    point: Union[np.ndarray, List[float], Vector], new_frame: List[str]
) -> np.ndarray:
    ...
    assert len(new_frame) == 3, (
        f"The specified coordinate frame has more or less than tree axes: {new_frame}"
    )
...
def change_target_coordinate_frame_of_transformation_matrix(
    matrix: Union[np.ndarray, Matrix], new_frame: List[str]
) -> np.ndarray:
    ...
    """
    tmat = MathUtility.build_coordinate_frame_changing_transformation_matrix(
        new_frame
    )
...
def change_source_coordinate_frame_of_transformation_matrix(
    matrix: Union[np.ndarray, Matrix], new_frame: list
) -> np.ndarray:
...
def build_transformation_mat(
    translation: Union[np.ndarray, List[float], Vector],
    rotation: Union[np.ndarray, List[List[float]], Matrix],
) -> np.ndarray:
...
class MathUtility:
    ...
    def build_coordinate_frame_changing_transformation_matrix(
        ...
        """
        assert len(destination_frame) == 3, (
            f"The specified coordinate frame has more or less than "
