...
def generate_random_pattern_img(
    width: int, height: int, n_points: int
) -> np.ndarray:
    ...
    for i, j in itertools.product(
        range(width // m_width), range(height // m_height)
    ):
        x_idx = random.randint(i * m_width, (i + 1) * m_width - 1)
        y_idx = random.randint(j * m_height, (j + 1) * m_height - 1)
        pattern_img = cv2.circle(
            pattern_img, (x_idx, y_idx), 1, (255, 255, 255, 255), -1
        )
