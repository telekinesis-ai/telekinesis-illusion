...
# TODO: Current workaround for managing the undo functionlity for physics sim
...
class Utility:
    ...
    def get_node_connected_to_the_output_and_unlink_it(
        ...
        material_output = Utility.get_the_one_node_with_type(
    ...
    def get_nodes_with_type(
        ...
        node_type: str,
        created_in_func: Optional[str] = None,
    ) -> List[bpy.types.Node]:
        """
        ...
        """
        nodes_with_type = [
            node for node in nodes if node_type in node.bl_idname
        ]
        if created_in_func:
    ...
    def get_the_one_node_with_type(
        ...
        if node and len(node) == 1:
            return node[0]
        raise RuntimeError(
            f"There is not only one node of this type: {node_type}, there are: {len(node)}"
        )
    ...
    def get_nodes_created_in_func(
        ...
        return [
            node
            for node in nodes
            if "created_in_func" in node
            and node["created_in_func"] == created_in_func
        ]

    @staticmethod
    def read_suncg_lights_windows_materials() -> Tuple[
        ...
        with open(
            resolve_resource(
                os.path.join("suncg", "light_geometry_compact.txt")
            ),
            "r",
            encoding="utf-8",
        ) as f:
        ...
        windows = []
        with open(
            resolve_resource(os.path.join("suncg", "ModelCategoryMapping.csv")),
            "r",
            encoding="utf-8",
        ) as csvfile:
    ...
    def add_output_entry(output: Dict[str, str]):
        ...
        if GlobalStorage.is_in_storage("output"):
            ...
            if Utility.output_already_registered(
                output, GlobalStorage.get("output")
            ):
    ...
    def output_already_registered(
        ...
        for _output in output_list:
            if (
                ...
                and output["path"] == _output["path"]
            ):
                # import warnings
                # warnings.warn("Detected output entries with duplicate keys and paths")
                return True
            if (
                output["key"] == _output["key"]
                or output["path"] == _output["path"]
            ):
                raise RuntimeError(
                    "Can not have two output entries with the same key/path but not same path/key."
...
class UndoAfterExecution:
    ...
    def __init__(
        self,
        check_point_name: Optional[str] = None,
        perform_undo_op: bool = True,
    ):
    ...
    def __enter__(self):
        if self._perform_undo_op:
            ...
            for instance in Background.__refs__:
                self.background_instances.append(
    ...
    def __exit__(
        ...
        if self._perform_undo_op:
            ...
            name, background_instances = self.background_instances[0]
            background_instances.update_blender_ref()
