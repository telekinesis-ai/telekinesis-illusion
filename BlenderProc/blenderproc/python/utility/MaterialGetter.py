...
class MaterialGetter:
    ...
    def perform_and_condition_check(
        and_condition, materials, used_materials_to_check=None
    ):
        ...
        for material in used_materials_to_check:
            ...
            for key, value in and_condition.items():
                ...
                if key.startswith("cf_"):
                    ...
                    key = key[3:]
                if (
                    hasattr(material, key)
                    and not requested_custom_property
                    and not requested_custom_function
                ):
                    ...
                    # if not, try to enforce some mathutils-specific type
                        ...
                        if isinstance(getattr(material, key), mathutils.Vector):
                            ...
                            getattr(material, key), mathutils.Euler
                        ):
                            new_value = mathutils.Euler(value)
                        elif isinstance(
                            getattr(material, key), mathutils.Color
                        ):
                            new_value = mathutils.Color(value)
                        # raise an exception if it is none of them
                        else:
                            raise Exception(
                                f"Types are not matching: {type(getattr(material, key))} "
                                "and {type(value)} !"
                            )
                    # or check for equality
                    if not (
                        (
                            isinstance(getattr(material, key), str)
                            and re.fullmatch(value, getattr(material, key))
                            is not None
                        )
                ...
                # check if a custom property with this name exists
                    ...
                    if isinstance(material[key], type(value)) or (
                        isinstance(material[key], int)
                        and isinstance(value, bool)
                    ):
                        # if it is a string and if the whole string matches the given pattern
                        if not (
                            (
                                isinstance(material[key], str)
                                and re.fullmatch(value, material[key])
                                is not None
                            )
                            or material[key] == value
                        ):
                            select_material = False
                            break
                    else:
                    ...
                    if key.startswith("texture_amount_"):
                        if material.use_nodes:
                            ...
                            if "min" in key:
                                ...
                                raise Exception(
                                    f"This type of key is unknown: {key}"
                                )
                        else:
                            select_material = False
                            break
                        ...
                        if material.use_nodes:
                            ...
                            amount_of_principled_bsdf_nodes = (
                                len(principled) if principled is not None else 0
                            )
                            if "min" in key:
                        ...
                        if material.use_nodes:
                            ...
                            extracted_input_name = key[
                                len("principled_bsdf_") : key.rfind("_")
                            ]
