...
class InstallUtility:
    ...
    def determine_blender_install_path(
        ...
        if is_using_external_bpy_module():
            raise RuntimeError(
                "USE_EXTERNAL_BPY_MODULE is set, no reason to determine the blender install path, it's given by the module."
            )
        ...
        if blender_install_path is None:
            ...
            blender_install_path = os.path.join(
                "/home_local", user_name, "blender"
            )
        return custom_blender_path, blender_install_path

    @staticmethod
    def make_sure_blender_is_installed(
        custom_blender_path: Optional[str],
        blender_install_path: str,
        reinstall_blender: bool = False,
    ) -> Tuple[str, str]:
        ...
        if custom_blender_path is None:
            ...
            if blender_install_path is not None:
                ...
                if blender_install_path.startswith(
                    "/home_local"
                ) and not os.path.exists("/home_local"):
                    user_name = getpass.getuser()
                    home_path = (
                    ...
                    print(
                        f"Warning: Changed install path from {join('/home_local', user_name)}... to {home_path}..., "
                    ...
                    blender_install_path = blender_install_path.replace(
                        os.path.join("/home_local", user_name), home_path, 1
                    )
                    blender_install_path = blender_install_path.replace(
                        "/", os.path.sep
                    )
            ...
            if platform in ["linux", "linux2"]:
                ...
                blender_path = os.path.join(
                    blender_install_path, blender_version
                )
                ...
                raise RuntimeError(
                    f"This system is not supported yet: {platform}"
                )
            ...
            if not os.path.exists(blender_path):
                if version_info.major != 3:
                    try:
                        ...
                        print(
                            'For decompressing ".xz" files in python 2.x is it necessary to use lzma'
                        )
                ...
                try:
                    try:
                        ...
                        file_tmp = urlretrieve(
                            url, None, DownloadProgressBar()
                        )[0]
                    except ImportError:
                        print(
                            "Progressbar for downloading, can only be shown, "
                ...
                if platform in ["linux", "linux2"]:
                    if version_info.major == 3:
                        SetupUtility.extract_file(
                            blender_install_path, file_tmp, "TAR"
                        )
                    else:
                    ...
                    os.rename(
                        file_tmp,
                        os.path.join(
                            blender_install_path, blender_version + ".dmg"
                        ),
                    )
                    ...
                    subprocess.Popen(
                        [
                            f"hdiutil attach {os.path.join(blender_install_path, blender_version + '.dmg')}"
                        ],
                        shell=True,
                    ).wait()
                    subprocess.Popen(
                        [
                            f"cp -r {os.path.join('/', 'Volumes', 'Blender', 'Blender.app')} "
                            f"{blender_install_path}"
                        ],
                        shell=True,
                    ).wait()
                    subprocess.Popen(
                        [
                            f"diskutil unmount {os.path.join('/', 'Volumes', 'Blender')}"
                        ],
                        shell=True,
                    )
                    ...
                    subprocess.Popen(
                        [
                            f"rm {os.path.join(blender_install_path, blender_version + '.dmg')}"
                        ],
                        shell=True,
                    ).wait()
                ...
                for folder in os.listdir(blender_install_path):
                    if os.path.isdir(
                        os.path.join(blender_install_path, folder)
                    ) and folder.startswith(
                        f"blender-{major_version}.{minor_version}"
                    ):
                        os.rename(
                            os.path.join(blender_install_path, folder),
            ...
            major_version = None
            if platform == "darwin":
                blender_version_base = os.path.join(
                    blender_path, "Contents", "Resources"
                )
            else:
            ...
            for sub_dir in os.listdir(blender_version_base):
                ...
                if (
                    os.path.isdir(os.path.join(blender_version_base, sub_dir))
