...
class SetupUtility:
    ...
    @staticmethod
    def setup(
        user_required_packages: Optional[List[str]] = None,
        ...
        if is_using_external_bpy_module():
            raise RuntimeError(
                "USE_EXTERNAL_BPY_MODULE is set, calling bproc.setup() is not necessary in external mode."
            )
        ...
        if not SetupUtility.main_setup_called:
            ...
            if is_debug_mode:
                ...
                SetupUtility.setup_utility_paths(
            ...
            if not is_debug_mode:
                ...
                sys.argv = (
                    sys.argv[
                        sys.argv.index("--") + 1 : sys.argv.index("--") + 2
                    ]
    ...
    @staticmethod
    def determine_python_paths(
        blender_path: Optional[str], major_version: Optional[str]
        ...
        if is_using_external_bpy_module():
            raise RuntimeError(
                "USE_EXTERNAL_BPY_MODULE is set, work with packages in the external environment directly."
            )

        # If no blender path is given, determine it based on sys.executable
        if blender_path is None:
            blender_path = os.path.abspath(
                os.path.join(os.path.dirname(sys.executable), "..", "..", "..")
            )
            major_version = os.path.basename(
                os.path.abspath(
                    os.path.join(os.path.dirname(sys.executable), "..", "..")
                )
            )
        ...
        if platform in ["linux", "linux2"]:
            python_bin_folder = os.path.join(
                blender_path, major_version, "python", "bin"
            )
            python_bin = os.path.join(python_bin_folder, current_python_version)
            packages_path = os.path.abspath(
                os.path.join(blender_path, "custom-python-packages")
            )
            packages_import_path = os.path.join(
                packages_path, "lib", current_python_version, "site-packages"
            )
            pre_python_package_path = os.path.join(
                blender_path,
                major_version,
                "python",
                "lib",
            ...
            python_bin_folder = os.path.join(
                blender_path, "..", "Resources", major_version, "python", "bin"
            )
            python_bin = os.path.join(python_bin_folder, current_python_version)
            packages_path = os.path.abspath(
                os.path.join(blender_path, "custom-python-packages")
            ...
            packages_import_path = os.path.join(
                packages_path,
                current_python_version.replace(".", "").capitalize(),
                "site-packages",
            )
            pre_python_package_path = os.path.join(
                blender_path, major_version, "python", "lib", "site-packages"
            )
        else:
        ...
        return (
            python_bin,
            packages_path,
            packages_import_path,
            pre_python_package_path,
        )
    ...
    def setup_pip(
        ...
        major_version: Optional[str] = None,
        reinstall_packages: bool = False,
        use_custom_package_path: bool = True,
        ...
        if reinstall_packages:
            raise ValueError(
                "The reinstall package mode is not supported right now!"
            )
        ...
        result = SetupUtility.determine_python_paths(
        ...
        if SetupUtility.package_list_is_from_cache:
            ...
            found_package_to_install = SetupUtility._pip_install_packages(
        ...
        packages_were_installed = SetupUtility._pip_install_packages(
            required_packages,
            python_bin,
            packages_path,
        ...
        cache_path = os.path.join(
            packages_path, "installed_packages_cache_v2.json"
        )
    ...
    def _pip_install_packages(
        ...
        for package in required_packages:
            ...
            if package_name == "opencv-python":
                raise RuntimeError(
                    "Please use opencv-contrib-python instead of opencv-python, as having both "
            ...
            if package_version is not None and already_installed:
                ...
                already_installed = (
                    package_version
                    == SetupUtility.installed_packages[package_name]
                )
            ...
            if not already_installed or reinstall_packages:
                print(
                    f"Installing pip package {package_name} {package_version}"
                )
                extra_args = []
                ...
                if os.getenv("BLENDER_PROC_NO_PIP_CACHE", "False").lower() in (
                    "true",
                    "1",
                    "t",
                ):
    ...
    def uninstall_pip_packages(
        package_names: List[str], blender_path: str, major_version: str
    ):
        ...
        python_bin, _, packages_import_path, _ = (
            SetupUtility.determine_python_paths(blender_path, major_version)
        )

        # Run pip uninstall
        # pylint: disable=consider-using-with
        subprocess.Popen(
            [python_bin, "-m", "pip", "uninstall"] + package_names,
            env=dict(os.environ, PYTHONPATH=packages_import_path),
        ).wait()
        # pylint: enable=consider-using-with
    ...
    def _ensure_pip(
        python_bin: str,
        packages_path: str,
        packages_import_path: str,
        pre_python_package_path: str,
        ...
        if SetupUtility.installed_packages is None:
            ...
            installed_packages_name, installed_packages_versions = zip(
                *[
                    str(line).lower().split("==")
                    for line in installed_packages.splitlines()
                ]
            )
            installed_packages_name = [
                ele[2:] if ele.startswith("b'") else ele
                for ele in installed_packages_name
            ]
            installed_packages_versions = [
                ele[:-1] if ele.endswith("'") else ele
                for ele in installed_packages_versions
            ]
            SetupUtility.installed_packages = dict(
                zip(installed_packages_name, installed_packages_versions)
            )
    ...
    def clean_installed_packages_cache(blender_path, major_version):
        ...
        if is_using_external_bpy_module():
            raise RuntimeError(
                "USE_EXTERNAL_BPY_MODULE is set, packages are handled in the external environment."
            )

        _, packages_path, _, _ = SetupUtility.determine_python_paths(
            blender_path, major_version
        )
        cache_path = os.path.join(
            packages_path, "installed_packages_cache_v2.json"
    ...
    @staticmethod
    def extract_file(
        output_dir: str, file: Union[str, BytesIO], mode: str = "ZIP"
    ):
        ...
        if os.path.exists(path_to_run_file):
            with open(path_to_run_file, "r", encoding="utf-8") as file:
                ...
                for index, line in enumerate(lines):
                    if (
                        "import blenderproc" in line
                        or "from blenderproc" in line
                    ):
                        return
                    code = "\n".join(lines[: index + 2])
                    raise RuntimeError(
                        f'The given script "{path_to_run_file}" does not have a blenderproc '
            ...
            raise RuntimeError(
                f"The given run script does not exist: {path_to_run_file}"
            )
    ...
    def determine_temp_dir(given_temp_dir: str) -> str:
        ...
        temp_dir = os.path.join(
            temp_dir, "blender_proc_" + str(uuid.uuid4().hex)
        )
        # Create the temp dir
