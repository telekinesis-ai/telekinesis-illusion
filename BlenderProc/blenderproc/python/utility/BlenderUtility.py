...
def add_object_only_with_vertices(
    vertices: List[List[float]], name: str = "NewVertexObject"
) -> bpy.types.Object:
    """
...
def add_object_only_with_direction_vectors(
    vertices: List[List[float]],
    normals: List[List[float]],
    radius: float = 1.0,
    name: str = "NewDirectionObject",
) -> bpy.types.Object:
    """
...
def add_cube_based_on_bb(
    bouding_box: List[Vector], name: str = "NewCube"
) -> bpy.types.Object:
    """
    ...
    if len(bouding_box) != 8:
        raise Exception(
            "The amount of vertices is wrong for this bounding box!"
        )
    mesh = bpy.data.meshes.new("mesh")
    # create new object
    obj = bpy.data.objects.new(name, mesh)
    ...
    bm.faces.new(
        [new_vertices[0], new_vertices[1], new_vertices[2], new_vertices[3]]
    )
    bm.faces.new(
        [new_vertices[0], new_vertices[4], new_vertices[5], new_vertices[1]]
    )
    bm.faces.new(
        [new_vertices[1], new_vertices[5], new_vertices[6], new_vertices[2]]
    )
    bm.faces.new(
        [new_vertices[2], new_vertices[3], new_vertices[7], new_vertices[6]]
    )
    bm.faces.new(
        [new_vertices[0], new_vertices[4], new_vertices[7], new_vertices[3]]
    )
    bm.faces.new(
        [new_vertices[4], new_vertices[5], new_vertices[6], new_vertices[7]]
    )
    bm.to_mesh(mesh)
    bm.free()
    return obj
...
def load_image(file_path: str, num_channels: int = 3) -> np.ndarray:
    ...
    if file_ending in ["exr", "png"]:
        try:
            return imageio.v3.imread(
                file_path, plugin="opencv", flags=cv2.IMREAD_UNCHANGED
            )[:, :, :num_channels]
        except ValueError:
            print(
                "It seems the freeimage library which is necessary to read .exr files cannot "
            ...
            try:
                ...
                error = (
                    "The automatic installation of the freeimage library failed, so you need to install "
                    "the imageio .exr extension manually. This is quite simple: \n"
                )
                error += (
                    "Use a different python environment (not blenders internal environment), "
                    "`pip install imageio`.\n"
                )
                error += (
                    "And then execute the following command in this env: \n"
                )
...
def copy_links(
    ...
    goal_nodes: bpy.types.Nodes,
    goal_links: bpy.types.NodeLinks,
):
    """
...
def copy_nodes_from_mat_to_material(
    from_material: bpy.types.Material, to_material: bpy.types.Material
):
    """
...
def add_nodes_to_group(
    nodes: bpy.types.Node, group_name: str
    ...
    # the group
    material_outputs = Utility.get_nodes_with_type(
        group.nodes, "OutputMaterial"
    )
    if len(material_outputs) == 1:
        for input_node in material_outputs[0].inputs:
            group.interface.new_socket(
                input_node.name,
                in_out="OUTPUT",
                socket_type=input_node.bl_idname,
            )
            for link in input_node.links:
                group.links.new(
                    link.from_socket, group_output.inputs[input_node.name]
                )
