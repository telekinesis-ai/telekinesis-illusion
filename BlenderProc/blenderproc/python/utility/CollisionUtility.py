...
class CollisionUtility:
    ...
    def check_intersections(
        ...
        for collision_obj in objects_to_check_against:
            ...
            intersection = CollisionUtility.check_bb_intersection(
                obj, collision_obj
            )
            # if they do
            if intersection:
                skip_inside_check = (
                    collision_obj in list_of_objects_with_no_inside_check
                )
                # then check for more refined collisions
                intersection, bvh_cache = (
                    CollisionUtility.check_mesh_intersection(
                        obj,
                        collision_obj,
                        bvh_cache=bvh_cache,
    ...
    def check_bb_intersection_on_values(
        ...
        """
        collide = True
        for min_b1_val, max_b1_val, min_b2_val, max_b2_val in zip(
            min_b1, max_b1, min_b2, max_b2
        ):
            # inspired by this:
            ...
            def is_overlapping_1D(x_min_1, x_max_1, x_min_2, x_max_2):
                ...
                return used_check(x_max_1, x_min_2) and used_check(
                    x_max_2, x_min_1
                )

            collide = collide and is_overlapping_1D(
                min_b1_val, max_b1_val, min_b2_val, max_b2_val
            )
        return collide
    ...
    def check_mesh_intersection(
        ...
        if (
            len(obj1.get_mesh().vertices) == 0
            or len(obj2.get_mesh().vertices) == 0
        ):
    ...
    @staticmethod
    def is_point_inside_object(
        obj: MeshObject,
        obj_bvh_tree: mathutils.bvhtree.BVHTree,
        point: Union[Vector, np.ndarray],
    ) -> bool:
        ...
        a = p2.normalized().dot(
            (Euler(obj.get_rotation_euler()).to_matrix() @ normal).normalized()
        )
        return a >= 0.0
