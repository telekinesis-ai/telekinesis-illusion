...
def set_lens_distortion(
    ...
    if all(v == 0.0 for v in [k1, k2, k3, p1, p2]):
        raise Exception(
            "All given lens distortion parameters (k1, k2, k3, p1, p2) are zero."
        )
    ...
    row = np.repeat(
        np.arange(0, original_image_resolution[0]), original_image_resolution[1]
    )
    column = np.tile(
        np.arange(0, original_image_resolution[1]), original_image_resolution[0]
    )

    # P_und is the undistorted pinhole projection at z==1 of all image pixels
    P_und = np.linalg.inv(camera_K_matrix) @ np.vstack(
        (column, row, np.ones(np.prod(original_image_resolution[:2])))
    )
    ...
    while res[-1] > 0.15:
        ...
        error = np.max(
            np.hypot(fx * (x_ - P_und[0, :]), fy * (y_ - P_und[1, :]))
        )
        res.append(error)
        it += 1
        ...
        if (it > 1) and (res[-1] > res[-2] * 0.99999):
            print(
            ...
            if error > 1e9:
                print(
                    "Some (corner) pixels of the desired image are not defined by the used lens distortion model."
                )
                print("We invite you to double-check your distortion model.")
                print(
                    "The parameters k3,p1,p2 can easily overshoot for regions where the calibration "
                    "software had no datapoints."
                )
                print("You can either:")
                print(
                    "- take more projections (ideally image-filling) at the image corners and repeat calibration,"
                )
                print(
                    "- reduce the # of released parameters to calibrate to k1,k2, or"
                )
                print(
                    "- reduce the target image size (subtract some lines and columns from the desired resolution"
                )
                print(
                    "  and subtract at most that number of lines and columns from the main point location)."
                )
                print(
                    "BlenderProc will not generate incomplete images with void regions since these are not "
                    "useful for ML (data leakage)."
                )
                print(
                    "For that, you can use the Matlab code in robotic.de/callab, which robustifies against "
                    "these unstable pixels."
                )
                raise Exception(
                    "The iterative distortion algorithm is unstable."
                )
    ...
    CameraUtility.set_intrinsics_from_K_matrix(
        camera_changed_K_matrix,
        new_image_resolution[0],
        new_image_resolution[1],
        clip_start,
        clip_end,
    )
...
def apply_lens_distortion(
    ...
    if mapping_coords is None or orig_res_x is None or orig_res_y is None:
        ...
        if GlobalStorage.is_in_storage("_lens_distortion_is_used"):
            ...
            raise Exception(
                "Applying of a lens distortion is only possible after calling "
    ...
    def _internal_apply(input_image: np.ndarray) -> np.ndarray:
        ...
        image_distorted = np.zeros(
            (orig_res_y, orig_res_x, amount_of_output_channels)
        )
    ...
    raise Exception(
        f"This type can not be worked with here: {type(image)}, only "
        f"np.ndarray or list of np.ndarray are supported"
    )


def set_camera_parameters_from_config_file(
    camera_intrinsics_file_path: str,
    read_the_extrinsics: bool = False,
    camera_index: int = 0,
    ...
    if not os.path.exists(camera_intrinsics_file_path):
        raise Exception(
            f"The camera intrinsics file does not exist: {camera_intrinsics_file_path}"
        )
    ...
    with open(camera_intrinsics_file_path, "r", encoding="utf-8") as file:
        ...
        for line in file.readlines():
            ...
            if "[" in line and "]" in line:
                ...
                if elements:
                    ...
                    for i in range(len(elements) - 1):
                        ...
                        if _is_number(elements[i]) and _is_number(
                            elements[i + 1]
                        ):
            ...
            if line.startswith("camera."):
                current_nr = line[
                    len("camera.") : line.find(".", len("camera."))
                ]
    ...
    print(
        f"Interpreted intrinsics from DLR-RMC camera calibration file: {extracted_camera_parameters}"
    )
    # check version and origin parameters
    if (
        extracted_camera_parameters.get("version") is None
        or extracted_camera_parameters["version"] != 2
    ):
        if extracted_camera_parameters.get("version") is None:
            raise RuntimeError(
                "The version tag is not set in the DLR-RMC camera calibration file!"
            )
        raise RuntimeError(
            f"Only version 2 is supported for the DLR-RMC camera calibration "
            f"file, not {extracted_camera_parameters['version']}"
        )
    if (
        extracted_camera_parameters.get("origin") is None
        or extracted_camera_parameters["origin"] != "center"
    ):
        raise RuntimeError(
            "The origin in the DLR-RMC camera calibration file has to be defined and "
    ...
    CameraUtility.set_intrinsics_from_K_matrix(
        extracted_camera_parameters.get("A"),
    ...
    mapping_coords = set_lens_distortion(
        extracted_camera_parameters["k1"],
        extracted_camera_parameters.get("k2", 0.0),
    ...
    if read_the_extrinsics:
        ...
        cam2world = change_source_coordinate_frame_of_transformation_matrix(
            cam2world, ["X", "-Y", "-Z"]
        )
