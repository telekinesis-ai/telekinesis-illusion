...
from blenderproc.python.camera.CameraUtility import (
    get_camera_pose,
    get_intrinsics_as_K_matrix,
)


def depth_via_raytracing(
    bvh_tree: BVHTree, frame: Optional[int] = None, return_dist: bool = False
) -> np.ndarray:
    ...
    depth = depth_at_points_via_raytracing(
        bvh_tree, points.reshape(-1, 2), frame, return_dist
    )
...
def depth_at_points_via_raytracing(
    bvh_tree: BVHTree,
    ...
    with KeyFrame(frame):
        ...
        for p in points_2d:
            # Compute current point on plane
                ...
                frame[0]
                + vec_x * (resolution_x - (p[0] + 0.5)) / float(resolution_x)
...
def project_points(
    points: np.ndarray, frame: Optional[int] = None
) -> np.ndarray:
...
def pointcloud_from_depth(
    depth: np.ndarray, frame: Optional[int] = None, depth_cut_off: float = 1e6
) -> np.ndarray:
    ...
    return unproject_points(
        points.reshape(-1, 2), depth.flatten(), frame, depth_cut_off
