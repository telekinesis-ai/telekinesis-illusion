...
def add_camera_pose(
    cam2world_matrix: Union[np.ndarray, Matrix], frame: Optional[int] = None
) -> int:
...
def is_point_inside_camera_frustum(
    point: Union[List[float], Vector, np.ndarray],
    clip_start: Optional[float] = None,
    clip_end: Optional[float] = None,
    frame: Optional[int] = None,
) -> bool:
...
def get_camera_frustum_as_object(
    clip_start: Optional[float] = None,
    clip_end: Optional[float] = None,
    frame: Optional[int] = None,
) -> MeshObject:
...
def rotation_from_forward_vec(
    forward_vec: Union[np.ndarray, Vector],
    up_axis: str = "Y",
    ...
    """
    rotation_matrix = (
        Vector(forward_vec).to_track_quat("-Z", up_axis).to_matrix()
    )
...
def set_intrinsics_from_blender_params(
    lens: float = None,
...
def set_intrinsics_from_K_matrix(
    K: Union[np.ndarray, Matrix],
    image_width: int,
    image_height: int,
    clip_start: float = None,
    clip_end: float = None,
    ...
    if abs(K[0][1]) > 1e-7:
        raise ValueError(
            f"Skew is not supported by blender and therefore "
    ...
    view_fac_in_px = get_view_fac_in_px(
        cam, pixel_aspect_x, pixel_aspect_y, image_width, image_height
    )
    ...
    shift_y = (
        (cy - (image_height - 1) / 2) / view_fac_in_px * pixel_aspect_ratio
    )
...
def get_view_fac_in_px(
    ...
    if cam.sensor_fit == "AUTO":
        if (
            pixel_aspect_x * resolution_x_in_px
            >= pixel_aspect_y * resolution_y_in_px
        ):
...
def get_projection_matrix(
    ...
    height, width = 1.0 / np.tan(y_fov * 0.5), 1.0 / np.tan(x_fov * 0.5)
...
def get_intrinsics_as_K_matrix() -> np.ndarray:
    ...
    cy = (
        resolution_y_in_px - 1
    ) / 2 + cam.shift_y * view_fac_in_px / pixel_aspect_ratio
...
def add_depth_of_field(
    ...
    if focal_point_obj is not None:
        ...
        raise RuntimeError(
            "Either a focal_point_obj have to be given or the focal_distance has to be higher "
            "than zero."
        )
