...
def perform_obstacle_in_view_check(
    cam2world_matrix: Union[Matrix, np.ndarray],
    proximity_checks: dict,
    bvh_tree: BVHTree,
    sqrt_number_of_rays: int = 10,
) -> bool:
    ...
    """
    if (
        not proximity_checks
    ):  # if no checks are in the settings all positions are accepted
        return True
    ...
    for operator in proximity_checks:
        if operator in ["min", "max"] and not isinstance(
            proximity_checks[operator], numbers.Number
        ):
            raise RuntimeError(
        ...
        if operator in ["avg", "var"]:
            if (
                "min" not in proximity_checks[operator]
                or "max" not in proximity_checks[operator]
            ):
                raise RuntimeError(
            ...
            if not isinstance(
                proximity_checks[operator]["min"], numbers.Number
            ) or not isinstance(
                proximity_checks[operator]["max"], numbers.Number
            ):
                raise ValueError(
                    "Threshold must be a number in perform_obstacle_in_view_check"
                )
    ...
    for x in range(0, sqrt_number_of_rays):
        for y in range(0, sqrt_number_of_rays):
            # Compute current point on plane
                ...
                frame[0]
                + vec_x * x / float(sqrt_number_of_rays - 1)
                + vec_y * y / float(sqrt_number_of_rays - 1)
            )
            # Send ray from the camera position through the current point on the plane
            if no_range_distance:
                _, _, _, dist = bvh_tree.ray_cast(position, end - position)
            else:
                _, _, _, dist = bvh_tree.ray_cast(
            ...
            if dist is not None:
                if (
                    "min" in proximity_checks
                    and dist <= proximity_checks["min"]
                ):
                    return False
                if (
                    "max" in proximity_checks
                    and dist >= proximity_checks["max"]
                ):
                    return False
    ...
    if "avg" in proximity_checks:
        ...
        if (
            avg >= proximity_checks["avg"]["max"]
            or avg <= proximity_checks["avg"]["min"]
        ):
            return False
    ...
    if "var" in proximity_checks:
        ...
        if (
            var >= proximity_checks["var"]["max"]
            or var <= proximity_checks["var"]["min"]
        ):
            return False
...
def visible_objects(
    cam2world_matrix: Union[Matrix, np.ndarray], sqrt_number_of_rays: int = 10
) -> Set[MeshObject]:
    ...
    for x in range(0, sqrt_number_of_rays):
        for y in range(0, sqrt_number_of_rays):
            ...
            _, _, _, _, hit_object, _ = bpy.context.scene.ray_cast(
                bpy.context.evaluated_depsgraph_get(), position, end - position
            )
...
def scene_coverage_score(
    cam2world_matrix: Union[Matrix, np.ndarray],
    special_objects: list = None,
    special_objects_weight: float = 2,
    sqrt_number_of_rays: int = 10,
) -> float:
    ...
    for x in range(0, sqrt_number_of_rays):
        for y in range(0, sqrt_number_of_rays):
            ...
            if hit:
                is_of_special_dataset = (
                    "is_suncg" in hit_object or "is_3d_front" in hit_object
                )
                is_suncg_object = (
                    "suncg_type" in hit_object
                    and hit_object["suncg_type"] == "Object"
                )
                is_front_3d_object = (
                    "3D_future_type" in hit_object
                    and hit_object["3D_future_type"] == "Object"
                )
                if (
                    is_of_special_dataset
                    and is_suncg_object
                    or is_of_special_dataset
                    and is_front_3d_object
                ):
...
def decrease_interest_score(
    interest_score: float, min_interest_score: float, interest_score_step: float
):
...
def check_novel_pose(
    cam2world_matrix: Union[Matrix, np.ndarray],
    existing_poses: List[Union[Matrix, np.ndarray]],
    check_pose_novelty_rot: bool,
    check_pose_novelty_translation: bool,
    min_var_diff_rot: float = -1,
    min_var_diff_translation: float = -1,
):
    ...
    def _variance_constraint(array, new_val, old_var, diff_threshold, mode):
        ...
        if (
            diff < diff_threshold
        ):  # Check if the variance increased sufficiently
            array.pop()
            return False
    ...
    if len(existing_poses) > 0:  # First pose is always novel
        ...
        if check_pose_novelty_translation:
            translations = [
                Matrix(pose).to_translation() for pose in existing_poses
            ]
            var_translation = np.var(translations)

            if not _variance_constraint(
                translations,
                cam2world_matrix.to_translation(),
                var_translation,
