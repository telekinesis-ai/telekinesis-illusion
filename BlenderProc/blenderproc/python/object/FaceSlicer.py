...
def slice_faces_with_normals(
    mesh_object: MeshObject,
    compare_angle_degrees: float = 7.5,
    up_vector_upwards: Optional[np.array] = None,
    new_name_for_object: str = "Surface",
) -> Optional[MeshObject]:
    ...
    list_of_median_poses: List[Tuple[float, bmesh.types.BMFace]] = [
        (
            FaceSlicer.get_median_face_pose(
                f, mesh_object.get_local2world_mat()
            )[2],
            f,
        )
        for f in bm.faces
        if FaceSlicer.check_face_angle(
            f,
            mesh_object.get_local2world_mat(),
            up_vector_upwards,
            np.deg2rad(compare_angle_degrees),
        )
    ]

    list_of_median_poses_only_z_value = [
        value for value, face in list_of_median_poses
    ]
...
def extract_floor(
    ...
    compare_angle_degrees: float = 7.5,
    compare_height: float = 0.15,
    up_vector_upwards: bool = True,
    height_list_path: str = None,
    new_name_for_object: str = "Floor",
    should_skip_if_object_is_already_there: bool = False,
) -> List[MeshObject]:
    ...
    if (
        ...
        and new_name_for_object.lower() in object_names
    ):
    ...
    for obj in mesh_objects:
        ...
        if height_list:
            ...
            for height_val in height_list:
                counter = FaceSlicer.select_at_height_value(
                    bm,
                    height_val,
                    compare_height,
                    up_vec,
            ...
            list_of_median_poses: Union[List[float], np.ndarray] = [
                FaceSlicer.get_median_face_pose(f, obj.get_local2world_mat())[2]
                for f in bm.faces
                if FaceSlicer.check_face_angle(
                    f,
                    obj.get_local2world_mat(),
            ...
            if not list_of_median_poses:
                print(
                    f"Object with name: {obj.get_name()} is skipped no faces were relevant, try with "
                    f"flipped up_vec"
                )
                list_of_median_poses = [
                    FaceSlicer.get_median_face_pose(
                        f, obj.get_local2world_mat()
                    )[2]
                    for f in bm.faces
                    if FaceSlicer.check_face_angle(
                        f,
                ...
                if not list_of_median_poses:
                    print(
                        f"Still no success for: {obj.get_name()} skip object."
                    )
...
class FaceSlicer:
    ...
    @staticmethod
    def select_at_height_value(
        bm: bmesh.types.BMesh,
        height_value: float,
        compare_height: float,
        up_vector: Union[mathutils.Vector, np.ndarray],
        cmp_angle: float,
        matrix_world: Union[mathutils.Matrix, np.ndarray],
    ) -> int:
        """
    ...
    def check_face_with(
        ...
        if fabs(median_pose[2] - height_value) < cmp_height:
            return FaceSlicer.check_face_angle(
                face, matrix_world, up_vector, cmp_angle
            )
        return False
