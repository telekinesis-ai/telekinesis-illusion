...
def simulate_physics_and_fix_final_poses(
    ...
    with UndoAfterExecution():
        ...
        origin_shifts = simulate_physics(
            min_simulation_time,
            max_simulation_time,
    ...
    for obj in get_all_mesh_objects():
        if obj.has_rigidbody_enabled():
            ...
            has_compound_parent = (
                obj.get_parent() is not None
                and isinstance(obj.get_parent(), MeshObject)
                and obj.get_parent().get_rigidbody() is not None
                and obj.get_parent().get_rigidbody().collision_shape
                == "COMPOUND"
            )
            if obj.get_rigidbody().type == "ACTIVE" and not has_compound_parent:
                ...
                R_obj_before_sim = mathutils.Euler(
                    obj_poses_before_sim[obj.get_name()]["rotation"]
                ).to_matrix()
...
def simulate_physics_and_persist_all_frames(
    min_simulation_time: float = 4.0,
    max_simulation_time: float = 40.0,
    check_object_interval: float = 2.0,
    object_stopped_location_threshold: float = 0.01,
    ...
    for obj in get_all_mesh_objects():
        if obj.has_rigidbody_enabled():
            ...
            if obj.get_rigidbody().type == "ACTIVE" and not has_compound_parent:
                ...
                for frame in range(final_frame):
                    R_obj_after = mathutils.Euler(
                        obj_poses[frame][obj.get_name()]["rotation"]
                    ).to_matrix()
                    R_obj_rel = R_obj_before_sim @ R_obj_after.transposed()
                    # Apply relative rotation to origin shift
                    origin_shift = R_obj_rel.transposed() @ mathutils.Vector(
                    ...
                    # Fix pose of object to the one it had at the end of the simulation
                    obj.set_location(
                        obj_poses[frame][obj.get_name()]["location"]
                        - origin_shift,
                        frame,
                    )
                    obj.set_rotation_euler(
                        obj_poses[frame][obj.get_name()]["rotation"], frame
                    )
...
def simulate_physics(
    ...
    for obj in get_all_mesh_objects():
        if obj.has_rigidbody_enabled():
            ...
            new_origin = obj.set_origin(
                mode="ORIGIN_CENTER_OF_VOLUME"
                if use_volume_com
                else "CENTER_OF_MASS"
            )
            origin_shift[obj.get_name()] = new_origin - prev_origin

            # TODO: Currently not required as the scale of all objects is assumed to be 1
    ...
    # Perform simulation
    _PhysicsSimulation.do_simulation(
        min_simulation_time,
        max_simulation_time,
        check_object_interval,
        object_stopped_location_threshold,
...
class _PhysicsSimulation:
    ...
    def do_simulation(
        ...
        if min_simulation_time >= max_simulation_time:
            raise Exception(
                "max_simulation_iterations has to be bigger than min_simulation_iterations"
            )

        # Run simulation starting from min to max in the configured steps
        for current_time in np.arange(
            min_simulation_time, max_simulation_time, check_object_interval
        ):
            current_frame = _PhysicsSimulation.seconds_to_frames(current_time)
            logger.info(
                f"Running simulation up to {current_time} seconds ({current_frame} frames)"
            )
            ...
            bpy.context.scene.frame_set(
                current_frame - _PhysicsSimulation.seconds_to_frames(1)
            )
            ...
            if _PhysicsSimulation.have_objects_stopped_moving(
                ...
                logger.info(
                    f"Objects have stopped moving after {current_time} seconds ({current_frame} frames)"
                )
                break
        ...
        for obj in objects_with_physics:
            if obj.rigid_body.type == "ACTIVE":
                ...
                rotation = mathutils.Vector(
                    bpy.context.scene.objects[obj.name].matrix_world.to_euler()
                )
                objects_poses.update(
                    {obj.name: {"location": location, "rotation": rotation}}
                )

        return objects_poses

    @staticmethod
    def have_objects_stopped_moving(
        last_poses: dict,
        new_poses: dict,
        object_stopped_location_threshold: float,
        object_stopped_rotation_threshold: float,
        ...
        for obj_name in last_poses:
            ...
            location_diff = (
                last_poses[obj_name]["location"]
                - new_poses[obj_name]["location"]
            )
            ...
            rotation_diff = (
                last_poses[obj_name]["rotation"]
                - new_poses[obj_name]["rotation"]
            )
            stopped = stopped and not any(
