...
def replace_objects(
    objects_to_be_replaced: List[MeshObject],
    objects_to_replace_with: List[MeshObject],
    ignore_collision_with: Optional[List[MeshObject]] = None,
    ...
    objects_to_be_replaced = random.sample(
        objects_to_be_replaced,
        k=int(len(objects_to_be_replaced) * replace_ratio),
    )
    ...
    for current_object_to_be_replaced in objects_to_be_replaced:
        ...
        while tries < max_tries:
            ...
            if _ObjectReplacer.replace(
                current_object_to_be_replaced,
                ...
                duplicate_new_object = (
                    current_object_to_replace_with.duplicate()
                )
                ...
                if copy_properties:
                    for (
                        key,
                        value,
                    ) in current_object_to_be_replaced.get_all_cps().items():
                ...
                print(
                    "Replaced ",
                    current_object_to_be_replaced.get_name(),
                    " by ",
                    duplicate_new_object.get_name(),
                )
        ...
        if tries == max_tries:
            print(
                "Could not replace " + current_object_to_be_replaced.get_name()
            )
    ...
    def replace(
        ...
        if scale:
            obj_to_add.set_scale(
                _ObjectReplacer.bb_ratio(
                    obj_to_remove.get_bound_box(True),
        ...
        objects_to_check_against = [
            obj
            for obj in check_collision_with
            if obj not in (obj_to_add, obj_to_remove)
        ]
        return CollisionUtility.check_intersections(
            obj_to_add, None, objects_to_check_against, []
        )
