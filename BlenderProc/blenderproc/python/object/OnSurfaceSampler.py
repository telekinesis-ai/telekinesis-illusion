...
def sample_poses_on_surface(
    ...
    for obj in objects_to_sample:
        ...
        for i in range(max_tries):
            ...
            if not _OnSurfaceSampler.check_above_surface(
                obj, surface, up_direction, check_all_bb_corners_over_surface
            ):
            ...
            if not _OnSurfaceSampler.check_spacing(
                obj, placed_objects, min_distance, max_distance
            ):
            ...
            print(
                f'Placed object "{obj.get_name()}" successfully at {obj.get_location()} after {i + 1} iterations!'
            )
...
class _OnSurfaceSampler:
    ...
    def check_above_surface(
        ...
        return surface.position_is_above_object(
            center + up_direction,
            -up_direction,
            check_no_objects_in_between=False,
        )
