...
def construct_random_room(
    ...
    interior_objects: List[MeshObject],
    ...
    floor_obj, wall_obj, ceiling_obj = _construct_random_room(
        used_floor_area,
        amount_of_extrusions,
        fac_from_square_room,
        corridor_width,
        wall_height,
    ...
    _assign_materials_to_floor_wall_ceiling(
        floor_obj, wall_obj, ceiling_obj, assign_material_to_ceiling, materials
    )
    ...
    for face in bm.faces:
        ...
        bb_min_point, bb_max_point = (
            np.min(list_of_verts, axis=0),
            np.max(list_of_verts, axis=0),
        )
    ...
    step_size = (
        1.0 / amount_of_objects_per_sq_meter * float(len(interior_objects))
    )
            ...
            if face_size < step_size:
                ...
                if current_accumulated_face_size > step_size:
                    for _ in range(placement_tries_per_face):
                        found_spot = _sample_new_object_poses_on_face(
                            current_obj,
                            face_bb,
                ...
                # face size is bigger than one step
                amount_of_steps = int(
                    (face_size + current_accumulated_face_size) / step_size
                )
...
def _construct_random_room(
    used_floor_area: float,
    amount_of_extrusions: int,
    fac_from_square_room: float,
    corridor_width: float,
    wall_height: float,
    amount_of_floor_cuts: int,
    ...
    for i in range(1, amount_of_extrusions):
        ...
        boundary_sizes = [
            (e, s) for e, s in boundary_sizes if s > corridor_width
        ]
            ...
            while edge_counter < len(used_edges):
                ...
                for tested_dir in [1, -1]:
                    ...
                    for existing_face in bm.faces:
                        ...
                        if CollisionUtility.check_bb_intersection_on_values(
...
def _assign_materials_to_floor_wall_ceiling(
...
def _sample_new_object_poses_on_face(
    current_obj: MeshObject,
    face_bb,
