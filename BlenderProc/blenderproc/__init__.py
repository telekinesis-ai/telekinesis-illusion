...
from .python.utility.SetupUtility import (
    SetupUtility,
...
if is_using_external_bpy_module():
    try:
        # Importing bpy puts Blender's *user* extensions site-packages ahead of the
        # active environment's, which lets the wheels bundled with an installed
        # add-on shadow (and binary-mismatch against) the environment's packages.
        # Must run before `import bpy`. Best-effort: BlenderProc stays usable
        # without the telekinesis package present.
        try:
            from telekinesis.illusion.utils.blender_env import (
        ...
        print(
            f"BlenderProc is using external 'bpy' ({bpy.app.version_string}) module found in the environment."
        )
        ...
        os.environ.setdefault(
            "INSIDE_OF_THE_INTERNAL_BLENDER_PYTHON_ENVIRONMENT", "1"
        )
    except ImportError:
        raise RuntimeError(
...
if "INSIDE_OF_THE_INTERNAL_BLENDER_PYTHON_ENVIRONMENT" in os.environ:
    ...
    file_names_of_stack = [
        os.path.basename(file_summary.filename)
        for file_summary in stack_summary
    ]
    # check if blenderproc is called via python3 -m blenderproc ...
    is_module_call = (
        file_names_of_stack[0] == "runpy.py"
        or file_names_of_stack[0] == "blenderproc-script.py"
    )
    if sys.platform == "win32":
        is_bproc_shell_called = file_names_of_stack[2] in [
            "metadata.py",
            "__main__.py",
        ]
        is_command_line_script_called = (
            file_names_of_stack[0] == "command_line.py"
        )

        is_correct_startup_command = (
            is_bproc_shell_called
            or is_command_line_script_called
            or is_module_call
        )
    else:
        is_bproc_shell_called = file_names_of_stack[0] in [
            "blenderproc",
            "command_line.py",
        ]
    ...
    if (
        ...
        and not is_using_external_bpy_module()
    ):
