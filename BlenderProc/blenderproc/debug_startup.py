...
class RunBlenderProcOperator(bpy.types.Operator):
    ...
    bl_description = (
        "This operator runs the loaded BlenderProc script and also makes sure to unload all "
    ...
    def execute(self, _):
        ...
        for module in list(sys.modules.keys()):
            if module.startswith("blenderproc") and module not in [
...
def draw(self, context):
    ...
    row = layout.row(align=True)
    row.template_ID(
        st, "text", new="text.new", unlink="text.unlink", open="text.open"
    )
    ...
    if context_text:
        ...
        if is_osl:
            row.operator(
                "node.shader_script_update", text="", icon="FILE_REFRESH"
            )
        else:
            row = layout.row()
